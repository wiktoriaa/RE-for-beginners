...
16 байт в 64-битной)}
\EN{This means that the memory footprint is enlarged by 2 \glslink{word}{words} for each element (8 bytes in 32-bit environment or 16
bytes in 64-bit)}.
...
\RU{он не будет передвинут куда-либо до самого уничтожения списка}\EN{it will not be moved until the list's disposal}.
...
\EN{This mean that this operation is $O(n)$, i.e., it gets slower as the list grows}.
...
\EN{This means that the .size() method is very fast ($O(1)$): it just reads one value from memory}.
