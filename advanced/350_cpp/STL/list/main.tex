...
\RU{Хотя и стандарт \Cpp не указывает, как он должен быть реализован, реализации
MSVC и GCC простые и похожи друг на друга, так что этот исходный код для обоих:}
\EN{Although the \Cpp standard does not say how to implement it, 
...
\RU{Иметь элемент-пустышку (\IT{dummy node} или \IT{sentinel node}) это очень популярная практика в реализации двусвязных списков.}%
\EN{Having a dummy element (\ac{AKA} \IT{sentinel node}) is a very popular practice in implementing doubly-linked lists.}
...
\EN{\TT{operator--} and \TT{operator++} just set the current iterator's value to the \TT{current\_node->prev} or \TT{current\_node->next}  values.}%
\RU{\TT{operator--} и \TT{operator++} просто выставляют текущее значение итератора на \TT{current\_node->prev} или \TT{current\_node->next}.}
