include(`commons.m4')main proc near
    ...
    mov  [esp+4], esi
    mov  [esp], ebx
    mov  dword ptr [esp+18h], 1 ; X _EN(`for new element')_RU(`нового элемента')
    mov  dword ptr [esp+1Ch], 2 ; Y _EN(`for new element')_RU(`нового элемента')
    call _ZNSt4listI1aSaIS0_EE10push_frontERKS0_ ; std::list<a,std::allocator<a>>::push_front(a const&)
    mov  [esp+4], esi
    mov  [esp], ebx
    mov  dword ptr [esp+18h], 3 ; X _EN(`for new element')_RU(`нового элемента')
    mov  dword ptr [esp+1Ch], 4 ; Y _EN(`for new element')_RU(`нового элемента')
    call _ZNSt4listI1aSaIS0_EE10push_frontERKS0_ ; std::list<a,std::allocator<a>>::push_front(a const&)
    mov  dword ptr [esp], 10h
    mov  dword ptr [esp+18h], 5 ; X _EN(`for new element')_RU(`нового элемента')
    mov  dword ptr [esp+1Ch], 6 ; Y _EN(`for new element')_RU(`нового элемента')
