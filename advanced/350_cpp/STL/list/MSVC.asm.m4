include(`commons.m4')_l$ = -16 ; size = 8
_t1$ = -8 ; size = 8
_main    PROC
    sub  esp, 16
    push ebx
    ...
    ; _EN(`allocate first "garbage" element')_RU(`выделить первый "мусорный" элемент')
    ...
    lea  ecx, DWORD PTR _l$[esp+36]
    push esi
    mov  DWORD PTR _t1$[esp+40], 1 ; _EN(`data for a new node')_RU(`данные для нового узла')
    mov  DWORD PTR _t1$[esp+44], 2 ; _EN(`data for a new node')_RU(`данные для нового узла')
    ; allocate new node
    ...
    mov  DWORD PTR _t1$[esp+28], 3 ; _EN(`data for a new node')_RU(`данные для нового узла')
    ...
    lea  ecx, DWORD PTR _l$[esp+36]
    push esi
    mov  DWORD PTR _t1$[esp+44], 4 ; _EN(`data for a new node')_RU(`данные для нового узла')
    ; allocate new node
    call ??$_Buynode@ABUa@@@?$_List_buy@Ua@@V?$allocator@Ua@@@std@@@std@@QAEPAU?$_List_node@Ua@@PAX@1@PAU21@0ABUa@@@Z ; std::_List_buy<a,std::allocator<a> >::_Buynode<a const &>
    mov  DWORD PTR [esi+4], eax
    mov  ecx, DWORD PTR [eax+4]
    mov  DWORD PTR _t1$[esp+28], 5 ; _EN(`data for a new node')_RU(`данные для нового узла')
    mov  DWORD PTR [ecx], eax
    lea  eax, DWORD PTR _t1$[esp+28]
    push eax
    ...
    lea  ecx, DWORD PTR _l$[esp+36]
    push ebx
    mov  DWORD PTR _t1$[esp+44], 6 ; _EN(`data for a new node')_RU(`данные для нового узла')
    ; allocate new node
    call ??$_Buynode@ABUa@@@?$_List_buy@Ua@@V?$allocator@Ua@@@std@@@std@@QAEPAU?$_List_node@Ua@@PAX@1@PAU21@0ABUa@@@Z ; std::_List_buy<a,std::allocator<a> >::_Buynode<a const &>
    ...
    call edi ; printf
    push DWORD PTR [ebx] ; _EN(`get next field of node $l$ variable points to')_RU(``взять поле следующего узла, на который указывает $l$'')
    ...
    call edi ; printf
    push DWORD PTR [ebx+12] ; _EN(`use x and y fields from the node $l$ variable points to')_RU(``использовать поля x и y того узла, на который указывает переменная $l$'')
    ...
    ; prev=next? 
    ; _EN(``it is the only element, "garbage one"'')_RU(``это единственный элемент, "мусор"'')?
    ; _EN(``if yes, do not delete it'')_RU(``если да, не удаляем его'')!
    cmp  edx, ebx
    je   SHORT $LN349@main
