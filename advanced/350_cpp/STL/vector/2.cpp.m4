...
struct vector_of_ints
{
    ...
    // _EN(``GCC structure is the same, but names are'')_RU(``структура в GCC такая же, а имена там''): _M_start, _M_finish, _M_end_of_storage
};
...
int main()
{
    ...
    c.push_back(6);
    dump ((struct vector_of_ints*)(void*)&c);
    printf ("%d\n", c.at(5)); // _EN(`with bounds checking')_RU(`с проверкой границ')
    printf ("%d\n", c[8]); // operator[], _EN(`without bounds checking')_RU(`без проверки границ')
};
