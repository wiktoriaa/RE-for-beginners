include(`commons.m4')#include <map>
#include <set>
#include <string>
#include <iostream>

// _EN(`structure is not packed')_RU(`структура не запакована')!
struct tree_node
{
    ...
    char Isnil;
    //std::pair Myval;
    unsigned int first; // _EN(`called')_RU(`называется') Myval _in std::set
    const char *second; // _EN(`not present in')_RU(`отсутствует в') std::set
};
