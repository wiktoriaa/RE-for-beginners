...
\RU{Это означает что каждая короткая строка будет занимать в памяти по крайней мере}\EN{This means
that a short string will occupy at least} $16 + 4 + 4 = 24$ 
...
\EN{This may stay unnoticed for a long time, until a longer string will appears there, then the process will crash}.

\myparagraph{GCC}

\RU{В реализации GCC в структуре есть еще одна переменная --- reference count}
\EN{GCC's implementation of this structure has one more variable---reference count}.
...
with the help of the ``assign'' method, the s1 and s2 strings are constructed}.
...
можно объявлять}\EN{Experienced \Cpp programmers may not agree, but a global 
variables with an \ac{STL} type can be defined}.
