...
\lstinputlisting[caption=\RU{пример для}\EN{example for} MSVC]{\CURPATH/STL/string/MSVC.cpp}
...
\lstinputlisting[caption=\RU{пример для}\EN{example for} GCC]{\CURPATH/STL/string/GCC.cpp}
...
\lstinputlisting[caption=MSVC 2012]{\CURPATH/STL/string/3_MSVC.asm}
...
\lstinputlisting[caption=GCC 4.8.1]{\CURPATH/STL/string/3_GCC.s}
...
\lstinputlisting[caption=MSVC 2012: \RU{здесь конструируется глобальная переменная а также регистрируется её десктруктор}\EN{here is global variable constructed and also its destructor is registered}]{\CURPATH/STL/string/5_MSVC_p2.asm}

\lstinputlisting[caption=MSVC 2012: 
\RU{здесь глобальная переменная используется в}\EN{here global variable is used in} \main]{\CURPATH/STL/string/5_MSVC_p1.asm}
