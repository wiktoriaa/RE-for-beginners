...
\lstinputlisting[caption=MSVC 2012: \RU{здесь конструируется глобальная переменная, а также регистрируется её деструктор}\EN{here is global variable constructed and also its destructor is registered}]{\CURPATH/STL/string/5_MSVC_p2.asm}
