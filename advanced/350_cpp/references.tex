...
\EN{If we will try to change the example with pointers (\ref{label_pointers}) to use references instead:}
...
\EN{Then we'll see that the compiled code is just the same 
as in the pointers example (\ref{label_pointers}):}
...
\EN{The reason why \Cpp functions has such strange names is explained here}: \ref{namemangling}.)
