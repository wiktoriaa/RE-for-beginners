...
\RU{Если мы попробуем изменить пример с указателями (\myref{label_pointers}) чтобы он использовал reference вместо указателей:}
\EN{If we will try to change the example with pointers (\myref{label_pointers}) to use references instead:}
