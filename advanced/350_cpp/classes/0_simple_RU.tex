...
\lstinputlisting{\CURPATH/classes/simple_RU.cpp}

\myparagraph{MSVC\EMDASH{}x86}
...
\myparagraph{MSVC\EMDASH{}x86-64}
...
Тем не менее, указатель на объект \ITthis передается через \RCX, а первый аргумент метода\EMDASH{}в \RDX, итд.
Здесь это видно во внутренностях метода 
\TT{c(int a, int b)}:

\lstinputlisting[caption=\Optimizing MSVC 2012 x64]{\CURPATH/classes/simple_MSVC_x64_RU.asm}
...
\lstinputlisting[caption=GCC 4.4.6 x64]{\CURPATH/classes/simple_GCC_x64_RU.s}
