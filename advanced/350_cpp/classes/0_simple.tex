...
This is called thiscall~(\ref{thiscall})~---the method for passing a pointer to the object.}
...
\EN{This is all because the thiscall~(\ref{thiscall}) calling convention is used here,
which together with the stdcall~(\ref{sec:stdcall}) method offers the \gls{callee} to correct the stack
...
\EN{It also should be noted that the compiler decides when to call the constructor and 
