...
(\TT{MB\_YESNOCANCEL}), \RU{а самый последний аргмент\EMDASH{}последним}\EN{and the last argument\EMDASH{}last} (NULL).
...
\RU{Это как}\EN{It is like} stdcall (\ref{sec:stdcall}), \RU{but arguments are passed in 
``natural'' order}\EN{только аргументы передаются в ``естественном'' порядке}.
