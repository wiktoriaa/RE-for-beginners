...
\EN{32-bit values (the \TT{long} data type means 32 bits, while \Tint is 16-bit)
...
\EN{It is just like when 64-bit values are used in a 32-bit environment} (\ref{sec:64bit_in_32_env}).
...
\EN{Other compiler functions that do the same are listed here}: \ref{sec:MSVC_library_func}, \ref{sec:GCC_library_func}.
...
\TT{ADD} \RU{может установить или сбросить флаг}\EN{may set/clear the} \TT{CF}\EN{ carry flag}, \TT{ADC} \RU{будет
...
\TT{SUB} \RU{может установить или сбросить флаг}\EN{may set/clear the} \TT{CF}\EN{ flag}, \TT{SBB} \RU{будет использовать
его}\EN{will use it}.
