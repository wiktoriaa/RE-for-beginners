% TODO TikZ
\chapterold{Packing 12-bit values into array using bit operations (x64, ARM/ARM64, MIPS)}

(This part has been first appeared in my blog at 4-Sep-2015.)

\sectionold{Introduction}
...
\sectionold{The C/C++ code}

\lstinputlisting{advanced/380_FAT12/FAT12.c}
...
\sectionold{Optimizing GCC 4.8.2 for x86-64}
...
\lstinputlisting{advanced/380_FAT12/GCC_getter.asm}

\subsectionold{Setter}

\lstinputlisting{advanced/380_FAT12/GCC_setter.asm}

\subsectionold{Other comments}
...
\sectionold{Optimizing Keil 5.05 (Thumb mode)}
...
\lstinputlisting{advanced/380_FAT12/Keil_thumb_O3_getter.asm}
...
\lstinputlisting{advanced/380_FAT12/Keil_thumb_O3_setter.asm}

\sectionold{Optimizing Keil 5.05 (ARM mode)}
...
\lstinputlisting{advanced/380_FAT12/Keil_ARM_O3_getter.asm}

\subsectionold{Setter}

\lstinputlisting{advanced/380_FAT12/Keil_ARM_O3_setter.asm}
...
\sectionold{(32-bit ARM) Comparison of code density in Thumb and ARM modes}
...
\sectionold{Optimizing GCC 4.9.3 for ARM64}

\subsectionold{Getter}

\lstinputlisting{advanced/380_FAT12/ARM64_getter.lst}
...
\lstinputlisting{advanced/380_FAT12/ARM64_setter.lst}

\sectionold{Optimizing GCC 4.4.5 for MIPS}
...
\lstinputlisting{advanced/380_FAT12/MIPS_getter.asm}

\subsectionold{Setter}

\lstinputlisting{advanced/380_FAT12/MIPS_setter.asm}

\sectionold{Difference from the real FAT12}
...
Bit shifts (\TT{<<} and \TT{>>} in C/C++, \INS{SHL}/{SHR}/{SAL}/{SAR} in x86, \INS{LSL}/{LSR} in ARM, \INS{SLL}/{SRL} in MIPS) are used 
