\section{\RU{Ф-ции работы со строками и памятью}\EN{Strings and memory functions}}

\RU{Другая очень частая оптимизация это вставка кода строковых ф-ций таких как}
\EN{Another very common automatic optimization tactic is the inlining of string functions like}
\IT{strcpy()}, \IT{strcmp()}, \IT{strlen()}, \IT{memset()}, \IT{memcmp()}, \IT{memcpy()}, \RU{и т.д}\EN{etc}.

\RU{Иногда это быстрее, чем вызывать отдельную ф-цию.}\EN{Sometimes it's faster than to call a separate function.}

\RU{Это очень часто встречающиеся шаблонные вставки, которые желательно распознавать 
reverse engineer-ам ``на глаз''.}
\EN{These are very frequent patterns and it is highly advisable for reverse engineers 
to learn to detect automatically.}

% subsections
\input{\CURPATH/str_mem/strcmp}
\input{\CURPATH/str_mem/strlen}
\input{\CURPATH/str_mem/strcpy}
\input{\CURPATH/str_mem/memset}
\input{\CURPATH/str_mem/memcpy}
\input{\CURPATH/str_mem/memcmp}

\subsection{\RU{Скрипт для IDA}\EN{IDA script}}

\index{IDA}
\RU{Я написал небольшой скрипт для \IDA для поиска и сворачивания таких очень часто 
попадающихся inline-функций:}
\EN{I wrote a small \IDA script for searching and folding such very frequently seen pieces of 
inline code:} \\
\href{\YurichevIDAIDCScripts}{GitHub}.

