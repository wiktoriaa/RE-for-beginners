...
\RU{MSVC часто использует \TT{OR} вместо \TT{MOV RAX, 0xFFFFFFFFFFFFFFFF}, потому что опкод получается короче.}
\EN{MSVC often uses \TT{OR} instead of \TT{MOV RAX, 0xFFFFFFFFFFFFFFFF}, because resulting opcode is shorter.}
