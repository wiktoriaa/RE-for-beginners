...
will be faster than the usual \strlen{} work + the cost of calling it and returning from it.}
...
\RU{Первая инструкция ф-ции \strlen{} вставленная здесь, это}\EN{The first instruction of 
...
in two's complement arithmetic: \ref{sec:signednumbers}.}
...
\lstinputlisting[caption=\RU{Моя версия}\EN{My version of} \strlen{}]{\CURPATH/my_strlen.asm.\LANG}

\RU{У меня не получилось. Нам придется вводить дополнительную инструкцию JMP!}
\EN{I failed. We need to use additional JMP instruction!}
...
\RU{Я добавил комментариев}\EN{I have added my comments}.
...
(это ``short-circuit'').}
\EN{The second will not be checked, if the first one (\IT{str\_len==0}) is false (this is ``short-circuit'').}
...
\item L2:
\item \RU{Вторая часть for(): проверить первое выражение, затем второе. 
