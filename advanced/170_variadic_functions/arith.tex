...
\subsection{\EN{Using va\_arg macro}\RU{Используя макрос va\_arg}}
...
\subsubsection{\RU{Соглашение о вызовах \IT{cdecl}}\EN{\IT{cdecl} calling conventions}}
...
\subsubsection{\RU{Соглашения о вызовах на основе регистров}\EN{Register-based calling conventions}}
...
\subsection{\RU{Используя указатель на первый аргумент ф-ции}\EN{Using pointer to the first function argument}}

\RU{Пример можно переписать без использования макроса \TT{va\_arg}:}%
\EN{The example can be rewritten without \TT{va\_arg} macro:}

\lstinputlisting{\CURPATH/arith2.c}

\RU{Иными словами, если набор аргументов -- это массив слов (32-битных или 64-битных), то мы просто перебираем элементы
этого массива, начиная с первого.}%
\EN{In other words, if an argument set is array of words (32-bit or 64-bit), we just enumerate array elements starting
at first one.}
