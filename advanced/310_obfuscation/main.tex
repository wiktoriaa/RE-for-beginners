...
\EN{\item \TT{PUSH op} can also be replaced with the following instructions pair: \TT{SUB ESP, 4 (or 8) / MOV [ESP], op}.}%
\RU{\item \TT{PUSH op} также можно заменить на пару инструкций: \TT{SUB ESP, 4 (или 8) / MOV [ESP], op}.}
\end{itemize}
...
\EN{A programmer can construct his/her own \ac{PL} or \ac{ISA} and interpreter for it.}%
\RU{Программист может также создать свой собственный \ac{PL} или \ac{ISA} и интерпретатор для него.}%
...
\RU{Использование инструкции}\EN{Using the} \MOV \RU{для действительно сложных вещей}
\EN{instruction for really complicated things}: 
