...
\RU{Как я указывал в}\EN{As I explained in} (\ref{sec:digging_strings}) \RU{текстовые строки могут быть крайне
...
then every string usage will preceded by a string decrypting routine.}
...
ничего полезного}\EN{Executable code obfuscation means inserting random garbage code between real one,
...
\EN{The r}\RU{R}everse engineer\RU{-у придется потратить какое-то время для понимания деталей всех инструкций в}
\EN{will have to spend some time to understand the meaning and details of all of the} \ac{ISA}\EN{'s instructions}.
