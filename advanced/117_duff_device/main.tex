...
\EN{Since we will work with a 64-bit example here, we will clear the memory in 8 byte blocks.}
...
\RU{( ``B'' это количество 8-байтных блоков и ``S'' это длина хвоста в байтах ).}
\EN{( ``B'' is number of 8-byte blocks and ``S'' is length of the tail in bytes ).}
...
\EN{We also need to find out if we will execute the 8-byte procedure at all, so we need
...
\EN{We have to ``write as many zero bytes in memory, as $count\&7$ value tells us''.}
...
\EN{7 as input value will lead to the execution of all 7 operations.}
\RU{8 здесь нет, потому что регион памяти размером в 8 байт будет обработан первой частью нашей ф-ции.}
\EN{There is no 8, because a memory region of 8 bytes will be processed by the first part of our function.}
...
so the execution will fall until the very end, executing as many pairs as needed.}
...
\EN{This will also be faster because we will not need to fetch a value from the jumptable.}
