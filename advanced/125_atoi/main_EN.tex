\chapterold{String to number conversion (atoi())}
...
All we need to know is that \q{0} minus \q{0} is 0, \q{9} minus \q{0}'is 9 and so on.
...
\subsectionold{\Optimizing MSVC 2013 x64}

\lstinputlisting[caption=\Optimizing MSVC 2013 x64]{\CURPATH/atoi.asm.MSVC2013.x64.Ox_EN}
...
\subsectionold{\Optimizing GCC 4.9.1 x64}

\Optimizing GCC 4.9.1 is more concise, but there is one redundant RET instruction at the end.
One would be enough.

\lstinputlisting[caption=\Optimizing GCC 4.9.1 x64]{\CURPATH/atoi.s.GCC491.O3.x64_EN}

\subsectionold{\OptimizingKeilVI (\ARMMode)}

\lstinputlisting[caption=\OptimizingKeilVI (\ARMMode)]{\CURPATH/atoi.s.ARM.O3_EN}

\subsectionold{\OptimizingKeilVI (\ThumbMode)}

\lstinputlisting[caption=\OptimizingKeilVI (\ThumbMode)]{\CURPATH/atoi.s.thumb.O3_EN}
...
\subsectionold{\Optimizing GCC 4.9.1 ARM64}

The ARM64 compiler can use the pre-increment instruction suffix:

\lstinputlisting[caption=\Optimizing GCC 4.9.1 ARM64]{\CURPATH/atoi.s.GCC49.ARM64.O3_EN}

\sectionold{A slightly advanced example}
...
\lstinputlisting[caption=\Optimizing GCC 4.9.1 x64]{\CURPATH/atoi2.s.GCC491.O3.x64_EN}

\myindex{x86!\Instructions!NEG}

If the \q{minus} sign was encountered at the string start, the NEG instruction is to be executed at the end.
...
\lstinputlisting[caption=\OptimizingKeilVI (\ARMMode),numbers=left]{\CURPATH/atoi2.s.ARM.O3_EN}

There is no NEG instruction in 32-bit ARM, so the \q{Reverse Subtraction} operation (line 31) 
is used here.

It is triggered if the result of the CMP instruction (at line 29) was \q{Not Equal} (hence -NE suffix).
