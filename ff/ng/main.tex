...
\EN{Norton Guide\footnote{\url{http://en.wikipedia.org/wiki/Norton_Guides}} was popular in MS-DOS epoch, it was resident program working as hypertext reference.}
\RU{Norton Guide\footnote{\url{http://en.wikipedia.org/wiki/Norton_Guides}} был популярен во времена MS-DOS, это была резидентная программа, работающая как
...
\EN{We see that 0x1A byte is ocurring so often, it would not be possible in compressed file.}
\RU{Мы видим как слишком часто попадается байт 0x1A, в сжатом файле такого не было бы никогда.}
...
\RU{Больше о формате файлов Norton Guide:} \url{http://www.davep.org/norton-guides/file-format/}.
