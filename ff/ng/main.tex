...
\subsection{\EN{One more word about base of entropy}\RU{Еще кое-что о базе энтропии}}
...
\EN{So I set base of 2 explicitely in \TT{Entropy} command, so Mathematica will give the 
same results as \IT{ent} utility.}
\RU{Так что я явно указал базу 2 в команде \TT{Entropy}, чтобы Mathematica давала те же результаты,
что и утилита \IT{ent}.}
