...
\EN{We see that 0x1A byte (looking like ``$\rightarrow$'') is occurring so often, it would not be possible in compressed file.}
...
\clearpage
\EN{Since 0x1A byte occurring so often, we can try to decrypt the file, assuming that it's encrypted
...
\EN{Now we understand why 0x1A byte was occurring so often: because there are so much zero bytes and they
