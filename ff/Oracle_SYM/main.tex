...
\EN{I chose the shortest \TT{orawtc8.sym} file that comes with the \TT{orawtc8.dll} file in Oracle 8.1.7}
\footnote{\RU{Я выбрал древнюю версию \oracle сознательно, из-за более короткого размера его модулей}\EN{I chose 
an ancient \oracle version intentionally due to the smaller size of its modules}}.
...
\clearpage
\RU{Я отметил сигнатуры OSYM и строки здесь}\EN{I marked the OSYM signatures and strings here}: 
...
файл.}
\EN{In Hiew, I marked the whole strings block (except the trailing OSYM signatures) and put it into a separate file.}
\RU{Затем я запустил UNIX-утилиты \IT{strings} и \IT{wc} для подсчета строк здесь:}
\EN{Then I ran UNIX \IT{strings} and \IT{wc} utilities to count the strings in there:}
...
\RU{Почему я думаю, что оно 32-битное}\EN{Why do I think it's 32-bit}?
...
\EN{I checked other .SYM files like this and it proves my guess: the value after the 32-bit OSYM signature always
...
\EN{Using Hiew again, I put the block starting at address 8 (i.e., after the 32-bit \IT{count} value) 
...
\RU{Когда я открываю файл orawtc8.dll в IDA, базовый адрес другой, но тем не менее, первая функция это:}
\EN{When I open the orawtc8.dll file in IDA, the base address is different, but nevertheless, the first function is:}
...
\RU{Так что имя этой функции, похоже}\EN{So the name of this function seems to be} ``ax\_reg''.
...
\EN{The ``ax\_unreg'' string is also the second string in the strings block!}
...
это просто адреса функций в DLL, включая некоторые метки,}
\EN{I can quickly check and be sure that the first 66 values in the array (i.e., the first half of the array) 
...
\index{IDA}
\RU{Я написал утилиту для конвертирования .SYM-файлов в \IDA-скрипт, 
так что я могу загружать .idc-скрипт и он выставит имена функций:}
\EN{I wrote an utility to convert these .SYM files into \IDA script, 
so I can load the .idc script and it sets the function names:}
...
\RU{Файлы, которые я использовал в этом примере, здесь}\EN{The example files I used are here}: 
...
я полагаю}\EN{to distinguish the target platform, I suppose}.\\
\\
\RU{Вот и всё}\EN{This is it}!
\RU{Вот также моя библиотека в которой есть функция для доступа к .SYM-файлам \oracle}\EN{Here is also my 
library in which I have functions to access \oracle .SYM-files}:
