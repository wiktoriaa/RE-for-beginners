...
\EN{To get the number of cores, one should call this function with the \TT{SystemBasicInformation} constant
...
\EN{The second argument has to point to the buffer in which we will receive all the information.}
...
\RU{О да, это значит, что }\EN{Oh yes, this means that }\TT{var\_C20} \RU{в локальном стеке это именно поле}\EN{ns the 
local stack is exactly the} \TT{NumberOfProcessors} \RU{структуры}\EN{field of the} 
...
try to restore the patched} \TT{taskmgr.exe}).
