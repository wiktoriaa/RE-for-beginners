...
\IFRU{Когда я в очередной раз вернулся к своему SAP 6.0 IDES заинсталлированному в виртуальной машине VMware, я обнаружил что забыл пароль, впрочем, затем я вспомнил его, но теперь я получаю такую ошибку:}{While returning again to my SAP 6.0 IDES installed in VMware box, I figured out I forgot the password for SAP* account, then it back to my memory, but now I got error message} \IT{<<Password logon no longer possible - too many failed attempts>>}, \IFRU{потому что я портатил все попытки на то, чтобы вспомнить его}{because I've spent all these attempts in trying to recall it}.

\IFRU{Первая очень хорошая новость состоит в том что с SAP поставляется полный файл \IT{disp+work.pdb}, он содержит все: имена функций, структуры, типы, локальные переменные, имена аргументов, итд. Какой щедрый подарок!}{First extremely good news is that full \IT{disp+work.pdb} file is supplied with SAP, it contain almost everything: function names, structures, types, local variable and argument names, etc. What a lavish gift!}
...
\IFRU{Немного поэкспериментировав с этой функцией, я быстро понял что проблема именно в ней}{After playing for a little with this function, I quickly noticed that the problem is exactly in it}. \IFRU{Она вызывается из функции \IT{chckpass()} ~--- одна из функций проверяющих пароль}{It is called from \IT{chckpass()} function ~--- one of the password checking functions}.
...
\IFRU{Число}{Number} 0x35 \IFRU{возвращается из}{is error returning in} \IT{chckpass()} \IFRU{в этом месте}{at that point}:
...
\IFRU{Кстати, я могу сделать вид что вообще забыл пароль, заставляя \IT{chckpass()} всегда возвращать ноль, и этого достаточно для отключения проверки пароля:}{By the way, if I try to pretend I forgot the password, fixing \IT{chckpass()} function return value at zero is enough to bypass check:}
...
можно увидеть следующий вызов:}{What also can be said while analyzing \IT{password\_attempt\_limit\_exceeded()} function is that at the very beginning of it, this call might be seen:}
...
\IFRU{Очевидно, функция \IT{sapgparam()} используется чтобы узнать значение какой-либо переменной конфигурации. Эта функция может вызываться из 1768 разных мест.}{Obviously, function \IT{sapgparam()} used to query value of some configuration parameter. This function can be called from 1768 different places.}
...
\IFRU{Замечательно! Имена функций очень понятны, куда понятнее чем в \oracle.}{It is really sweet. Function names are very clear, much clearer than in \oracle.} \IFRU{По всей видимости, процесс \IT{disp+work} весь написан на Си++. Вероятно, он был переписан не так давно?}{It seems, \IT{disp+work} process written in C++. It was probably rewritten some time ago?}
