\subsection{\IFRU{Пример}{Example} \#2: SCO UNIX}

\index{SCO UNIX}
\IFRU{Древняя программа для}{An ancient software for} SCO UNIX \IFRU{от}{from} 1997 
...
{After driver installation in SCO UNIX, these device files are appeared in /dev filesystem}:
...
\index{COFF}
\IFRU{Спасибо \ac{IDA}, она легко загружает исполняемые файлы формата COFF использующиеся в}
{Thanks to \ac{IDA}, it does its job perfectly working out COFF executable used in} SCO UNIX.
...
\IFRU{называется}{function called is the} \gls{thunk function}:
...
\index{CRC32}
\IFRU{Но что такое хеш-функция}{But what is hashing function}?
...
проверить его хеш с тем что лежит в БД, и дать вам доступ если проверка проходит}
...
можно отличать друг от друга: ведь более одной может быть подключено к LPT-порту}
{selection (that's how Sentinel Pro dongles are differentiated from each other: more than one
can be connected to LPT port)}.
...
\IFRU{Еще одна возможность это написать свой драйвер для SCO UNIX}
{Another option is to write our own SCO UNIX driver}.
