...
such bits as ``paper out'', ``ack'', ``busy''~---thus the printer may signal to the host computer
...
\RU{Вот ф-ция проверки донглы}\EN{Here is the dongle checking routine}:
...
\EN{The}\RU{Ф-ция} \TT{get\_rand()} \RU{использует стандартную ф-цию Си}
...
\EN{This means that the string passed to the function cannot be located on the boundary between two data segments.}
