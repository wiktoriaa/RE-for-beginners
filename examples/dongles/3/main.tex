...
\EN{This is again a Sentinel Pro ``hashing'' dongle as in the previous example}.
...
\EN{I figured out its type by noticing that text strings are passed here, too, and 16 bit values
are returned and compared with others}.
...
он содержит такие биты как ``paper out'', ``ack'', ``busy''\EMDASH{}так принтер может сигнализировать
...
such bits as ``paper out'', ``ack'', ``busy''\EMDASH{}thus the printer may signal to the host computer
...
\EN{It seems that the dongle returns information via the ``busy'' flag at} \TT{seg030:00B9}: 
...
\EN{The}\RU{Ф-ция} \TT{get\_rand()} \RU{использует стандартную функцию Си}
...
\EN{The text strings seem to be chosen randomly as well}.
...
\EN{At \TT{seg030:00F2} only a 16-bit word is \glslink{increment}{incremented}, not segment value.}
\RU{Это значит, что переданная в функцию строка не может находится на границе двух сегментов.}
