\subsection{\RU{Пример}\EN{Example} \#3: MS-DOS}
...
\RU{Трудно сказать. Возможно, команды донглы.}\EN{Hard to say. Perhaps, commands to the dongle.}
...
\RU{В функции \TT{sent\_pro()} каждый байт строки загружается на\\
\TT{seg030:00EF}: инструкция
\TT{LES} загружает из переданного аргумента пару ES:BX одновременно}
\EN{In the \TT{sent\_pro()} function, each byte of the string is loaded at\\
