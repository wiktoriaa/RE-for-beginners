...
\EN{We know that a ``good'' r3 value should be non-zero, zero r3 will lead the execution
...
\TT{B} (\RU{означает}\EN{means} \IT{branch}) \RU{~--- безусловный переход}\EN{unconditional jump}.
...
\EN{OK, all we've got is that \TT{check1()} should always return 1 or any other non-zero value}.
