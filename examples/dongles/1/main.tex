...
\subsection{\RU{Пример}\EN{Example} \#1: MacOS Classic \AndENRU PowerPC}
...
\RU{Из}\EN{From the} \PPCABI \RU{мы узнаем, что регистр r3 используется для возврата
...
\RU{Из}\EN{From} \PPC \RU{мы узнаем, что эта инструкция одновременно и очищает и загружает}%
...
16 bits, i.e.}, \\
\TT{.RBEFINDFIRST()} \RU{вероятно возвращает 16-битное значение}
...
\RU{Эта функция, должно быть, читает какие-то значения из донглы, которые потом сравниваются здесь при помощи}
\EN{Perhaps, the function returns some values from the dongle,
...
\EN{We can spot its opcode in the code without even referring to} \PPC.
