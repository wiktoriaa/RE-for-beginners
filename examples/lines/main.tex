...
\EN{So let's see, will it be possible to find the random generator and do some trick with it.}
\IDA \RU{быстро распознает стандартную ф-цию}\EN{quickly recognize the standard} \TT{\_rand} \RU{в}\EN{function in} 
...
\RU{Эта ф-ция вызывается из трех мест}\EN{This function is referred from 3 places}.
...
\RU{Так что у ф-ции только один аргумент}\EN{So the function has only one argument}.
...
a number in the} \TT{0..0x7FFF} \RU{и это неудобно, так что многие программисты пишут свою ф-цию,
...
единственный аргумент в ф-цию}\EN{is passed as the sole argument of the function}.
\RU{Мы можем быстро проверить это в отладчике}\EN{We can quickly check this in any debugger}.

\RU{Сделаем так, чтобы третий вызов ф-ции всегда возвращал ноль}\EN{So let's fix the third function call to always return zero}.
...
\RU{Что я сделал, это заменил вызов ф-ции}\EN{So what I did is I replaced a call to the} \TT{random()} 
...
\RU{Но почему аргументы ф-ции}\EN{But why are the arguments to the} \TT{random()} \RU{это глобальные 
