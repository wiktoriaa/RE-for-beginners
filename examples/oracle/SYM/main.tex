...
\RU{В этом примере мы попробуем разобрать очень простую структуру бинарного файла, даже без того чтобы
заглядывать в код \oracle.}
\EN{In this example we will try to understand the very simple structure of binary file, even without looking
into \oracle code.}
