\section{\RU{Таблица \TT{X\$KSMLRU} в}\EN{\TT{X\$KSMLRU} table in} \oracle}
\index{\oracle}
...
к ней.}\EN{However, as it can be easily checked, this table's contents is cleared each time table querying.}
...
вызывается ф-ция \TT{ksmlrs()}:}\EN{Let's back to tables we already know: \TT{kqftab} and \TT{kqftap} 
...
\index{tracer}
\RU{Действительно, при помощи \tracer легко убедиться что эта ф-ция вызывается каждый раз, когда мы обращаемся 
к таблице \TT{X\$KSMLRU}.}\EN{Indeed, with the \tracer help it is easy to see this function is called each 
time we query the \TT{X\$KSMLRU} table.}

\RU{Здесь есть ссылки на ф-ции \TT{ksmsplu\_sp()} и \TT{ksmsplu\_jp()}, каждая из которых в итоге вызывает 
...
\RU{В конце ф-ции \TT{ksmsplu()} мы видим вызов \TT{memset()}:}\EN{At the end of the \TT{ksmsplu()} function we see 
...
\RU{Мы можем попробовать рискнуть, заблокировав вызов \TT{memset()} и посмотреть, что будет?}\EN{What if we would take a risk, block \TT{memset()} call and see what will happen?}

\index{tracer}
\RU{Запускаем \tracer со следующей опцией: поставить точку останова на \TT{0x434C7A} (там где начинается передача 
...
\EN{Let's run \tracer with the following options: set breakpoint at \TT{0x434C7A} (the point where \TT{memset()} arguments are to be passed), thus, that \tracer set program counter \TT{EIP} at this point to the point where passed to the \TT{memset()} arguments are to be cleared (at \TT{0x434C8A})}
\RU{Можно сказать, при помощи этого, мы симулируем безусловный переход с адреса}\EN{It can be said, we just simulate an unconditional jump from the address} \TT{0x434C7A} \RU{на}\EN{to} \TT{0x434C8A}.

\begin{lstlisting}
tracer -a:oracle.exe bpx=oracle.exe!0x00434C7A,set(eip,0x00434C8A)
\end{lstlisting}

\RU{(Важно: все эти адреса справедливы только для win32-версии \oracle 11.2)}\EN{(Important: all these addresses are valid only for win32-version of \oracle 11.2)}

\RU{Действительно, после этого мы можем обращаться к таблице \TT{X\$KSMLRU} сколько угодно, и она уже не очищается!}\EN{Indeed, now we can query \TT{X\$KSMLRU} table as many times as we want and it is not clearing anymore!}

\RU{\sout{Не делайте этого дома ("Разрушители легенд")} Не делайте этого на своих production-серверах.}\EN{\sout{Do not try this at home ("MythBusters")} Do not try this on your production servers.}
...
\EN{It is probably not a very useful or desired system behaviour, but as an experiment of locating piece of code we need, that is perfectly suit our needs!}
