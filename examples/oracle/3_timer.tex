...
\EN{is another \IT{fixed view}, reflecting a rapidly changing value:}
...
\EN{It is interesting the periods are different for Oracle for win32 and for Linux. 
...
\EN{Now we stuck in a small problem, there are no references to value generating function(s) 
...
\EN{Let's try to find a string \TT{KSUTMTIM}, and we find it in this function:}
...
\RU{Сама ф-ция}\EN{The function} \TT{kqfd\_DRN\_ksutm\_c()} \RU{упоминается в таблице}\EN{is mentioned in} 
...
\RU{Упоминается также некая ф-ция \TT{ksugtm()}}\EN{There are is a function \TT{ksugtm()} referenced here}.
...
\RU{В win32-версии тоже самое}\EN{Almost the same code in win32-version}.
...
\EN{Indeed~---the value is the same we see in SQL*Plus and it is returning via second argument}.
...
\EN{and division of its result by} 10~(\ref{sec:divisionbynine}))
...
\RU{Это просто результат}\EN{It is just result of} \TT{GetTickCount()
\footnote{\url{http://msdn.microsoft.com/en-us/library/windows/desktop/ms724408(v=vs.85).aspx}}} 
\RU{поделенный на}\EN{divided by} 10~(\ref{sec:divisionbynine}).

\RU{Вуаля! Вот почему в win32-версии и версии Linux x86 разные результаты, потому что они получаются разными 
системными ф-циями \ac{OS}.}\EN{Voilà! That's why win32-version and Linux x86 version show different results, 
just because they are generated by different \ac{OS} functions.}

\RU{\IT{Drain} по-английски дренаж, отток, водосток. Таким образом, возможно имеется ввиду \IT{подключение} 
определенного столбца системной таблице к функции.}\EN{\IT{Drain} apparently means \IT{connecting} specific 
table column to specific function.}

\RU{Я добавил поддержку таблицы}\EN{I added the table} \TT{kqfd\_tab\_registry\_0} \RU{в}\EN{to} \oracletables, 
\RU{теперь мы можем видеть, при помощи каких ф-ций, столбцы в системных таблицах \IT{подключаются} к значениям, 
например}\EN{now we can see, how table column's variables are \IT{connected} to specific functions}:
...
\IT{OPN}, \RU{возможно}\EN{apparently}, \IT{open}, \RU{а}\EN{and} \IT{DRN}, \RU{вероятно, означает}
\EN{apparently, meaning} \IT{drain}.
