...
\EN{Second: a string like ``data compression switched off'' must be present somewhere in it.}
\RU{При помощи файлового менеджера FAR\footnote{\url{http://www.farmanager.com/}} я нашел обе эти строки в файле SAPguilib.dll.}
\EN{With the help of the FAR file manager\footnote{\url{http://www.farmanager.com/}} I found that both of these strings are stored in the SAPguilib.dll file.}
...
\EN{So let's open SAPguilib.dll in \IDA and search for the \IT{``\TDWNC''} string. 
...
\EN{Also take a look at the \TT{chk\_env()} function (I gave this name to it)}:
...
\RU{А теперь, можем ли мы найти строку \IT{``data record mode switched on''}?}\EN{Now, can we find the \IT{``data record mode switched on''} string?}
...
\EN{How do I know the class/method names? There are a lot of special debugging calls that write to the log files, like}:
...
Depending on the option turned on in the object, a specific message part will be concatenated to the resulting message.}
...
(I gave it this name) function and the \TT{this+0x3D} variable in \IT{CDwsGui::PrepareInfoWindow}. 
...
\EN{I'm starting to search for all occurrences of the \TT{0x15} value in code. 
