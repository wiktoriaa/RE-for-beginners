...
\EN{I was asked to reverse engineer an amateur cryptoalgorithm of some data encryption utility, 
the source code for which was lost\footnote{I also got permission from the customer to publish the algorithm's details}.}
...
\EN{I started from the top. Here is a function that takes two file names and password.}
...
\EN{The newly created file handle is in the \EDI register now. Write signature ``QR9''.}
...
\EN{This fragment of code copies a part of the input buffer to an internal array I later named ``cube64''.}
...
\EN{Hmm, if the password contains non-alphabetical Latin character, it is skipped! 
Indeed, when we run the encryption utility and try non-alphabetical Latin characters in the password, 
...
\RU{Отнимем значение ``a'' (97) от символа.}\EN{Subtract the value of ``a'' (97) from the character.}
...
This means that the 26 Latin alphabet symbols become values in the range of 0..23, (24 in total).}
...
You can read more about it in the ``\DivisionByNineSectionName'' section~(\myref{sec:divisionbynine}).}
...
\RU{\EDX ~--- остаток от деления.}\EN{\EDX is the remainder of the division.}
...
As I already said, 1 is for the encryption operations and 3 is for the decryption. 
So, here is a loop. When encrypting, rotate1/2/3 will be called the same number of times as 
...
I eventually named them \TT{set\_bit()} and \TT{get\_bit()}.}
...
For example, if arg\_8 is 4, the value in the \TT{DL} register will be 
...
\RU{Вытащить бит из массива и явно выставить его.}\EN{Get a bit from array and explicitly set it.} % TODO: rewrite
...
there will be 0xEF after the \NOT instruction (or 11101111b in binary form).}
...
This means that if \TT{DL} is 11101111b in binary form,
all bits will be saved except the 5th (counting from lowest bit).}
...
could be seen as 2D coordinates. Then the array will be an 8*8 matrix.}
...
\RU{Рассмотрим функцию \TT{decrypt()}, я переписал её:}\EN{Let's look closer into the \TT{decrypt()} function, 
I rewrote it here:}
...
\EN{This means that in case of decryption, each corresponding rotate1/2/3 call will be performed thrice.}
...
If you need to undo the effect of rotating one place in clockwise direction, rotate it thrice
in counter-clockwise direction.}
% This doesn't sound right - if you want to undo a rotation, you either do one
% in the reverse direction, or three in the same direction, not three in the reverse
% direction
...
\EN{\TT{rotate1()} is apparently for rotating the ``front'' plane. 
\TT{rotate2()} is apparently for rotating the ``top'' plane. 
\TT{rotate3()} is apparently for rotating the ``left'' plane.}
