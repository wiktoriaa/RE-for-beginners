\chapter{\RU{``QR9'': Любительская криптосистема, вдохновленная кубиком Рубика}
\EN{``QR9'': Rubik's cube inspired amateur crypto-algorithm}}
...
\RU{Однажды меня попросили разобраться с одним таким любительским криптоалгоритмом встроенным в 
...
\EN{I was asked to reverse engineer an amateur cryptoalgorithm of some data crypting utility, 
...
\EN{Here is also \IDA exported listing from original crypting utility}:

\lstinputlisting{examples/qr9/qr9_original.lst}

\RU{Все имена функций и меток даны мною в процессе анализа.}
\EN{All function and label names are given by me while analysis.}

\RU{Я начал с самого верха. Вот функция, берущая на вход два имени файла и пароль.}
\EN{I started from top. Here is a function taking two file names and password.}
...
\RU{Открыть файл и сообщить об ошибке в случае ошибки:}\EN{Open file and report error in case of error:}
...
\RU{Узнать размер файла используя}\EN{Get file size via} \TT{fseek()}/\TT{ftell()}:

\lstinputlisting{\RU{examples/qr9/1ru.asm}\EN{examples/qr9/1en.asm}}
...
\EN{This fragment of code calculates file size aligned on a 64-byte boundary. 
This is because this cryptoalgorithm works with only 64-byte blocks. 
Its operation is pretty straightforward: divide file size by 64, forget about remainder and add 1, 
then multiple by 64. 
The following code removes remainder as if value was already divided by 64 and adds 64. 
It is almost the same.}

\lstinputlisting{\RU{examples/qr9/2ru.asm}\EN{examples/qr9/2en.asm}}
...
заменить на calloc()}.}\EN{Call memset(), e.g., clears allocated buffer\footnote{malloc() + memset() could 
be replaced by calloc()}.}

\lstinputlisting{\RU{examples/qr9/3ru.asm}\EN{examples/qr9/3en.asm}}

\RU{Чтение файла используя стандартную функцию Си}\EN{Read file via standard C function} \TT{fread()}.
...
\EN{Call \TT{crypt()}. This function takes buffer, buffer size (aligned) and password string.}
...
\EN{Create output file. By the way, developer forgot to check if it is was created correctly! 
...
\RU{Теперь хэндл созданного файла в регистре \EDI. Записываем сигнатуру ``QR9''.}
\EN{Newly created file handle is in the \EDI register now. Write signature ``QR9''.}
...
\RU{Записываем настоящую длину файла (не выровненную)}\EN{Write actual file size (not aligned)}:
...
\RU{Записываем шифрованный буфер}\EN{Write encrypted buffer}:
...
\RU{Закрыть файл и освободить выделенный буфер}\EN{Close file and free allocated buffer}:
...
\RU{Переписанный на Си код}\EN{Here is reconstructed C-code}:
...
\RU{Процедура дешифрования почти такая же}\EN{Decrypting procedure is almost the same}:
...
\RU{Сообщить об ошибке если сигнатура отсутствует}\EN{Report an error if signature is absent}:
...
\RU{Этот фрагмент кода копирует часть входного буфера во внутренний буфер, который я позже назвал ``cube64''.}
\EN{This fragment of code copies part of input buffer to internal array I named later ``cube64''.}
\RU{Длина в регистре \ECX. \TT{MOVSD} означает \IT{скопировать 32-битное слово}, так что, 16 32-битных слов
это как раз 64 байта.}\EN{The size is in the \ECX register. \TT{MOVSD} means \IT{move 32-bit dword}, so, 
...
\RU{Скопировать зашифрованное содержимое из ``cube64'' назад в буфер}
\EN{Copy encrypted contents back from ``cube64'' to buffer}:
...
\EN{If \EBP is not bigger that input argument size, then continue to next block.}
...
строку пароля и число.}\EN{OK, now let's go deeper into function \TT{rotate\_all\_with\_password()}. 
...
\EN{In \TT{crypt()}, number 1 is used, and in the \TT{decrypt()} function (where \TT{rotate\_all\_with\_password()} function 
...
\RU{Проверяем символы в пароле. Если это ноль, выходим:}\EN{Check for character in password. If it is zero, exit:}
...
\RU{Вызываем \TT{tolower()}, стандартную функцию Си.}\EN{Call \TT{tolower()}, standard C function.}
...
\EN{Hmm, if password contains non-alphabetical Latin character, it is skipped! 
...
\RU{Отнимем значение ``a'' (97) от символа.}\EN{Subtract ``a'' value (97) from character.}

\begin{lstlisting}
.text:005411DF                 sub     ecx, 'a'  ; 97
\end{lstlisting}

\RU{После вычитания, тут будет 0 для ``a'', 1 для ``b'', и так далее. И 25 для ``z''.}
\EN{After subtracting, we'll get 0 for ``a'' here, 1 for ``b'', etc. And 25 for ``z''.}
...
\RU{Похоже, символы ``y'' и ``z'' также исключительные.
После этого фрагмента кода, ``y'' становится 0, а ``z'' ~--- 1.
Это значит, что 26 латинских букв становятся значениями в интервале 0..23, (всего 24).}
\EN{It seems, ``y'' and ``z'' are exceptional characters too. 
...
Читайте об этом больше в секции ``\DivisionByNineSectionName''~(\myref{sec:divisionbynine}).}
\EN{This is actually division via multiplication. 
Read more about it in the ``\DivisionByNineSectionName'' section~(\myref{sec:divisionbynine}).}

\RU{Это код, на самом деле, делит значение символа пароля на 3.}
\EN{The code actually divides password character value by 3.}
% FIXME: add Mathematica calculations
\begin{lstlisting}
...
\RU{\EDX ~--- остаток от деления.}\EN{\EDX is the remainder of division.}

\lstinputlisting{\RU{examples/qr9/4ru.asm}\EN{examples/qr9/4en.asm}}
...
в первом аргументе.}
\EN{If remainder is 2, call \TT{rotate3()}. 
The \EDI is a second argument of the \TT{rotate\_all\_with\_password()} function.
...
\RU{Достать следующий символ из строки пароля.}\EN{Fetch next character from password string.}
...
\RU{\glslink{increment}{Инкремент} указателя на символ в строке пароля:}\EN{\Gls{increment} character pointer within password string:}
...
\RU{Реконструированный код на Си:}\EN{Here is reconstructed C code:}
...
\EN{Now let's go deeper and investigate rotate1/2/3 functions. 
...
Изолировать самый младший бит и вернуть его}\EN{Then shift a byte from an array by arg\_8 bits right. 
...
\EN{Value in the \TT{DL} is 1 here. Shift left it by arg\_8.
For example, if arg\_8 is 4, value in the \TT{DL} register became 
...
\RU{Вытащить бит из массива и явно выставить его.}\EN{Get bit from array and explicitly set one.} % TODO: rewrite

\begin{lstlisting}
.text:00541020                 or      cl, dl
\end{lstlisting}

\RU{Сохранить его назад:}\EN{Store it back:} % TODO: rewrite
...
\EN{\dots invert DL. For example, if DL state after shift was 0x10 or 1000b in binary form, 
there will be 0xEF after \NOT instruction or 11101111b in binary form.}
...
\EN{This instruction clears bit, in other words, it saves all bits in \TT{CL} which are also set in 
\TT{DL} except those in \TT{DL} which are cleared.
This means that if \TT{DL} is e.g. 11101111b in binary form,
...
\EN{It is almost the same as \TT{get\_bit()}, except, if arg\_C is zero, the function clears specific bit in array, 
...
\EN{We also know the array size is 64. First two arguments both in the \TT{set\_bit()} and \TT{get\_bit()} functions
could be seen as 2D coordinates. Then array will be 8*8 matrix.}

\RU{Представление на Си всего того, что мы уже знаем:}\EN{Here is C representation of what we already know:}
...
\RU{Теперь вернемся к функциям rotate1/2/3.}\EN{Now let's get back to rotate1/2/3 functions.}
...
\EN{Internal array allocation in local stack, its size 64 bytes:}
...
\EBX \RU{указывает на внутренний массив}\EN{is a pointer to internal array:}
...
\RU{Здесь два вложенных цикла:}\EN{Two nested loops are here:}

\lstinputlisting{\RU{examples/qr9/5ru.asm}\EN{examples/qr9/5en.asm}}
...
\EN{\dots we see that both loop counters are in range 0..7. 
Also they are used as the first and the second arguments of the \TT{get\_bit()} function.
Third argument of the \TT{get\_bit()} is the only argument of \TT{rotate1()}. 
What \TT{get\_bit()} returns, is being placed into internal array.}

\RU{Снова приготовить указатель на внутренний массив:}\EN{Prepare pointer to internal array again:}

\lstinputlisting{\RU{examples/qr9/6ru.asm}\EN{examples/qr9/6en.asm}}
...
\EN{\dots this code is placing contents from internal array to cube global array via \TT{set\_bit()} function, 
\IT{but}, in different order!
Now loop 1 counter is in range 7 to 0, \glslink{decrement}{decrementing} at each iteration!}

\RU{Представление кода на Си выглядит так:}\EN{C code representation looks like:}
...
\EN{Not very understandable, but if we will take a look at \TT{rotate2()} function:}

\lstinputlisting{\RU{examples/qr9/7ru.asm}\EN{examples/qr9/7en.asm}}
...
\EN{It is \IT{almost} the same, except of different order of arguments of the \TT{get\_bit()} and \TT{set\_bit()}. 
...
\RU{Перепишем так же функцию \TT{rotate3()}:}\EN{Let's also rewrite \TT{rotate3()} function:}
...
\EN{Well, now things are simpler. If we consider cube64 as 3D cube 8*8*8, where each element is bit, 
...
\EN{rotate1/2/3 functions are in fact rotating all bits in specific plane. 
Three functions are each for each cube side and \TT{v} argument is setting plane in range 0..7.}


\RU{Может быть, автор алгоритма думал о кубике Рубика 8*8*8}
\EN{Maybe, algorithm's author was thinking of 8*8*8 Rubik's cube}
\footnote{\href{http://go.yurichev.com/17115}{wikipedia}}?!

\RU{Да, действительно.}\EN{Yes, indeed.}

\RU{Рассмотрим функцию \TT{decrypt()}, я переписал её:}\EN{Let's get closer into \TT{decrypt()} function, 
I rewrote it here:}
...
\EN{It is almost the same except of \TT{crypt()}, \IT{but} password string is reversed by}
...
\EN{This means, in case of decryption, each corresponding rotate1/2/3 call will be performed thrice.}
...
\RU{\TT{rotate1()}, вероятно, поворот ``лицевой'' плоскости. 
\TT{rotate2()}, вероятно, поворот ``верхней'' плоскости.
\TT{rotate3()}, вероятно, поворот ``левой'' плоскости.}
\EN{\TT{rotate1()} is apparently for rotating ``front'' plane. 
\TT{rotate2()} is apparently for rotating ``top'' plane. 
\TT{rotate3()} is apparently for rotating ``left'' plane.}

\RU{Вернемся к ядру функции \TT{rotate\_all()}}\EN{Let's get back to the core of \TT{rotate\_all()} function:}
...
из 24-х элементов.}
\EN{Now it is much simpler to understand: each password character defines side (one of three) and plane (one of 8). 
...
\EN{The algorithm is clearly weak: in case of short passwords, one can see,
that in encrypted file there are 
an original bytes of the original file in binary files editor.}

\RU{Весь исходный код в реконструированном виде:}\EN{Here is reconstructed whole source code:}
