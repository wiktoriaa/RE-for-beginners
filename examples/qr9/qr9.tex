\chapter{\IFRU{``QR9'': Любительская криптосистема, вдохновленная кубиком Рубика}
{``QR9'': Rubik's cube inspired amateur crypto-algorithm}}

\IFRU{Любительские криптосистемы иногда встречаются довольно странные.}
...
\lstinputlisting{\IFRU{examples/qr9/1ru.asm}{examples/qr9/1en.asm}}

\IFRU{Этот фрагмент кода вычисляет длину файла, выровненную по 64-байтной границе.
...
{This fragment of code calculates file size aligned on a 64-byte boundary. 
...
\lstinputlisting{\IFRU{examples/qr9/2ru.asm}{examples/qr9/2en.asm}}
...
заменить на calloc()}.}{Call memset(), e.g., clears allocated buffer\footnote{malloc() + memset() could 
be replaced by calloc()}.}

\lstinputlisting{\IFRU{examples/qr9/3ru.asm}{examples/qr9/3en.asm}}
...
\IFRU{Теперь хэндл созданного файла в регистре \EDI. Записываем сигнатуру ``QR9''.}
...
\IFRU{Этот фрагмент кода копирует часть входного буфера во внутренний буфер, который я позже назвал ``cube64''.}
...
это как раз 64 байта.}{The size is in the \ECX register. \TT{MOVSD} means \IT{move 32-bit dword}, so, 
...
{In \TT{crypt()}, number 1 is used, and in the \TT{decrypt()} function (where \TT{rotate\_all\_with\_password()} function 
...
\lstinputlisting{\IFRU{examples/qr9/4ru.asm}{examples/qr9/4en.asm}}
...
\IFRU{\glslink{increment}{Инкремент} указателя на символ в строке пароля:}{\Gls{increment} character pointer within password string:}
...
\IFRU{\dots иными словами: подсчитать индекс в массиве cube64}{\dots in other words: calculate an index in 
...
\IFRU{\TT{DL} тут равно 1. Сдвигаем эту единицу на количество указанное в arg\_8. Например, если в arg\_8 число 4,
тогда значение в \TT{DL} станет 0x10 или 1000 в двоичной системе счисления.}
{Value in the \TT{DL} is 1 here. Shift left it by arg\_8.
...
\IFRU{Если arg\_C не ноль\dots}{If arg\_C is not zero\dots}
...
\IFRU{\dots инвертировать DL. Например, если состояние DL после сдвига 0x10 или 1000 в двоичной системе,
...
\IFRU{Эта инструкция сбрасывает бит, иными словами, она сохраняет все биты в \TT{CL} которые также
...
{This instruction clears bit, in other words, it saves all bits in \TT{CL} which are also set in 
...
\IFRU{Мы также знаем что размер массива 64. Первые два аргумента и у \TT{set\_bit()} и у \TT{get\_bit()}
могут быть представлены как двумерные координаты. Таким образом, массив ~--- это матрица 8*8.}
{We also know the array size is 64. First two arguments both in the \TT{set\_bit()} and \TT{get\_bit()} functions
...
\lstinputlisting{\IFRU{examples/qr9/5ru.asm}{examples/qr9/5en.asm}}
...
{\dots we see that both loop counters are in range 0..7. 
...
\lstinputlisting{\IFRU{examples/qr9/6ru.asm}{examples/qr9/6en.asm}}

\IFRU{\dots этот код помещает содержимое из внутреннего массива в глобальный массив cube используя функцию 
\TT{set\_bit()}, \IT{но}, в обратном порядке!
Теперь счетчик первого цикла в интервале 7 до 0, уменьшается на 1 на каждой итерации!}
{\dots this code is placing contents from internal array to cube global array via \TT{set\_bit()} function, 
...
\lstinputlisting{\IFRU{examples/qr9/7ru.asm}{examples/qr9/7en.asm}}
...
{It is \IT{almost} the same, except of different order of arguments of the \TT{get\_bit()} and \TT{set\_bit()}. 
...
\IFRU{Перепишем так же функцию \TT{rotate3()}:}{Let's also rewrite \TT{rotate3()} function:}
...
\IFRU{Может быть, автор алгоритма думал о кубике Рубика 8*8*8}
...
{It is almost the same except of \TT{crypt()}, \IT{but} password string is reversed by}
strrev() \footnote{\url{http://msdn.microsoft.com/en-us/library/9hby7w40(VS.80).aspx}}
\IFRU{и \TT{rotate\_all()} вызывается с аргументом 3.}
...
\IFRU{\TT{rotate1()}, вероятно, поворот ``лицевой'' плоскости. 
\TT{rotate2()}, вероятно, поворот ``верхней'' плоскости.
\TT{rotate3()}, вероятно, поворот ``левой'' плоскости.}
{\TT{rotate1()} is apparently for rotating ``front'' plane. 
\TT{rotate2()} is apparently for rotating ``top'' plane. 
\TT{rotate3()} is apparently for rotating ``left'' plane.}

\IFRU{Вернемся к ядру функции \TT{rotate\_all()}}{Let's get back to the core of \TT{rotate\_all()} function:}
