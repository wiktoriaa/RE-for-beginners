...
\RU{будет читать байт по адресу}\EN{instruction will read a byte from the} \TT{DS:SI}\RU{, но это не проблема
...
(pseudo-random) bit in the} \TT{CF} \RU{в значение}\EN{flag to a vale of} \TT{0x5C} \OrENRU \TT{0x2F}.
