...
или вызов стандартой ф-ции Си \TT{rand()}.}
...
\TT{cBombStart} \RU{это глобальная переменная, содержащая количество мин. So this is loop.}
\EN{is the global variable containing total number of mines. Так что это цикл.}
...
\RU{Я запустил Сапёр, присоеденил (attach) \olly к нему и увидел содержимое памяти по адресу где массив}\EN{I run 
...
and trapped into mine, but now I see all mines}: \figref{fig:minesweeper1}.
...
\EN{Now I removed all border bytes (0x10) and what's beyond those:}
...
\url{http://beginners.re/examples/minesweeper_WinXP/minesweeper_cheater.exe}}.
...
\EN{Why border bytes (0x10) are exist in array?}
...
\RU{Да, вполне возмжоно автоматически найти информацию о доске в сегменте данных в запущенном процессе Сапёра.}
