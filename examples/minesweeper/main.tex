...
\EN{I'm not very good at playing Minesweeper, so I will try to see the hidden mines in the debugger.}
...
\EN{As we know, Minesweeper places mines randomly, so there should be some kind of random number generator or
...
(\RU{это возможно, если пара ф-ций}\EN{it's possible if the pair of} \TT{Rnd()} 
...
\RU{перейдет на повторную генерацию пары}\EN{will jump to the generation routine again}.
...
0x8F --- мина.}
\EN{By comparing the mine places and the dump, we can conclude that 0x10 means border, 0x0F---empty block, 0x8F---mine.}
...
\EN{Modify my utility so it will remove all mines or set them in a fixed pattern that you want in the Minesweeper
