...
\EN{I once found this one-way hash function that converted a 64-bit value to another and we need to try
...
\EN{A very simple example is CRC32, an algorithm that provides ``stronger'' checksum for integrity checking purposes}.
...
\EN{Indeed: an internet forum database may not contain user passwords (a stolen database will compromise
...
to see which will match the resulting value that we need}.
% TODO add about Rainbow tables
...
\EN{If you are careful enough, this code can be compiled and will even work in the same way as the original}.
...
нашего алгоритма, при котором он выдаст на выходе}\EN{The last constraint at 17 is the most important one: 
we will try to find an input value for which our algorithm will produce} $10816636949158156260$.
...
by adding line 19, which means ``look for any other results than this''}:
...
\EN{Let's be more sadistic and add another constraint: last the 16 bits should be} \TT{0x1234}:
...
\EN{Z3 works very fast and it means that the algorithm is weak, it is not cryptographic at all
...
\RU{Настоящие алгоритмы, такие как}\EN{Real algorithms like} AES, RSA, \RU{и т.д., так же могут быть представлены
