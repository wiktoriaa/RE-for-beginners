...
\RU{Мы находим деление через умножение}\EN{We may spot division using multiplication} (\myref{sec:divisionbynine}).
...
для определенного выходного значения}\EN{Since we are not cryptanalysts we can't find an easy way to 
