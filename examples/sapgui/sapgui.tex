...
(\IFRU{чиайте}{read} \href{http://blog.yurichev.com/node/44}{\IFRU{здесь}{here}} \AndENRU \href{http://blog.yurichev.com/node/47}{\IFRU{здесь}{here}}). 
...
{Settings for each variable are written to the array via pointer in the \EDI register. \EDI is being set before that function call}:
...
{replace it with just \JMP, and get SAPGUI working without that pesky nagging pop-up window appearing!}
...
(I gave that name to that function) function and \TT{this+0x3D} variable in the \IT{CDwsGui::PrepareInfoWindow}. 
...
{At this point we know that compression flag is stored in the \IT{CNetwork} class at \IT{this+0x3A4} address.}
