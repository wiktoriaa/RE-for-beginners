...
\EN{(Tracing connection between \TDWNC{} SAPGUI\footnote{SAP GUI client} environment variable to 
the pesky nagging pop-up window and actual data compression routine.)}

\RU{Известно, что сетевой траффик между SAPGUI и SAP по умолчанию не шифруется а сжимается} 
\EN{It is known that network traffic between SAPGUI and SAP is not crypted by default, it is rather compressed}
(\RU{читайте}\EN{read} \RU{здесь}\EN{here}\footnote{\url{http://blog.yurichev.com/node/44}} 
\AndENRU 
\RU{здесь}\EN{here}\footnote{\url{http://blog.yurichev.com/node/47}}). 

\RU{Известно также что если установить переменную окружения \IT{\TDWNC} в 1, можно выключить сжатие сетевых пакетов.}
\EN{It is also known that by setting environment variable \IT{\TDWNC} to 1, it is possible to turn network packets compression off.}

\RU{Но вы увидите окно, которое нельзя будет закрыть}
\EN{But you will see a nagging pop-up window cannot be closed}:

\begin{figure}[H]
\centering
\includegraphics[scale=\FigScale]{examples/sapgui/sapgui720.png}
...
\EN{First: we know the environment variable \IT{\TDWNC} is checked somewhere inside of SAPGUI client.}
\RU{Второе: строка вроде ``data compression switched off'' также должна где-то присутствовать.}
\EN{Second: string like ``data compression switched off'' must be present somewhere too.}
\RU{При помощи файлового менеджера FAR я нашел обе эти строки в файле SAPguilib.dll.}
\EN{With the help of FAR file manager I found that both of these strings are stored in the SAPguilib.dll file.}

\RU{Так что давайте откроем файл SAPguilib.dll в \IDA и поищем там строку \IT{``\TDWNC''}.
Да, она присутствует и имеется только одна ссылка на эту строку.}
\EN{So let's open SAPguilib.dll in \IDA and search for \IT{``\TDWNC''} string. 
...
\EN{String returned by \TT{chk\_env()} via second argument is then handled by MFC string functions and then 
...
\RU{Обратите также внимание на функцию \TT{chk\_env} (это я так назвал её)}
\EN{Also take a look onto \TT{chk\_env()} function (I gave a name to it)}:
...
\RU{Да}\EN{Yes}. \RU{Функция}\EN{} \TT{getenv\_s()}\footnote{\url{http://msdn.microsoft.com/en-us/library/tb2sfw2z(VS.80).aspx}} 
...
\EN{function is Microsoft security-enhanced version of \TT{getenv()}\footnote{Standard C library returning environment variable}}.

\RU{Тут также имеются манипуляции со строками при помощи функций из MFC}
\EN{There is also a MFC string manipulations}.
...
\EN{Settings for each variable are written to the array via pointer in the \EDI register.
...
\RU{А теперь, можем ли мы найти строку \IT{``data record mode switched on''}?}\EN{Now, can we find \IT{``data record mode switched on''} string?}
\RU{Да, и есть только одна ссылка на эту строку в функции}\EN{Yes, and here is the only reference in function} \TT{CDwsGui::PrepareInfoWindow()}.
\RU{Откуда я узнал имена классов/методов? Здесь много специальных отладочных вызовов, пишущих в лог-файл вроде}
\EN{How do I know class/method names? There is a lot of special debugging calls writing to log-files like}:
...
\RU{Они \textbf{очень} полезны}\EN{It is \textbf{very} useful}.

\RU{Посмотрим содержимое функции ``назойливого всплывающего окна''}
\EN{So let's see contents of the pesky nagging pop-up window function}:
...
\RU{\ECX в начале функции содержит в себе указатель на объект (потому что это тип функции thiscall~(\ref{thiscall})).
...
\EN{\ECX at function start gets pointer to object (since it is thiscall~(\ref{thiscall})-type of function).
In our case, the object obviously has class type \IT{CDwsGui}. 
...
\EN{If value at \TT{this+0x3D} address is not zero, compression is off}:
...
\EN{It is interesting, that finally, \IT{var\_10} variable state defines whether the message is to be shown at all}:

\lstinputlisting{\RU{examples/sapgui/1ru.lst}\EN{examples/sapgui/1en.lst}}
...
\lstinputlisting{\RU{examples/sapgui/2ru.lst}\EN{examples/sapgui/2en.lst}}

\dots \RU{заменим просто на \JMP и получим SAPGUI работающим без этого назойливого всплывающего окна!}
\EN{replace it with just \JMP, and get SAPGUI working without the pesky nagging pop-up window appearing!}
...
\EN{Now let's dig deeper and find connection between 0x15 offset in the \TT{load\_command\_line()} 
(I gave the name to the function) function and \TT{this+0x3D} variable in the \IT{CDwsGui::PrepareInfoWindow}. 
Are we sure the value is the same?}

\RU{Начинаю искать все места где в коде используется константа \TT{0x15}.
Для таких небольших программ как SAPGUI, это иногда срабатывает. Вот первое что я нашел:}
\EN{I'm starting to search for all occurrences of \TT{0x15} value in code. 
For a small programs like SAPGUI, it sometimes works. Here is the first occurrence I got:}
...
\RU{Но настоящий ответ находится в функции}\EN{But the real answer in the function} \IT{CDwsGui::Init()}:
...
\EN{Finally, we understand: array filled in the \TT{load\_command\_line()} function is actually placed in the \IT{CDwsGui} class 
but on \TT{this+0x28} address. 0x15 + 0x28 is exactly 0x3D. OK, we found the point where the value is copied to.}
...
\EN{Let's also find other places where 0x3D offset is used. 
Here is one of them in the \IT{CDwsGui::SapguiRun} function (again, thanks to debugging calls):}
...
\TDWNC и запускаем SAPGUI. Wow! Назойливого окна больше нет (как и ожидалось: ведь переменной окружении также
...
\EN{Let's check our findings. Replace the \TT{setz al} here to the \TT{xor eax, eax / nop} instructions,
clear \TDWNC 
...
Obviously, this is the point where compression flag is to be set in the \IT{CConnectionContext} object.}
...
\EN{So, compression flag is passed in the 5th argument of function \IT{CConnectionContext::CreateNetwork}. 
...
\EN{Compression flag is passing here in the 5th argument to the \IT{CNetwork::CNetwork} constructor.}

\RU{И вот как конструктор \IT{CNetwork} выставляет некоторые флаги в объекте \IT{CNetwork} в соответствии с пятым аргументом \textbf{и}
еще какую-то переменную, возможно, также отвечающую за сжатие сетевых пакетов.}
\EN{And here is how \IT{CNetwork} constructor sets a flag in the \IT{CNetwork} object according to the 5th argument \textbf{and}
...
\EN{At this point we know the compression flag is stored in the \IT{CNetwork} class at \IT{this+0x3A4} address.}
...
\EN{Now let's dig across SAPguilib.dll for \TT{0x3A4} value. And here is the second occurrence in 
the \IT{CDwsGui::OnClientMessageWrite} (endless thanks for debugging information):}
...
\EN{Let's take a look into \IT{sub\_644055C5}. In it we can only see call to memcpy() and an other function named 
...
Voilà! \RU{Мы находим функцию которая собственно и сжимает сетевые пакеты.}\EN{We've found the function which actually compresses data.}
\RU{Как я уже разобрался}\EN{As I revealed in past}
...
\EN{this function is used in SAP and also open-source MaxDB project. 
So it is available in sources.}

\RU{Последняя проверка:}\EN{Doing last check here:}
...
\RU{Заменим \JNZ на безусловный переход \JMP. Уберем переменную окружения \TDWNC.}
\EN{Replace \JNZ here for unconditional \JMP. Remove environment variable \TDWNC.} Voilà! 
\RU{В Wireshark мы видим, что сетевые пакеты, исходящие от клиента, не сжаты. Ответы сервера, впрочем, сжаты.}
\EN{In Wireshark we see the client messages are not compressed. Server responses, however, are compressed.}
...
\EN{So we found exact connection between environment variable and the point where data compression 
