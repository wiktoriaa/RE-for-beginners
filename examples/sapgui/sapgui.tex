\section{\IFRU{Касательно сжимания сетевого траффика в клиенте SAP}{About SAP client network traffic compression}}
\label{sec:SAPGUI}
...
{It is known that network traffic between SAPGUI and SAP is not crypted by default, it is rather compressed}
(\IFRU{читайте}{read} \IFRU{здесь}{here}\footnote{\url{http://blog.yurichev.com/node/44}} 
\AndENRU 
\IFRU{здесь}{here}\footnote{\url{http://blog.yurichev.com/node/47}}). 

\IFRU{Известно также что если установить переменную окружения \IT{\TDWNC} в 1, можно выключить сжатие сетевых пакетов.}
{It is also known that by setting environment variable \IT{\TDWNC} to 1, it is possible to turn network packets compression off.}

\IFRU{Но вы увидите окно, которое нельзя будет закрыть}
...
\IFRU{Посмотрим, сможем ли мы как-то убрать это окно}
...
\IFRU{Второе: строка вроде ``data compression switched off'' также должна где-то присутствовать.}
{Second: string like ``data compression switched off'' must be present somewhere too.}
\IFRU{При помощи файлового менеджера FAR я нашел обе эти строки в файле SAPguilib.dll.}
...
\IFRU{Мы увидим такой фрагмент кода
...
\IFRU{Обратите также внимание на функцию \TT{chk\_env} (это я так назвал её)}
...
\IFRU{Тут также имеются манипуляции со строками при помощи функций из MFC}
...
а также сообщение записываемое им в лог-файл, если переменная включена}{Lots of other environment variables are checked as well. 
...
\IFRU{Они \textbf{очень} полезны}{It is \textbf{very} useful}.

\IFRU{Посмотрим содержимое функции ``назойливого всплывающего окна''}
...
\lstinputlisting{\IFRU{examples/sapgui/1ru.lst}{examples/sapgui/1en.lst}}

\IFRU{Давайте проверим нашу теорию на практике}{Let's check our theory on practice}.

\JNZ \IFRU{в этой строке}{at this line} \dots

\lstinputlisting{\IFRU{examples/sapgui/2ru.lst}{examples/sapgui/2en.lst}}

\dots \IFRU{заменим просто на \JMP и получим SAPGUI работающим без этого назойливого всплывающего окна!}
...
{Now let's dig deeper and find connection between 0x15 offset in the \TT{load\_command\_line()} 
...
\IFRU{Начинаю искать все места где в коде используется константа \TT{0x15}.
Для таких небольших программ как SAPGUI, это иногда срабатывает. Вот первое что я нашел:}
{I'm starting to search for all occurrences of \TT{0x15} value in code. 
...
{Finally, we understand: array filled in the \TT{load\_command\_line()} function is actually placed in the \IT{CDwsGui} class 
but on \TT{this+0x28} address. 0x15 + 0x28 is exactly 0x3D. OK, we found the point where the value is copied to.}

\IFRU{Посмотрим также и другие места, где используется смещение 0x3D.
...
{Let's check our findings. Replace the \TT{setz al} here to the \TT{xor eax, eax / nop} instructions,
clear \TDWNC 
...
{Compression flag is passing here in the 5th argument to the \IT{CNetwork::CNetwork} constructor.}
...
{And here is how \IT{CNetwork} constructor sets a flag in the \IT{CNetwork} object according to the 5th argument \textbf{and}
...
{At this point we know the compression flag is stored in the \IT{CNetwork} class at \IT{this+0x3A4} address.}

\IFRU{Поищем теперь значение \TT{0x3A4} в SAPguilib.dll. Находим второе упоминание этого значения в функции
\IT{CDwsGui::OnClientMessageWrite} (бесконечная благодарность отладочной информации):}
{Now let's dig across SAPguilib.dll for \TT{0x3A4} value. And here is the second occurrence in 
...
{Let's take a look into \IT{sub\_644055C5}. In it we can only see call to memcpy() and an other function named 
...
\IFRU{Как я уже разобрался}
{As I revealed in past}
\footnote{\url{http://conus.info/utils/SAP_pkt_decompr.txt}},
\IFRU{эта функция используется в SAP и в опен-сорсном проекте MaxDB.
...
\IFRU{Так что мы нашли связь между переменной окружения и местом где функция сжатия данных вызывается, а также
