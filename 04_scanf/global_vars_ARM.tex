...
данного участка кода и её адрес нужно сохранить в переменной рядом с кодом.}
{This is because \TT{x} variable can be located somewhere far from this code fragment and its address
should be saved somewhere near.}
...
адрес глобальной переменной \TT{x} нужно иметь где-то рядом, ведь нет никакой гарантии, что
компоновщик\footnote{linker в англоязычной литературе} сможет разместить саму переменную где-то рядом, 
...
should be located somewhere near, because, there are no guarantee that linker will able to place
