...
Еще одна такая ошибка компилятора описана здесь~\ref{anomaly:LLVM}.

Я показываю здесь подобные случаи для того, чтобы легче было понимать, что подобные ошибки компиляторов 
все же имеют место быть, и не следует ломать голову над тем, почему он сгенерировал именно так.
