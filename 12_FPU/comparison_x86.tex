...
they will be at the same positions in the \AX register and all them are placed in high part of the \AX ~--- \AH.}
...
If we recall values of the \CThreeBits for various cases, we will see that conditional jump 
...
to the \STZERO register, and if it's not triggered, the value of the \TT{\_a} variable will be loaded.}
...
{\FCOM is distinguished from \FCOMP is that sense that it just comparing values and leaves FPU stack 
in the same state. 
Unlike previous example, operands here in reversed order. 
And that is why result of comparision in the \CThreeBits will be different:}
...
In other words, that instruction copies \STZERO (where \TT{\_a} value is now) into the \STONE.
...
That instruction used here probably since FPU has no instruction to pop value from stack and 
