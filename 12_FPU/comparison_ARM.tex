...
\IFRU{Входные величины помещаются в}{Input values are placed into} \TT{D17} \IFRU{и}{and} \TT{D16} 
\IFRU{и сравниваются при помощи инструкции}{and then compared with the help of} 
...
{because there is a need to store coprocessor-specific flags.}
...
{then $a$ value will stay in \TT{D16}.}

\IFRU{Предпоследняя инструкция \TT{VMOV} подготовит то что было в \TT{D16} для возврата через 
пару регистров \Rzero и \Rone.}
{Penultimate instruction \TT{VMOV} will prepare value in \TT{D16} for returning via \Rzero and \Rone
registers pair.}
...
{because there are \TT{``IT GT''} instruction placed right before}.
...
{After that instruction, it's possible to place up to 4 instructions, to which predicate suffix will be added.}
...
{4 ``T'' symbols in instruction mnemonic mean that 4 next instructions will be executed if condition is true.}
...
\IFRU{А если бы здесь было, например}{And if there will be, for example}, \TT{ITEEE EQ} (\IT{if-then-else-else-else}), 
...
\TT{ITTE} (\IT{if-then-then-else}) \IFRU{означает что первая и вторая инструкции исполнятся}{mean that 1st and 2nd
...
{In \IDA it's possible to produce listing files, so I did it, but I also set in options 
to show 4 bytes of each opcodes .}
\IFRU{Затем, зная что старшая часть 16-битного опкода \TT{IT} это $0xBF$, я сделал при помощи \TT{grep} это:}
{Then, knowing that high part of 16-bit opcode \TT{IT} is $0xBF$, I did this with \TT{grep}:}
...
assembler will add \TT{IT} instructions automatically, with respectable flags, where it's need.}
...
{Keil not generate special instruction for float numbers comparing, because, it cannot rely it will 
