...
\EN{A lot of instructions has synonyms (denoted with AKA), this was done for convenience}.
\RU{Синонимичные инструкции транслируются в один и тот же опкод}
\EN{Synonymous instructions are translating into the same opcode}.
...
\item[JPO] \RU{переход если сброшен флаг PF}\EN{jump if PF flag is cleared}
\item[JP] \ac{AKA} JPE: \RU{переход если выставлен флаг PF}\EN{jump if PF flag is set}
