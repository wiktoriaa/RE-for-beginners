...
\IFRU{Если он не загружен, Windows предложит запустить один из зарегистрированных в системе отладчиком}
{If it is not loaded, Windows will offer to run one of the registered in the system debuggers}.
\IFRU{Если}{If} \ac{MSVS} \IFRU{заинсталлирована, его отладчик может быть загружен и подключен к процессу}{is installed, 
...
\IFRU{В }{}\ac{MSVC} \IFRU{есть}{has} \gls{compiler intrinsic} \IFRU{для этой инструкции}{for the instruction}:
