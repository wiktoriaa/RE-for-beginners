...
\EN{Registers prefixed with R- appeared in x86-64, and those prefixed with E-~---in 80386}.
...
\ac{AKA} ``source index''. \RU{Используется как источник в инструкциях}\EN{Used as source in the instructions} 
REP MOVSx, REP CMPSx.

\subsection{RDI/EDI/DI/DIL}
\RegTableTwo{RDI}{EDI}{DI}{DIL}

\ac{AKA} ``destination index''. \RU{Используется как указатель на место назначения в инструкции}
...
Больше о нем}
\EN{Usually used for local variables and arguments of function accessing. More about it}: (\myref{stack_frame}).
...
\RU{О}\EN{About the} C3, C2, C1, C0 \RU{читайте больше}\EN{read more}: (\myref{Czero_etc}).
...
\EN{Each tag contain information about physical FPU register (R(x)), not logical (ST(x)).}
