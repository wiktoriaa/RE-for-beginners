...
\begin{description}
  ...
  \item[MOVZX]
  \item[MOV] \IFRU{скопировать значение}{copy value}
  \item[NOP] \ac{NOP}
  \item[NOT]
  \item[OR]
  ...
  \item[RET] \ac{AKA} RETN
  \item[SAHF]
  \item[SHL] \IFRU{сдвинуть значение влево на один бит}{shift value by one bit left}
  \item[SHR]
  \item[SUB] \IFRU{вычесть одно значение из другого. часто встречающаяся инструкция \TT{SUB reg,reg} означает обнуление reg.}{subtract values. frequenly occured pattern \TT{SUB reg,reg} meaning write 0 to reg.}
  \item[TEST]
  \item[XOR] \ac{XOR} \IFRU{значений}{values}. \IFRU{часто встречающаяся инструкция}{frequenly occured pattern} \TT{XOR reg,reg} \IFRU{означает обнуление reg}{meaning write 0 to reg}.
  \item[SBB]
  \item[ADC]
\end{description}
...
\begin{description}
  \item[BSF]
  \item[CMOVcc] \IFRU{условный}{conditional} MOV: \IFRU{скопировать значение если условие верно}{do copy if condition is true}
