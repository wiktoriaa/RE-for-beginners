...
\RU{Общее для}\EN{Common for} 16-bit (8086/80286), 32-bit (80386, \RU{и т.д.}\EN{etc}), 64-bit.
...
\begin{description}
	\item[byte] 8-\bitENRU. 
		...
		\EN{Bytes are passed in 8-bit part of registers}: \TT{AL/BL/CL/DL/AH/BH/CH/DH/SIL/DIL/R*L}.
	\item[word] 16-\bitENRU. \RU{\dittoclosing{} директива ассемблера DW}
		\EN{DW assembly directive \dittoclosing{}}.
		\RU{Слова передаются в 16-битных частях регистров}\EN{Words are passed in 16-bit part
		of registers}: \TT{AX/BX/CX/DX/SI/DI/R*W}.
	\item[double word] (``dword'') 32-\bitENRU. \RU{\dittoclosing{} директива ассемблера DD}
		...
		\EN{or in 32-bit part of registers} (x64). 
	...
	\item[quad word] (``qword'') 64-\bitENRU. \RU{\dittoclosing{} директива ассемблера DQ}
		\EN{DQ assembly directive \dittoclosing{}}.
		\RU{В 32-битном коде, учетверенные слова передаются в парах 32-битных регистров}
		\EN{In 32-bit code, quad words are passed in 32-bit register pairs}.
