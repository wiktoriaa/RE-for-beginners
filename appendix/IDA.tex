...
N 	& \RU{переменовать идентификатор}\EN{rename identificator} \\
? 	& \RU{калькулятор}\EN{calculator} \\
G 	& \RU{переход на адрес}\EN{jump to address} \\
: 	& \RU{добавить комментарий}\EN{add comment} \\
Ctrl-X 	& \RU{показать ссылки на текущую ф-цию, метку, переменную (в т.ч., в стеке)}
		\EN{show refernces to the current function, label, variable (incl. in local stack)} \\
X 	& \RU{показать ссылки на ф-цию, метку, переменную, итд}
...
Alt-T 	& \RU{искать текст (включая инструкции, итд)}\EN{search for text (including instructions, etc)} \\
...
Enter 	& \RU{перейти к ф-ции, переменной, итд}\EN{jump to function, variable, etc} \\
Esc 	& \RU{вернуться назад}\EN{get back} \\
Num -   & \RU{свернуть ф-цию или отмеченную область}\EN{fold function or selected area} \\
Num + 	& \RU{снова показать ф-цию или область}\EN{unhide function or area}\\
\hline
\end{tabular}
\end{center}

\RU{Сворачивание ф-ции или области может быть удобно чтобы прятать те части ф-ции,
...
\RU{это используется в моем скрипте\footnote{\url{\YurichevIDAIDCScripts}}}\EN{this is used in my}
\RU{для сворачивания некоторых очень часто используемых фрагментов inline-кода}
\EN{script\footnote{\url{\YurichevIDAIDCScripts}} for hiding some often used patterns of inline code}.
