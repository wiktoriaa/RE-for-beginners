...
\EN{This string was not googleable upon wide backdoor information discolsure.}
...
\\
\index{Security through obscruity}
\EN{More preciously, such method of hiding backdoors is called ``security through obscruity''.}
\RU{Более точно, такой метод сокрытия бэкдоров называется ``security through obscruity'' (безопасность через
запутанность).}
