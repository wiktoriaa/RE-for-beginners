...
{But not because programmers are writting a lot, but in a reason that all libraries are usually linked statically
to executable files.
...
{Thus, it is very important to determine origin of some function, if it is from standard library or 
...
So when you see some constants or text strings looks familiar, it is always worth to google it.
...
XML-library is used for processing since standard (or well-known) library is often used
...
is working.
It is a huge software, but a detailed .PDB with debugging information is present, and that's cosily.
