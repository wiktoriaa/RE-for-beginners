...
\RU{Иногда, чтобы понять что делает та или иная функция, можно её не разбирать, а просто посмотреть
...
\ifx\LITE\undefined
\RU{Один пример}\EN{One example}: \myref{chap:color_lines}.
\fi
...
\RU{Немалая часть может вызываться из библиотечных функций и}\EN{A lot of functions might
