\chapter{\IFRU{Связь с внешним миром}{Communication with the outer world} (win32)}
...
\IFRU{от}{utility from} SysInternals\EN{ may help}. 
...
\section{\IFRU{Часто используемые ф-ции}{Often used functions in} Windows API}

\IFRU{Это ф-ции которые можно увидеть в числе импортируемых}{These functions may be among imported}.
\IFRU{Но также нельзя забывать, что далеко не все они были использованы в коде написанном автором}
{It is worth to note that not every function might be used by the code written by author}.
\IFRU{Немалая часть может вызываться из библиотечных ф-ций и}{A lot of functions might
be called from library functions and} \ac{CRT}\IFRU{-кода}{ code}.
...
\section{tracer: \IFRU{Перехват всех ф-ций в отдельном модуле}
{Intercepting all functions in specific module}}
\index{tracer}

\IFRU{В \tracer есть INT3-брякпойнты, хотя и срабатывающие только один раз, но зато их можно установить на все
...
{Another drawback~---it is impossible to see function's arguments.}
