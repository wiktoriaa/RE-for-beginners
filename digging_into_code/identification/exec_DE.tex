...
Man kann mehr nachlesen über MSVC's \gls{name mangling} hier: \myref{namemangling}. % <-- bessere formulierung
...
Man kann mehr lesen über GCC's \gls{name mangling} hier: \myref{namemangling}.
...
libifcoremd.dll hat eine menge an Funktionen die das \TT{for\_\ prefix haben, was \IT{Fortran} bedeutet.
...
Zum Beispiel wird so eine Methode benannt \q{method} der Klasse \q{class} die keine Argumente hat und \Tvoid zurück gibt: % <-- Finde was besseres!
...
Hier ist ein Beispiel für Borland Delphi's und C++Builder's \gls{name mangling}:
...
werden in .bpl Datein gehalten anstatt .dll's, zum Beispiel vcl50.dll, rtl60.dll.
...
Fast alle Delphi executables haben den \q{Boolean} Text String am anfang des Code Segments, zusammen mit anderen Typen namen liegen. %  <-- Finde eine bessere formulierung
...
Dies ist ein sehr typischeer anfang für das \TT{CODE} Segment bei einem 
...
Die ersten 4 Btyes des Daten Segments (\TT{DATA}) können \TT{00 00 00 00}, \TT{32 13 8B C0} oder \TT{FF FF FF FF} sein.
