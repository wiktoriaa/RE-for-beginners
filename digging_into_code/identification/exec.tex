...
\hline
\HeaderColor \RU{Маркетинговая версия}\EN{Marketing version} & 
\HeaderColor \RU{Внутренняя версия}\EN{Internal version} & 
\HeaderColor \RU{Версия }CL.EXE\EN{ version} &
\HeaderColor \RU{Импортируемые DLL}\EN{DLLs that can be imported} &
\HeaderColor \RU{Дата выхода}\EN{Release date} \\
\hline
% 4.0, April 1995
...
\EN{The first 4 bytes of the data segment (\TT{DATA}) can be \TT{00 00 00 00}, \TT{32 13 8B C0} or \TT{FF FF FF FF}.}%
\RU{Первые 4 байта сегмента данных (\TT{DATA}) в исполняемых файлах могут быть \TT{00 00 00 00}, \TT{32 13 8B C0} или \TT{FF FF FF FF}.}
