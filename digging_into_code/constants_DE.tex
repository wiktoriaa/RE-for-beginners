...
Der angehende Reverse Engineer kennd diese Werte und ihre Hexadezimale Representation sehr gut:
...
Das hilft villeicht ein Signal von einem Signal zu unterscheiden bei dem alle bits eingeschaltet werden  (0b1111 \dots) oder ausgeschaltet (0b0000 \dots).
Zum Beispiel die \TT{0x55AA} Konstante wird
zumindest beim Boot Sektor benutzt, \ac{MBR},
und im \ac{ROM} von IBM-Kompatiblen erweiterungs Karten.
...
Machnche Algorithmen, speziell die Kryptografischen benutzen eindeutige Konstanten, die einfach zu finden 
...
Zum Beispiel, der MD5\footnote{\href{\URLMD}{wikipedia}} Algorithmus inizialisiert seine Internen Variablen wie folgt:
...
Wenn man diese vier Konstanten im Code hintereinander benutzt findet, dann ist die warscheinlichkeit das diese Funktion 
...
\par Ein weiteres Beispiel sind die CRC16/CRC32 Alrogithmen,
deren berechnungs Algorithmen benutzen oft vorberechnete Tabellen wie diese:
...
Man beachte auch die vorberechnete Tabelle für CRC32: \myref{sec:CRC32}.
% See also the precomputed table for CRC32: \myref{sec:CRC32}.

In tabellen losen CRC Alrgorithmen werden bekannte polynome benutzt, zum Beispiel, 0xEDB88320 für CRC32.
...
Viele Datei Formate definieren einen Standart Datei Header wo eine \IT{magic number(s)}\FNURLMAGIC{} benutzt wird, einzelne oder
sogar mehrere. 
