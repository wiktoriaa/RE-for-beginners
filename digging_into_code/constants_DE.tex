...
beliebigen anderen code bis hin zu einem \TT{CMPSB} (\myref{REPE_CMPSx}) Instruction.
...
Das Datum kann auch in umgekehrter folge auftretten, wie z.B \TT{0x16011987}. 
Datums angaben im Amerikanischen-stihl sind auch weit verbreitet wie \TT{0x01161987}.
...
Solche Zahlen sind auch sehr beliebt in amateur Kryptografie, zum Beispiel, ein Ausschnitt aus den \IT{secret function} Internas aus dem HASP3 Dongle %  <-- Vielleicht bessere formulierung?
...
Zum Beispiel, die Packete des DHCP Protokoll's beinhalten so genannte \IT{magic cookie}: \TT{0x63538263}.
...
Jedes Programm das DHCP Packete emfpangen kann muss verifizieren das der \IT{magic cookie} mit der Konstante 
übereinstimmt. 
...
Machmal, gibt es spezifische Konstanten für gewissen code % <-- Besser? 
...
gibt es ein kleines Utility gennant \IT{binary grep}\footnote{\BGREPURL}.
