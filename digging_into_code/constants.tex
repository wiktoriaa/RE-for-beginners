...
\EN{For example, MD5\footnote{\href{\URLMD}{wikipedia}} algorithm initializes its own internal variables like:}
...
используют заранее заполненные таблицы, вроде}\EN{Another example is CRC16/CRC32 algorithms, 
...
\EN{For example, all Win32 and MS-DOS executables are started with two characters} ``MZ''\footnote{\href{http://go.yurichev.com/17113}{wikipedia}}.
...
\EN{\dots or by calling a function for comparing memory blocks \TT{memcmp()} or any other equivalent code
...
Any program which can receive DHCP packet, must check \IT{magic cookie}, comparing it with the constant.}
