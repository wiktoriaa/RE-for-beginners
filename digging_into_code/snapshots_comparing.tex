% FIXME comparison!
\subsection{\RU{Сравнение \q{снимков} памяти}\EN{Memory \q{snapshots} comparing}}
...
and then compare both: it must be a byte somewhere which was 100 at the beginning, and now it is 99.}
...
\subsubsection{\RU{Реестр Windows}\EN{Windows registry}}
...
\EN{Perhaps, this is the reason why the \q{windows registry cleaner} shareware is so popular.}
\RU{Наверное это причина, почему так популярны shareware-программы для очистки реестра в Windows.}

\subsubsection{\EN{Blink-comparator}\RU{Блинк-компаратор}}
