...
to zero, but if the operands are different, the} ``\RU{исключающего или}\EN{exclusive or}''\EN{ operation
is executed}.
...
\EN{One exception to this observation worth noting is the ``canary'' (\ref{subsec:BO_protection}). 
...
Such instructions are marked as (M) in the instructions list in this appendix: \ref{sec:x86_instructions}.}\\
\\
