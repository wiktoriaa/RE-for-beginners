...
one can try to check each one manually with a debugger.}\PTBRph{}\ESph{}\\
\\
...
operand\EMDASH{}obviously, they do not suit us):}\PTBRph{}\ESph{}\\

\begin{lstlisting}
cat EXCEL.lst | grep fdiv | grep -v dbl_ > EXCEL.fdiv
\end{lstlisting}

\RU{\dots то окажется, что их всего 144.}\EN{\dots then we see that there are 144 of them.}\PTBRph{}\ESph{}\\
\\
\RU{Мы можем вводить в Excel строку вроде \TT{=(1/3)} и проверить все эти инструкции.}
\EN{We can enter a string like \TT{=(1/3)} in Excel and check each instruction.}\PTBRph{}\ESph{}\\
\\
\index{tracer}
...
\EN{Also as a practical joke, we can modify it on the fly:}\PTBRph{}\ESph{}\\
