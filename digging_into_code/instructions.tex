...
one can try to check each one manually with a debugger.}\PTBRph{}\SPph{}\\
\\
...
operand\EMDASH{}obviously, they do not suit us):}\PTBRph{}\SPph{}\\

\begin{lstlisting}
cat EXCEL.lst | grep fdiv | grep -v dbl_ > EXCEL.fdiv
\end{lstlisting}

\RU{\dots то окажется, что их всего 144.}\EN{\dots then we see that there are 144 of them.}\PTBRph{}\SPph{}\\
\\
\RU{Мы можем вводить в Excel строку вроде \TT{=(1/3)} и проверить все эти инструкции.}
\EN{We can enter a string like \TT{=(1/3)} in Excel and check each instruction.}\PTBRph{}\SPph{}\\
\\
\index{tracer}
...
\EN{Also as a practical joke, we can modify it on the fly:}\PTBRph{}\SPph{}\\
