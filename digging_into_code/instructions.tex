...
\EN{If the program is utilizing FPU instructions and there are very few of them in a code,
...
\EN{For example, we may be interesting, how Microsoft Excel calculating formulae entered by user.
...
\EN{If to load excel.exe (from Office 2010) version 14.0.4756.1000 into \IDA, then make a full listing
...
\RU{\dots то окажется, что их всего 144.}\EN{\dots then we realizing they are just 144.}\\
\\
\RU{Мы можем вводить в Excel строку вроде \TT{=(1/3)} и проверить все эти инструкции.}
\EN{We can enter string like \TT{=(1/3)} in Excel and check each instruction.}\\
\\
\index{tracer}
...
\EN{Checking each instruction in debugger or \tracer
...
\RU{В \STZERO содержится первый аргумент (1), второй содержится в}
\EN{\STZERO holding first argument (1) and second one is in} \TT{[EBX]}.\\
\\
\index{x86!\Instructions!FDIV}
\RU{Следующая за \FDIV инструкция записывает результат в память:}
\EN{Next instruction after \FDIV writes result into memory:}\\
...
\EN{If to set breakpoint on it, we may see result:}
...
\EN{Also as a practical joke, we can modify it on-fly:}\\
...
\EN{Excel showing $666$ in the cell what finally convincing us we find the right point.}

\begin{figure}[H]
\centering
\includegraphics[scale=0.66]{digging_into_code/Excel_prank.png}
\caption{\RU{Пранк сработал}\EN{Practical joke worked}}
\end{figure}
...
\EN{If to try the same Excel version, but x64,
...
\EN{It seems, a lot of division operations of \Tfloat and \Tdouble types, compiler replaced by SSE-instructions
