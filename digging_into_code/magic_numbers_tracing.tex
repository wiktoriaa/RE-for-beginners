\section{\IFRU{Использование magic numbers для трассировки}{Magic numbers usage while tracing}}
...
{It's often good to trace, how some value was read from file, or received via network, being used. Often, manual tracing of some value
is very labouring task. One of the simple methods (however, not 100\% reliable) is to use your own \IT{magic number}.}
...
which is used for improving visibility of internal structures in X-rays. For example, it's well known how blood of healthy man/woman
...
{We can take some 32-bit number like \IT{0x0badf00d}, or someone birth date like \IT{0x11101979}
and to write this, 4 byte holding number, to some place in file used by the program we investigate.}
...
это значение, и как.}
{Then, while tracing this program, with \tracer in \IT{code coverage} mode, and then, with help if \IT{grep}
...
It's important to be unique for \IT{magic number} and not to be present in the program's code.}
...
can write information about all register's states for each executed instruction of program to plain text file\footnote{See also my 
blog post about this DosBox feature: \DOSBOXURL{}}, so this method may be useful for DOS programs as well.}

% FIXME: тут примера не хватает
