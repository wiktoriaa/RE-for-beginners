...
\IFRU{генерирующую код для ARM-совместимых процессоров и}{which produces code for ARM-compatible processors and} \IFRU{SoC}{SoCs}\footnote{system on chip} \InENRU \idevices, 
...
\SPwithfootnote is pointing, then it decrements \SP so it will point to the place in the stack that 
...
{It loads values from the stack in order to save them into \Rfour and \PC, and increments stack pointer \SP}.
...
{The difference in a way that beside saving of \ac{RA} in the \LR register and passing control to the \puts function, processor is also switching from the thumb mode to the ARM (or back)}.
