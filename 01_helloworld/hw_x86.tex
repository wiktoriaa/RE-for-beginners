...
{The string \TT{``hello, world''} in \CCpp has type \TT{const char*}, however it doesn't have
its own name.}

\IFRU{Но компилятору нужно как-то с ней работать, так что он дает ей внутреннее имя \TT{\$SG3830}.}
{The compiler needs to work with the string somehows so it defines the internal name \TT{\$SG3830} for it.}
...
{is effectively equivalent to \TT{``POP register''} but without using any register\footnote{CPU flags, however, modified}.}
...
{Some compilers (like Intel C++ Compiler) in the same situation could emit \TT{POP ECX} 
...
\IFRU{Замечание: мы также можем заставить GCC генерировать листинги в этом формате при помощи ключей}
...
{String address (or pointer to string) is then writing directly on the stack space without \PUSH instruction use.
...
emits \TT{MOV EAX, 0} instead of shorter opcode.}
...
{The last instruction \LEAVE ~--- is effectively \TT{MOV ESP, EBP} and \TT{POP EBP} instructions pair equivalent ~--- 
in other words, this instruction setting back stack pointer (\ESP) and the \EBP register to its initial state.} 

\IFRU{Это необходимо, т.к., в начале функции мы модифицировали регистры \ESP и \EBP (при помощи}
{This is necessary since we modified these register values (\ESP and \EBP) at the function start (executing}
