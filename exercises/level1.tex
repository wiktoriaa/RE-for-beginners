\chapter{\RU{Уровень}\EN{Level} 1}

\RU{Задачи первого уровня, это те, которые можно решать голове}\EN{Level 1 exercises are ones you
may try to solve in mind}.

% 1.1
% 1.2
% 1.3

\section{\Exercise 1.4}

\RU{Эта программа запрашивает пароль}\EN{This program requires password}. \RU{Найдите его}\EN{Find it}.

\RU{Как дополнительное упражнение, попробуйте изменить пароль, модифицируя исполняемый файл,
в т.ч., на более короткий или более длинный.}
\EN{As an additional exercise, try to change the password by patching executable file. 
It may also has a different length.}
\RU{Какой самый короткий пароль можно сделать}\EN{What shortest password is possible here}?

\RU{Попытайтесь также вызвать аварийное завершение программы только при помощи ввода строки}
\EN{Try also to crash the program using only string input}.

\begin{itemize}
\item win32: \url{http://beginners.re/exercises/1/4/password1.exe}
\item Linux x86: \url{http://beginners.re/exercises/1/4/password1_Linux_x86.tar}
\item \MacOSX: \url{http://beginners.re/exercises/1/4/password1_MacOSX64.tar}
\item MIPS: \url{http://beginners.re/exercises/1/4/password1_MIPS.tar}
\end{itemize}

