...
\section{\RU{Как работает односторонняя ф-ция}\EN{How one-way function works}?}

\RU{Односторонняя ф-ция, это ф-ция, которая способна превратить из одного значения другое,
...
\RU{Алгоритм простейшей односторонней ф-ции выглядит так}\EN{Algorithm of simplest possible one-way function is}:
...
\RU{Глядя на результат, и даже зная алгоритм ф-ции, мы не можем однозначно восстановить изначальное
положение чисел.
...
\RU{Это крайне упрощенный пример для демонстрации, настоящие односторонние ф-ции могут быть значительно
сложнее.}
