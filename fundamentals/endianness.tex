...
\IFRU{CPU с таким порядком включают в себя}{Big-endian CPUs include} Motorola 68k, IBM POWER.
...
\IFRU{CPU с таким порядком байт включают в себя}{Little-endian CPUs include} Intel x86.
...
{In TCP/IP, big-endian is also called} ``network byte order'',
\IFRU{а}{while} little-endian\EMDASH{}``host byte order''.

\index{x86!\Instructions!BSWAP}
\RU{Инструкция }{The }\TT{BSWAP} \IFRU{также может использоваться для конвертирования}
{instruction can also be used for conversion}.
