...
\RU{Пример в этой книге}\EN{There's an example in this book}: \ref{scanf_global_variable}.
...
\RU{Обычно это локальные для ф-ции переменные}\EN{These are usually local variables for the function}.
...
\EN{But they're also not convenient for buffers/arrays, because the buffer size should be fixed,
unless \TT{alloca()} (\ref{alloca}) (or a variable-length array) is used.}
...
overwrite important stack structures}: \ref{subsec:bufferoverflow}.
...
\EN{Heap allocations are also source of memory leak problems: each memory block should be deallocated
