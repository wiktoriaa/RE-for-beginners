...
{
  name=NEON,
  description={\ac{AKA} ``Advanced SIMD''\IFRU{ --- }{---}\ac{SIMD} \IFRU{от}{from} ARM}
}
...
\newglossaryentry{compiler intrinsic}
{
  ...
  description={\IFRU{Специфичная для компилятора ф-ция не являющяяся обычной библиотечной ф-цией.
	...
	It is often a pseudofunction for specific \ac{CPU} insruction. Read more:} (\ref{compiler_intrinsic})}
}
