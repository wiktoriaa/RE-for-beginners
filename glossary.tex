...
{
  name=\RU{хвостовая рекурсия}\EN{tail call}\ESph{}\PTBRph{},
  ...
  into an iteration for efficiency}\ESph{}\PTBRph{}: \href{http://go.yurichev.com/17105}{wikipedia}}
}

\newglossaryentry{endianness}
{
  name=endianness,
  description={\RU{Порядок байт}\EN{Byte order}\ESph{}\PTBRph{}: \myref{sec:endianness}}
}

\newglossaryentry{caller}
{
  name=caller,
  description={\RU{Функция вызывающая другую функцию}\EN{A function calling another}\ESph{}\PTBRph{}}
}

\newglossaryentry{callee}
{
  name=callee,
  description={\RU{Вызываемая функция}\EN{A function being called by another}\ESph{}\PTBRph{}}
}

\newglossaryentry{debuggee}
{
  name=debuggee,
  description={\RU{Отлаживаемая программа}\EN{A program being debugged}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{leaf function}
{
  ...
  \EN{A function which does not call any other function}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{link register}
{
  ...
  This makes it possible to call leaf functions without using the stack, i.e., faster}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{anti-pattern}
{
  ...
  \EN{Generally considered as bad practice}\ESph{}\PTBRph{}}
}

\newglossaryentry{stack pointer}
{
  name=\RU{указатель стека}\EN{stack pointer}\ESph{}\PTBRph{},
  description={\RU{Регистр указывающий на место в стеке}
  \EN{A register pointing to a place in the stack}\ESph{}\PTBRph{}}
}

\newglossaryentry{decrement}
{
  name=\RU{декремент}\EN{decrement}\ESph{}\PTBRph{},
  description={\RU{Уменьшение на 1}\EN{Decrease by 1}\ESph{}\PTBRph{}}
}

\newglossaryentry{increment}
{
  name=\RU{инкремент}\EN{increment}\ESph{}\PTBRph{},
  description={\RU{Увеличение на 1}\EN{Increase by 1}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{loop unwinding}
{
  ...
  loop body, in order to get rid of the instructions for loop maintenance}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{register allocator}
{
  ...
  \EN{The part of the compiler that assigns CPU registers to local variables}\ESph{}\PTBRph{}}
}

\newglossaryentry{quotient}
{
  name=\RU{частное}\EN{quotient}\ESph{}\PTBRph{},
  description={\RU{Результат деления}\EN{Division result}\ESph{}\PTBRph{}}
}

\newglossaryentry{product}
{
  name=\RU{произведение}\EN{product}\ESph{}\PTBRph{},
  description={\RU{Результат умножения}\EN{Multiplication result}\ESph{}\PTBRph{}}
}

\newglossaryentry{NOP}
{
  name=NOP,
  description={\q{no operation}, \RU{холостая инструкция}\EN{idle instruction}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{POKE}
{
  ...
  description={\RU{Инструкция языка BASIC записывающая байт по определенному адресу}
  	\EN{BASIC language instruction for writing a byte at a specific address}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{keygenme}
{
  ...
  for which one needs to make a key/license generator}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{dongle}
{
  ...
  \EN{Dongle is a small piece of hardware connected to LPT printer port (in past) or to USB}\ESph{}\PTBRph{}.
  ...
  it has some memory and, sometimes, a secret (crypto-)hashing algorithm}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{thunk function}
{
  ...
  \EN{Tiny function with a single role: call another function}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{user mode}
{
  ...
  \EN{A restricted CPU mode in which it all application software code is executed. cf.}\ESph{}\PTBRph{} \gls{kernel mode}}
}
...
\newglossaryentry{kernel mode}
{
  ...
  \EN{A restrictions-free CPU mode in which the OS kernel and drivers execute. cf.}\ESph{}\PTBRph{} \gls{user mode}}
}
...
\newglossaryentry{atomic operation}
{
  ...
  to be interrupted by other threads}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{NaN}
{
  ...
  description={
  	...
  	a special cases for floating point numbers, usually signaling about errors}\ESph{}\PTBRph{}
  }
}
...
\newglossaryentry{basic block}
{
  ...
  description={
	...
	In \IDA it looks just like as a list of instructions without empty lines}\ESph{}\PTBRph{}
  }
}
...
{
  name=NEON,
  description={\ac{AKA} \q{Advanced SIMD}\EMDASH\ac{SIMD} \RU{от}\EN{from}\ESph{}\PTBRph{} ARM}
}
...
\newglossaryentry{reverse engineering}
{
  ...
  \EN{act of understanding how the thing works, sometimes in order to clone it}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{compiler intrinsic}
{
  ...
  description={\RU{Специфичная для компилятора функция не являющаяся обычной библиотечной функцией.
	...
	Often, it's a pseudofunction for a specific \ac{CPU} instruction. Read more:}\ESph{}\PTBRph{} (\myref{sec:compiler_intrinsic})}
}
...
\newglossaryentry{heap}
{
  ...
  malloc()/free() work with the heap}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{name mangling}
{
  ...
  which will become the internal name of the function. You can read more about it here}\ESph{}\PTBRph{}: \myref{namemangling}}
}
...
\newglossaryentry{xoring}
{
  ...
  \ac{XOR}}
  \EN{often used in the English language, which implying applying the \ac{XOR} operation}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{security cookie}
{
  ...
  \EN{A random value, different at each execution. You can read more about it here}\ESph{}\PTBRph{}: \myref{subsec:BO_protection}}
}
...
\newglossaryentry{tracer}
{
  ...
  \EN{My own simple debugging tool. You can read more about it here}\ESph{}\PTBRph{}: \myref{tracer}}
}
...
\newglossaryentry{GiB}
{
  ...
  \EN{Gibibyte: $2^{30}$ or 1024 mebibytes or 1073741824 bytes}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{CP/M}
{
  ...
  \EN{a very basic disk \ac{OS} used before}\ESph{}\PTBRph{} MS-DOS}
}
...
\newglossaryentry{stack frame}
{
  ...
  local variables, function arguments, \ac{RA}, \etc{}}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{jump offset}
{
  ...
  of the next instruction, and this is how the new \ac{PC} is calculated. May be negative as well}\ESph{}\PTBRph{}}
}

\newglossaryentry{integral type}
{
  name=\RU{интегральный тип данных}\EN{integral data type}\ESph{}\PTBRph{},
  ...
  \EN{usual numbers, but not a real ones. may be used for passing variables of boolean data type and enumerations}\ESph{}\PTBRph{}}
}

\newglossaryentry{real number}
{
  name=\RU{вещественное число}\EN{real number}\ESph{}\PTBRph{},
  description={\RU{числа, которые могут иметь точку. в \CCpp это \Tfloat и \Tdouble}
  \EN{numbers which may contain a dot. this is \Tfloat and \Tdouble in \CCpp}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{PDB}
{
  ...
  arguments and local variables names}\ESph{}\PTBRph{}}
}
...
\newglossaryentry{NTAPI}
{
  ...
  Largely not documented by Microsoft}\ESph{}\PTBRph{}}
}
...
{
  name=word,
  description={\EN{data type fitting in \ac{GPR}}\RU{(слово) тип данных помещающийся в \ac{GPR}\ESph{}\PTBRph{}}. 
  \RU{В компьютерах старше персональных, память часто измерялась не в байтах, 
  а в словах}\EN{In the computers older than PCs, 
  the memory size was often measured in words rather than bytes}\ESph{}\PTBRph{}}
}

\newglossaryentry{arithmetic mean}
{
  name=\RU{среднее арифметическое}\EN{arithmetic mean}\ESph{}\PTBRph{},
  description={\EN{a sum of all values divided by their count}
  \RU{сумма всех значений, разделенная на их количество}\ESph{}\PTBRph{}}
}
