\newglossaryentry{tail call}
{
  ...
  into iteration for efficiency}: \href{http://go.yurichev.com/17105}{wikipedia}}
}
...
\newglossaryentry{compiler intrinsic}
{
  ...
  description={\RU{Специфичная для компилятора ф-ция не являющаяся обычной библиотечной ф-цией.
	...
	Often, it's a pseudofunction for specific \ac{CPU} instruction. Read more:} (\myref{sec:compiler_intrinsic})}
}
...
\newglossaryentry{name mangling}
{
  ...
  which will become internal name of the function. read more here}: \myref{namemangling}}
}
...
\newglossaryentry{security cookie}
{
  ...
  \EN{A random value, different at each execution. Read more about it}: \myref{subsec:BO_protection}}
}
...
\newglossaryentry{tracer}
{
  ...
  \EN{My own simple debugging tool. Read more about it}: \myref{tracer}}
}
...
\newglossaryentry{stack frame}
{
  ...
  аргументы ф-ции, \ac{RA}, и т.д.}\EN{Part of stack containing information specific to the current functions:
