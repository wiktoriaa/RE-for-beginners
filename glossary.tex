...
{
  name=caller,
  description={\RU{Функция вызывающая другую функцию}\EN{A function calling another}}
}
...
{
  name=NOP,
  description={\q{no operation}, \RU{холостая инструкция}\EN{idle instruction}}
}
...
{
  name=NEON,
  description={\ac{AKA} \q{Advanced SIMD}\EMDASH\ac{SIMD} \RU{от}\EN{from} ARM}
}
...
\newglossaryentry{integral type}
{
  ...
  \EN{usual numbers, but not a real ones. may be used for passing variables of boolean data type and enumerations}}
}

\newglossaryentry{real number}
{
  name=\RU{вещественное число}\EN{real number},
  description={\RU{числа, которые могут иметь точку. в \CCpp это \Tfloat и \Tdouble}
  \EN{numbers which may contain a dot. this is \Tfloat and \Tdouble in \CCpp}}
}
