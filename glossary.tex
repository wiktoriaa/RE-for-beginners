...
{
  name=GiB,
  description={\IFRU{Гибибайт: $2^{30}$ или 1024 мебибайт или 1073741824 байт}
...
{
  name=CP/M,
  description={Control Program for Microcomputers: \IFRU{очень простая дисковая \ac{OS} использовавшаяся перед}
  {a very basic disk \ac{OS} used before} MS-DOS}
}

\newglossaryentry{stack frame}
{
  name=stack frame,
  description={\IFRU{Часть стека, в которой хранится информация связанная с текущей ф-цией: локальные переменные,
  аргументы ф-ции, \ac{RA}, итд}{Part of stack containing information specific to the current functions:
...
{
  name=jump offset,
  description={\IFRU{Часть опкода JMP или Jcc инструкции, просто прибавляется к адресу следующей инструкции,
  и так вычисляется новый \ac{PC}. Может быть отрицательным.}{a part of JMP or Jcc instruction opcode, 
