...
is beginning to process instruction after the next one, so that is why \PC pointing there.}
...
{But it is interesting to note that the function uses \LR reigster as a pointer to the table.}
...
{It is also worth noting that the code is generated as a separate function in order to reuse it, in similar places,
