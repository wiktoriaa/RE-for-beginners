...
вывод строки}
{Let's keep in mind that maximum value for $a$ is $4$ and any greater value should cause}
...
(\IFRU{это случай с}{it's a} $R0 \geq 5$\IFRU{}{ case}), 
\IFRU{выполнится переход на метку}{a jump to} 
\IT{default\_case}\IFRU{.}{label will be occured.}

\IFRU{Но если}{But if} $R0 < 5$ \IFRU{и}{and} \TT{ADDCC} \IFRU{сработает, то произойдет следующее:}
{will trigger, following events will happen:}

\IFRU{Значение в \Rzero умножается на $4$}{Value in \Rzero is multiplied by $4$}.
\IFRU{Фактически}{In fact}, \TT{LSL\#2} \IFRU{в конце инструкции означает ``сдвиг влево на 2 бита''.}
{at the instruction's ending mean ``shift left by 2 bits''.}
...
\IFRU{содержимое \PC на 8 байт больше}{\PC value is 8 bytes ahead} ($0x180$) 
\IFRU{чем адрес по которому расположена сама инструкция} 
{than address at which} \TT{ADDCC} \IFRU{}{instruction is located} ($0x178$), 
...
{This is how ARM processor pipeline works: when \TT{ADDCC} instruction is executed, the processor at that moment
is beginning to process instruction after the next one, so that's why \PC pointing there.}
...
{If $a=0$, then nothing will be added to \PC,
and actual \PC value will be written into \PC (which is 8 bytes ahead)
and jump to label \IT{loc\_180} will be happen,
this is 8 bytes ahead of the place where \TT{ADDCC} instruction is.}

\IFRU{В случае, если}{In case of} $a=1$, \IFRU{тогда в \PC запишется}{then} 
$PC+8+a*4 = PC+8+1*4 = PC+16 = 0x184$\IFRU{, это адрес метки \IT{loc\_184}}{will be written to \PC,
this is address of \IT{loc\_184} label}.
...
{It's even can be said that in these modes instructions has variable length, just like in x86.}
...
default-case, and offset, for each, each encoding a label, to which control should be passed in 
corresponding case.}
...
{It's too complex for describing it here now, so I'll omit elaborations.}
%TODO дописать когда-то?

\index{ARM!\Registers!Link Register}
\IFRU{Но можно отметить, что эта функция использует регистр \LR как указатель на таблицу.}
{But it's interesting to note that the function uses \LR reigster as a pointer to the table.}
...
{It's also worth noting that the code is generated as a separate function in order to reuse it, in similar places,
in similar cases, for \IT{switch()} processing, so compiler will not generate same code at each place.}
% код выделен -> en?
