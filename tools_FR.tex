...
Has assembleur/désassembleur.

\item {Libre, open-source} \IT{GHex}\footnote{\url{https://wiki.gnome.org/Apps/Ghex}}: éditeur
hexadécimal simple pour Linux.

\item (Libre, open-source) \IT{xxd} et \IT{od}: utilitaires standards UNIX pour réaliser un dump.

\item (Libre, open-source) \IT{strings}: outil *NIX pour rechercher des chaînes ASCII dans des fichiers binaires, fichiers exécutables inclus.
...
\item (Libre, open-source) \IT{Binwalk}\footnote{\url{http://binwalk.org/}}: analyser les images firmware.

\item
\myindex{binary grep}
(Libre, open-source) \IT{binary grep}:
...
\item IDA possède une calculatruce intégrée (``?'').

\item rada.re a \IT{rax2}.
