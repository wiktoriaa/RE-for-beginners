...
{It is very good if the program has as few jumps as possible, conditional and unconditional, so that's why,
...
{There are no such feature in x86, if not to count \TT{CMOVcc} instruction, it is the same as \MOV,
but it is triggered only when specific flags are set, usually, set while comparing by \CMP.}
...
(\IT{Unsigned lower or same}) \AndENRU \TT{BCS} (\IT{Carry Set (Greater than ir equal)}).
