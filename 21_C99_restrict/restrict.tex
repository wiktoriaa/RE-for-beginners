...
\index{C99!restrict}
\index{\CLanguageConstructs!restrict}
...
{because, there are nothing criminal in it, right?}
...
{on this stage, we need to load from memory already calculated} \TT{sum[i]} \IFRU{и}{and} \TT{product[i]}
\end{itemize}

\IFRU{Возможно ли соптимизировать последнюю стадию?}{Is it possible to optimize the last stage?}
\IFRU{Ведь подсчитанные}{Because already calculated} \TT{sum[i]} \IFRU{и}{and} \TT{product[i]} 
...
{If to be more precise and describe this formally, \IT{restrict} shows that only this pointer will be used
...
\IFRU{Разница между скомпилированной функцией \TT{f1()} и \TT{f2()} такая}{The difference between
compiled \TT{f1()} and \TT{f2()} function as follows}:
\IFRU{в}{in} \TT{f1()}, \TT{sum[i]} \IFRU{и}{and} \TT{product[i]} \IFRU{загружаются снова посреди тела цикла}
...
\IFRU{ведь мы ``пообещали'' компилятору}{because we ``promised'' to compiler}, 
\IFRU{что никто и ничто не изменит значения в}{that no one and nothing will change values in} \TT{sum[i]} 
\IFRU{и}{and} \TT{product[i]} \IFRU{во время исполнения тела цикла}{while execution of loop body}, 
