...
bundle types (\ac{AKA} ``templates''): a bundle code defines the instruction types in the bundle}.
...
\EN{For example, the zeroth bundle type is \TT{MII}, meaning: 
...
\EN{Each bundle may have a ``stop bit'',
...
\RU{А если это так, то результаты будут непредсказуемые}\EN{If they do, the results will be undefined}.
...
\EN{This means that these instructions must be executed isolated from all others (as in \ac{CISC})}.
...
\gls{NaN}-числа}
\EN{Another very interesting Itanium feature is the \IT{speculative execution} and the NaT (``not a thing'') bit,
