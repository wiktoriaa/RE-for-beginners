...
\EN{It is probably code generator bug was not found by tests, because, 
resulting code is working correctly anyway.}

\RU{Еще подобные аномалии компиляторов в этой книге}
\EN{Another compiler anomalies here in this book}: 
\ref{anomaly:LLVM}, \ref{loops_iterators_loop_anomaly}, \ref{Keil_anomaly},
\ifdefined\RUSSIAN
\else
\ref{MSVC2013_anomaly},
\ref{MSVC_double_JMP_anomaly},
\fi
\ref{MSVC2012_anomaly}.
...
\EN{I demonstrate such cases here, so to understand that such compilers errors are possible and sometimes
