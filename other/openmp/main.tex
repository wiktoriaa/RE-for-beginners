...
\index{Bitcoin}
\IFRU{В качестве примера, попытаемся написать программу для вычисления криптографического nonce}
{As an example, let's try to build a program to compute cryptographic nonce}.
\IFRU{В моем простейшем примере, nonce это число добавляемое к нешифрованному тексту, чтобы получить
хэш с какой-то особенностью}
{In my simplistic example, 
nonce is a number added to the plain unencrypted text in order to produce hash with some specific 
feauture}.
\IFRU{Например, на одной из стадии, протокол Bitcoin требует найти такую nonce, чтобы в результате
хэширования подряд шли определенное количество нулей}
{For example, at some step, Bitcoin protocol require to find a such nonce so resulting hash
...
\footnote{\IFRU{файлы }{}sha512.c \AndENRU u64.h \IFRU{можно взять из библиотеки OpenSSL}
...
vcomp*.dll\IFRU{}{ file}.
\IFRU{Так что тут запускаются группа тредов}{So here is a group of threads are started}.
...
\InENRU task manager \IFRU{я вижу 5 тредов}{I see 5 threads}: \IFRU{один главный тред + 4 запущенных}
...
parts}.
\IFRU{Хорошо, почти равных, если не учитывать последний бит}{Well, almost even, 
...
\TT{vcomp\_enter\_critsect()} \IFRU{в конце концов вызывает ф-цию win32 API}
{eventually callins win32 API function} \TT{EnterCriticalSection()}
...
\IFRU{В отличие от реализации MSVC, то что делает код GCC это запускает 4 треда, но также запускает 
