...
% FIXME rewrite!
\RU{У всех инструкций суффикс -SS, это означает ``Scalar Single''.}
\EN{All instructions have the -SS suffix, which stands for ``Scalar Single''.}
\RU{``Scalar'' означает что только одно значение хранится в регистре.}
\EN{``Scalar'' implies that only one value is stored in the register.}
\RU{``Single'' означает что это тип \Tfloat.}
\EN{``Single'' stands for \Tfloat data type.}
