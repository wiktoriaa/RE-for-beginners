...
\TT{DIVSD} \RU{это SSE-инструкция, означает}\EN{is SSE-instruction, meaning} 
...
путаницы и переименовали инструкцию в}\EN{Apparently, Microsoft developers wanted to get rid of mess,
...
GCC \RU{работает понятнее}\EN{makes more clear result}. 
...
флаги}\EN{instruction compares values and set/clear} \CF \AndENRU \PF\EN{ flags}:
...
\EN{Let's revisit ``calculating machine epsilon'' example for \Tdouble \lstref{machine_epsilon_double_c}.}
...
\EN{There are no way to add 1 to value in 128-bit XMM register, so it must be placed into memory yet.}
...
\EN{There are, however, ADDSD instruction (\IT{Add Scalar Double-Precision Floating-Point Values}) 
which can add a value to lowest 64-bit half of XMM register while ignoring highest one, 
...
eliminate local stack usage}.}.

\RU{Так или иначе, значение затем перезагружается в XMM-регистр и происходит вычитание.}
\EN{Nevertheless, the value is then reloaded to XMM register and subtraction is occurred.}
...
\EN{i.e., it operates on lowest 64-bit part of 128-bit XMM register}.
\RU{Результат возвращается в регистре XMM0}\EN{Result is returned in XMM0 register}.
...
\EN{And it is easier than FPU, probably because SIMD extensions 
were evolved not as chaotic as FPU in the past.}
...
\EN{Hence, FPU may produce less round-off errors, as a consequence, FPU may give more precise
calculation results.}
