...
\lstinputlisting[caption=\Optimizing GCC 4.4.6 x64]{patterns/205_floating_SIMD/pow_GCC446_x64_O3_\LANG.s}
...
\ifdefined\RUSSIAN
\else
\section{Calculating machine epsilon: x64 and SIMD}
\label{machine_epsilon_x64_and_SIMD}

Let's revisit ``calculating machine epsilon'' example for \Tdouble \lstref{machine_epsilon_double_c} again.
...
There are no way to add 1 to 128-bit XMM register, so it must be placed into memory yet.

There are, however, ADDSD instruction (\IT{Add Scalar Double-Precision Floating-Point Values}) 
...
\footnote{As an exercise, you may try to rework this code to eliminate local stack usage.}.

Nevertheless, the value is then reloaded to XMM register and subtraction is occurred.
SUBSD is ``Subtract Scalar Double-Precision Floating-Point Values'', i.e., it operates
on lowest 64-bit part of 128-bit XMM register.
Result is returned in XMM0 register.
\fi
