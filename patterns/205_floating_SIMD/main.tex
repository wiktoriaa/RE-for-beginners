...
(``Scalar Single-Precision''), \RU{например}\EN{for example}, \TT{MOVSS}, \TT{COMISS}, \TT{ADDSS}, \etc{}.

``Scalar'' \RU{означает что SIMD-регистр будет хранить только одно значение, вместо нескольких.}
...
\EN{Instructions working with several values in a register simultaneously has ``Packed'' in their name}.
