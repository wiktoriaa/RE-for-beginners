...
\IFRU{Счетчик итераций \TT{i} будет храниться в регистре \Reg{4}.}
{Iteration counter \TT{i} is to be stored in the \Reg{4} register.}

\RU{Инструкция }\TT{``MOV R4, \#2''} \IFRU{просто инициализирует}{instruction just initializing} \IT{i}.

\RU{Инструкции }\TT{``MOV R0, R4''} \AndENRU \TT{``BL f''} \IFRU{составляют тело цикла}{instructions are
compose loop body}, 
...
\RU{Инструкция }\TT{``ADD R4, R4, \#1''} \IFRU{прибавляет единицу к \IT{i} при каждой итерации.}
...
\IFRU{В противном случае}{Otherwise}, \IFRU{в \Reg{0} запишется $0$}{$0$ will be written into \Reg{0}} 
