...
\lstinputlisting[caption=GCC 4.9 x64 optimized for size (-Os),style=customasm]{patterns/09_loops/memcpy/memcpy_GCC49_x64_Os_EN.s}

\lstinputlisting[caption=GCC 4.9 ARM64 optimized for size (-Os),style=customasm]{patterns/09_loops/memcpy/memcpy_GCC49_ARM64_Os_EN.s}

\lstinputlisting[caption=\OptimizingKeilVI (\ThumbMode),style=customasm]{patterns/09_loops/memcpy/memcpy_Keil_Thumb_O3_EN.s}

\subsubsection{ARM in ARM mode}

Keil in ARM mode takes full advantage of conditional suffixes:

\lstinputlisting[caption=\OptimizingKeilVI (\ARMMode),style=customasm]{patterns/09_loops/memcpy/memcpy_Keil_ARM_O3_EN.s}

That's why there is only one branch instruction instead of 2.

\subsubsection{MIPS}

\lstinputlisting[caption=GCC 4.4.5 optimized for size (-Os) (IDA),style=customasm]{patterns/09_loops/memcpy/memcpy_MIPS_Os_IDA_EN.lst}
