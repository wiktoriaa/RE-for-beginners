...
\lstinputlisting[caption=GCC 4.9 x64 оптимизация по размеру (-Os),style=customasm]{patterns/09_loops/memcpy/memcpy_GCC49_x64_Os_RU.s}

\lstinputlisting[caption=GCC 4.9 ARM64 оптимизация по размеру (-Os),style=customasm]{patterns/09_loops/memcpy/memcpy_GCC49_ARM64_Os_RU.s}

\lstinputlisting[caption=\OptimizingKeilVI (\ThumbMode),style=customasm]{patterns/09_loops/memcpy/memcpy_Keil_Thumb_O3_RU.s}

\subsubsection{ARM в режиме ARM}

Keil в режиме ARM пользуется условными суффиксами:

\lstinputlisting[caption=\OptimizingKeilVI (\ARMMode),style=customasm]{patterns/09_loops/memcpy/memcpy_Keil_ARM_O3_RU.s}

Вот почему здесь только одна инструкция перехода вместо двух.

\subsubsection{MIPS}

\lstinputlisting[caption=GCC 4.4.5 оптимизация по размеру (-Os) (IDA),style=customasm]{patterns/09_loops/memcpy/memcpy_MIPS_Os_IDA_RU.lst}
