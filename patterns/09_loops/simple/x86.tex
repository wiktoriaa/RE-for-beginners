...
\\
\RU{OK, увеличим максимальное значение $i$ в цикле до $100$ и попробуем снова. GCC выдает:}
...
\RU{после \glslink{increment}{инкремента} не срабатывает и ф-ция заканчивается}
...
\RU{Опция }\TT{BPX} 
\RU{просто ставит брякпойнт по адресу и tracer затем будет выдавать состояние регистров}
