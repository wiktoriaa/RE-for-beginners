...
\EN{The generated code will consist of four parts as well.}
...
\RU{Использовать большие развернутые циклы в наше время не рекомендуется, потому что б\'{о}льшие
ф-ции требуют больше кэш-памяти}
\footnote{
...
\EN{OK, let's increase the maximum value of the $i$ variable to $100$ and try again. GCC will do:}
...
with the exception that the \EBX register will be allocated for the $i$ variable.}
...
\EN{will not trigger after the \gls{increment}, and the function will finish}:
...
\EN{just sets a breakpoint at the address and will then print the state of the registers}.
...
\TT{BPF} \RU{означает установить брякпойнт на ф-ции}\EN{stands for set breakpoint on function}.
