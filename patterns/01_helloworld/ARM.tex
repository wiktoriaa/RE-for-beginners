...
{That's because our C-function returns $0$ and the returned value is to be placed in the \Reg{0} register}.
...
{It loads values from the stack in order to save them into \Reg{4} and \PC, and \glslink{increment}{increments} \gls{stack pointer} \SP}.
...
{writes $0$ into the \Reg{0} register in order to return zero value}.
...
{instruction adds value in the \PC to the value in the \Reg{0}, to calculate absolute address of the ``Hello world!'' string. 
...
{instruction intended to set $0$ to the \Reg{0} register}.
