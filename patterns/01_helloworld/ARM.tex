...
(\Reg{4} \AndENRU \LR) \IFRU{в стек}{into the stack}.
...
\IFRU{Итак, эта инструкция уменьшает \TT{SP}, чтобы он указывал на место в стеке, свободное для записи
новых значений, затем записывает значения регистров \Reg{4} и \LR 
по адресу в памяти, на который указывает измененный регистр \SP}
{This instruction first \glspl{decrement} \SP so it will point to the place in the stack
that is free for new entries, then it writes the values of the \Reg{4} and \LR registers at the address
in changed \SP}.
...
\IFRU{Также следует заметить, что \TT{STMFD} ~--- генерализация инструкции \PUSH (то есть, расширяет её возможности), потому что может работать с любым регистром, а не только с \SP, это тоже может быть очень удобно}
{It can also be noted that the \TT{STMFD} instruction is a generalization 
of the \PUSH instruction (extending its features), since it can work with any register, not just with \SP, and this can be very useful}.
...
\IFRU{Причем здесь \PC, можно спросить}{How is the \TT{PC} register used here, one might ask}?
...
\IFRU{Она загружает из стека значения для сохранения их в \Reg{4} и \PC, увеличивая \glslink{stack pointer}{указатель стека} \SP}
{It loads values from the stack in order to save them into \Reg{4} and \PC, and \glslink{increment}{increments} the \gls{stack pointer} \SP}.
\IFRU{Это, в каком-то смысле, аналог \POP}{It can be said that it is similar to \POP}. 
N.B. \IFRU{Самая первая инструкция \TT{STMFD} сохранила в стеке \Reg{4} и \LR, а \IT{восстанавливаются} во время исполнения \TT{LDMFD} регистры \Reg{4} и \PC}
{The very first instruction \TT{STMFD} saves the \Reg{4} and \LR registers pair on the stack, but \Reg{4} and \PC are \IT{restored} during execution of \TT{LDMFD}}.

\IFRU{Как я уже описывал, в регистре \LR обычно сохраняется адрес места, куда нужно всякой функции вернуть управление}
{As I wrote before, the address of the place to where each function must return control is usually saved in the \LR
register}.
...
{This is because it is impossible to load an offset for the \printf function into \PC while using the small space in one 16-bit opcode}.
...
{This mean it is impossible for a thumb-instruction to be at an odd address whatsoever}.
...
{As for the other instructions in the function: \PUSH and \POP work just like the described \TT{STMFD}/\TT{LDMFD} but the \SP register is not mentioned explicitly here}.
...
\IFRU{Инструкция}{The} \TT{``ADD R0, PC, R0''} \IFRU{прибавляет \PC к \Reg{0} для вычисления действительного адреса строки ``Hello world!''. Как нам уже известно, это ``\PICcode'', поэтому такая корректива необходима}
{instruction adds the value in the \PC to the value in the \Reg{0}, to calculate absolute address of the ``Hello world!'' string. 
...
{The difference is that, besides saving the \ac{RA} in the \LR register and passing control to the \puts function, the processor is also switching from thumb mode to ARM (or back)}.
