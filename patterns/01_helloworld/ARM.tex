...
{That's because our C-function returns $0$ and the return value is to be placed in the \Rzero register}.
...
{It loads values from the stack in order to save them into \Rfour and \PC, and \glslink{increment}{increments} the \gls{stack pointer} \SP}.
...
{This is because it is not possible to load an offset for the \printf function into \PC while using the small space in one 16-bit opcode}.
...
{This mean it is not possible for a thumb-instruction to be at an odd address whatsoever}.
...
{writes $0$ into the \Rzero register in order to return zero}.
...
{instruction adds the value in the \PC to the value in the \Rzero, to calculate absolute address of the ``Hello world!'' string. 
...
{instruction intended to set the \Rzero register to $0$}.
