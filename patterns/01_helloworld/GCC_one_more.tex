...
\RU{Тот факт, что \IT{анонимная} Си-строка имеет тип}\EN{The fact \IT{anonymous} C-string has} 
...
fact C-strings allocated in constants segment are guaranteed to be immutable, has interesting consequence}:
\RU{компилятор может использовать определенную часть строки}\EN{compiler may use specific part of string}.
...
GCC 4.8.1}\EN{Common \CCpp{}-compiler (including MSVC) will allocate two strings, but let's see what 
GCC 4.8.1 is doing}:
...
\RU{Действительно: когда мы выводим строку}\EN{Indeed: when we print ``hello world'' string}, 
\RU{эти два слова расположены в памяти в притык друг к другу и \puts, вызываясь из ф-ции f2(), вообще не знает
что эти строки разделены}\EN{these two words are laying in memory adjacently and \puts called from f2() 
...
\EN{This clever trick is often used by at least GCC and can save some memory bytes.}
