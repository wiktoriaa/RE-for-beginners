...
% Line 5 refers to the GP, but only the FP ($sp) and RA ($31) are saved.
% It seems that saving the GP is not done,
% and is not necessary in both the optimized (which is missing the recovery instruction in both versions) and unoptimized cases,
...
% IDA does not see $gp in line 10, but $ra.
