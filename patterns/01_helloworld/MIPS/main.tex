...
\subsection{\RU{О ``глобальном указателе'' (``global pointer'')}\EN{A word about ``global pointer''}}
\label{MIPS_GP}

\index{MIPS!\GlobalPointer}
\RU{``Глобальный указатель'' (``global pointer'')~--- это важная концепция в MIPS.}
...
\EN{In an ELF file this 64KiB area is located partly in sections .sbss (``small \ac{BSS}'') for not initialized data and 
...
\RU{Некоторые программисты ``старой школы'' могут вспомнить модель памяти в MS-DOS \myref{8086_memory_model} 
...
\EN{Lets consider the following example which illustrates ``global pointer'' concept.}
\RU{Рассмотрим следующий пример, иллюстрирующий концепцию ``глобального указателя''.}
...
\RU{Адрес функции \puts загружается в \$25 инструкцией LW (``Load Word'').}
...
\RU{Затем адрес текстовой строки загружается в \$4 парой инструкций LUI (``Load Upper Immediate'') и
