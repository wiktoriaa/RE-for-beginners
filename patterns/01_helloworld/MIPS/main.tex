...
\RU{Эта область обычно содержит глоабльные переменные и адреса импортированных ф-ций вроде \printf,
...
\RU{И еще одна важная вещь это то что адрес сохраняемый в \ac{RA} это адрес не следующей инструцкии (потому что
...
\RU{Очевидно, разработчики MIPS хотели сделать как можно более комактную таблицу опкодов.}
...
\EN{Most likely, these pseudoinstructions are optimized by \ac{CPU} and \ac{ALU} is never used.}
...
\RU{Инструкция на строке 15 сохраняет GP в локальном стеке, и эта инструкция мистическим образом осутствует
