...
\EN{Some old-school programmers may recall the MS-DOS memory model \ref{8086_memory_model} 
...
\EN{The \$GP register is set in function prologue to be in the middle of this area.}
...
\EN{ADDIU follows after JALR (remember \IT{branch delay slots}?).}
...
is available in appendix} \ref{MIPS_registers_ref}}.
...
\EN{Oh, and one important thing is that address saved in \ac{RA} is not the address of the next instruction (because
...
\EN{Apparently, the MIPS developers came with idea that zero in fact is busiest constant in computer programming,
...
\EN{This does not mean actual addition happens at each MOVE instruction.}
...
\EN{The instruction at line 15 saves GP value into the local stack, and this instruction is mystically missing
from the GCC output listing, maybe by GCC error\footnote{Apparently, functions which generate listings 
...
\EN{We see here that register FP is used as a pointer to stack frame.}
\RU{Мы также видим 3 \ac{NOP}-а.}\EN{We also see 3 \ac{NOP}s.}
\RU{Второй и третий следуют за инструкциями перехода.}
\EN{The second and third follow the branch instructions.}
...
(because \printf is called), and the local stack is used for it.}
\RU{Если бы это была \gls{leaf function}, тогда можно было бы избавиться от пролога и эпилога ф-ции,
например:}
\EN{If this was a \gls{leaf function}, it would have been possible to get rid of function prologue and epilogue,
