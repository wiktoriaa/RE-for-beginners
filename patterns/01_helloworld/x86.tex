...
\lstinputlisting{patterns/01_helloworld/1_2.c}

\IFRU{Вернемся к листингу на ассемблере. Как видно, строка заканчивается нулевым байтом ~--- это требования стандарта \CCpp для строк.}
{Let's back to the assembly listing. As we can see, the string is terminated by a zero byte which is standard for \CCpp strings.}
...
{Before the call, the address of the string containing our greeting is placed on the stack with the help of the \PUSH instruction.}
...
{When the \printf function returns flow control to the \main function, the address of the string is still on the stack.}
...
{Why 4? Since it is 32-bit code, we need exactly 4 bytes for passing an address through the stack. 
...
{Some compilers (like Intel's C++ Compiler) in the same situation may emit \TT{POP ECX} 
instead of \ADD (e.g. such a pattern can be observed in the \oracle{} code as it is compiled by Intel's C++ compiler).
...
\TT{ADD ESP, x} (1 byte versus 3 bytes).}
...
{~---again because it is a slightly shorter opcode (2 bytes versus 5).}
