...
\RU{Это нужно для установки стекового фрейма ф-ции}\EN{This is done to set up the function stack frame}.

\label{pointers_ADRP_and_ADD}
\index{ARM!\Instructions!ADRP/ADD pair}
\RU{Инструкции }\TT{ADRP} \AndENRU \ADD \EN{instructions are used to fill the 
string}\RU{нужны для формирования адреса строки} ``Hello!'' \EN{address into the \RegX{0} register}\RU{в регистре \RegX{0}}, 
...
length is limited to 4 bytes, read more about it here}: \ref{ARM_big_constants_loading}).
...
\EN{More about that in}\RU{Читайте больше об этом}: \ref{ARM64_relocs}.
...
\EN{\puts is called afterwards using \TT{BL} instruction. This was already discussed: \ref{puts}.}
...
регистра \RegX{0}.}
\EN{So if a function returns 32-bit \Tint, only the low 32 bits of \RegX{0} register should be filled.}
