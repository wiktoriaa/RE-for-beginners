...
\EN{The compiler generated \TT{1.obj} file, which is to be linked into \TT{1.exe}.}
...
\RU{Так что пример можно было бы переписать вот так}\EN{That is why the example may be rewritten as follows}:
...
\RU{Вот пример оттуда}\EN{Here is an example}:
...
\TT{SUB EAX, EAX}, 
\RU{что значит \IT{отнять значение в \EAX от значения в \EAX}, 
что в любом случае даст 0 в результате.}
\EN{which means \IT{SUBtract the value in the \EAX from the value in \EAX}, 
which in any case resulting in zero.}
...
Usually, this is \CCpp \ac{CRT} code, which in its turn returns the control to the \ac{OS}.}
