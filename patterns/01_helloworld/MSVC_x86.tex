...
\EN{The difference between Intel-syntax and AT\&T-syntax will be discussed hereafter.}
...
\EN{In our case, the file contain two segments: \TT{CONST} (for data constants) and \TT{\_TEXT} (for code).} 

\index{\CLanguageElements!const}
\RU{Строка \TT{``hello, world''} в \CCpp имеет тип \TT{const char*}, однако не имеет имени.}
\EN{The string \TT{``hello, world''} in \CCpp has type \TT{const char*}, however it does not have
its own name.}
...
\EN{Let's back to the assembly listing. 
...
\EN{Read more about it in section about function prolog and epilog}
...
\EN{When the \printf function returns flow control to the \main function, string address (or pointer to it) is still in stack.}
...
\EN{Some compilers (like Intel C++ Compiler) in the same situation may emit \TT{POP ECX} 
...
\EN{In the generated code this is implemented by instruction} \TT{XOR EAX, EAX} 
...
\EN{but compilers often use it instead of}
...
\EN{which means \IT{SUBtract the value in the \EAX from the value in \EAX}, which in any case will result zero.}
