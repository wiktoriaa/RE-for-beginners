...
\EN{In x86-64, all registers were extended to 64-bit and now have an \TT{R-} prefix}.
\index{fastcall}
...
\EN{The pointers are 64-bit now, so they are passed in the 64-bit part of registers (which have the \TT{R-} prefix)}.
...
\RU{Это}\EN{It's} ``shadow space'', \RU{которое мы будем рассматривать позже}\EN{about which we will talk later}: \myref{shadow_space}.
