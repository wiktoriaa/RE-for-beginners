...
|L0.16|
        CMP      r0,`#'0        ; str_len==0?
        ADDNE    r2,r4,r0     ; (_EN(`if')_RU(`если') str_len!=0) R2=R4+R0=s+str_len
        LDRBNE   r1,[r2,`#'-1]  ; (_EN(`if')_RU(`если') str_len!=0) R1=_EN(`load byte at address')_RU(`загрузить байт по адресу') R2-1=s+str_len-1
        CMPNE    r1,`#'0        ; (_EN(`if')_RU(`если') str_len!=0) _EN(`compare loaded byte against')_RU(`сравнить загруженный байт с') 0
        ...
        SUBEQ    r0,r0,`#'1     ; (_EN(`if loaded byte is')_RU(`если загруженный байт это') '\r' _EN(`or')_RU(`или') '\n') R0-- _EN(`or')_RU(`или') str_len--
        STRBEQ   r3,[r2,`#'-1]  ; (_EN(`if loaded byte is')_RU(`если загруженный байт это') '\r' _EN(`or')_RU(`или') '\n') _EN(`store R3 (zero) at address')_RU(`записать R3 (ноль) по адресу') R2-1=s+str_len-1
        BEQ      |L0.16|      ; _EN(`jump to loop begin if loaded byte was')_RU(``перейти на начало цикла, если загруженный байт был'') '\r' _EN(`or')_RU(`или') '\n'
|L0.56|
