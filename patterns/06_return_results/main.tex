...
If it is byte type or a character (\Tchar)~---then the lowest part of register \EAX is used~---\AL. 
If a function returns a \Tfloat number, the FPU register 
\ST{0} is used instead.
...
\EN{So, what if returned value of the \main function was declared not as \Tint but as \Tvoid?}

\RU{Т.н. startup-код вызывает \main примерно так:}
\EN{The so-called startup-code is calls \main roughly as:}
...
(\RU{мы видели это прежде}\EN{we saw this before}: \myref{puts}), 
...
zeroed before} \main\EN{'s finish}.
...
\EN{It's possible to call a function whose essence is in returning a value, and not use it:}
...
register (usually \Tint), but if one needs it, one should return information via pointers passed 
...
\EN{Now it has become possible to return, let's say, a whole structure, but it still is not very popular. 
If a function has to return a large structure, the \gls{caller} must allocate it and pass a pointer to it via the first argument, 
...
\EN{The macro name for internal passing of pointer to a structure is \TT{\$T3853} here.}
...
\EN{As we see, the function is just filling fields in the structure allocated by
the caller function,
