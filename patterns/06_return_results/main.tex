...
\myindex{puts() \RU{вместо}\EN{instead of} printf()}
...
\EN{But in the first 3 cases, the value in \EAX is just not used.}
\RU{Но в первых трех случаях значение, лежащее в \EAX, просто не используется.}
...
\RU{\GTT{\$T3853} это имя внутреннего макроса для передачи указателя на структуру.}
\EN{The macro name for internal passing of pointer to a structure here is \GTT{\$T3853}.}
