include(`commons.m4'); int __cdecl f(int, int *, int *, int *)
...
loc_7F: ; CODE XREF: f(int,int *,int *,int *)+65
       mov     edi, eax        ; edi = ar1
       and     edi, 0Fh        ; _EN(`is ar1 16-byte aligned')_RU(`ar выровнен по 16-байтной границе')?
       jz      short loc_9A    ; _EN(`yes')_RU(`да')
       test    edi, 3
       jnz     loc_162
       neg     edi
       add     edi, 10h
...
loc_D6: ; CODE XREF: f(int,int *,int *,int *)+B2
       mov     esi, [esp+10h+ar2]
       lea     esi, [esi+edi*4] ; _EN(`is ')ar2+i*4 _EN(`16-byte aligned')_RU(`выровнен по 16-байтной границе')?
       test    esi, 0Fh
       jz      short loc_109   ; _EN(`yes')_RU(`да')!
       mov     ebx, [esp+10h+ar1]
       mov     esi, [esp+10h+ar2]
