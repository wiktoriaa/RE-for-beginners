...
qui prend un couple de tableaux en entrée et produit un tableau.
...
sa version \q{lite} le Cray Y-MP EL\footnote{Á distance. Il est installé dans le
...
de charger 4 éléments de A dans un registre XMM 128-bit, de B dans un autre registre
...
\newcommand{\URLINTELVEC}{\href{http://go.yurichev.com/17082}{Extrait: Vectorisation automatique éfficace}}
...
\begin{itemize}
\item
\MOVDQU (\IT{Move Unaligned Double Quadword})---charge juste 16 octets depuis la
mémoire dans un registre XMM.

\item
\PADDD (\IT{Add Packed Integers})---ajoute 4 paires de nombres 32-bit et laisse le
...
Donc, ces instructions SSE2 sont exécutées seulement dans le cas oú il y a plus
de 4 paires à traiter et que le pointeur \TT{ar3} est aligné sur une frontière de
16-octets.

Ainsi, si \TT{ar2} est également aligné sur une frontière de 16 octets, ce morceau
