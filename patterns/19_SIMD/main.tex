...
\label{SIMD_x86}
\ac{SIMD} \RU{это акроним:}\EN{is just acronym:} \IT{Single Instruction, Multiple Data}.
...
\EN{Just as \ac{FPU}, that \ac{CPU} subsystem looks like separate processor inside x86.}
...
\EN{Of course, memory copying (\TT{memcpy}), memory comparing (\TT{memcmp}) and so on.}
...
\RU{А иначе, значение из \TT{ar2} загрузится в \XMMZERO используя инструкцию \MOVDQU, 
которая не требует выровненного указателя, зато может работать чуть медленнее:}
\EN{Otherwise, value from \TT{ar2} will be loaded into \XMMZERO using \MOVDQU,
...
\RU{(пример базируется на исходнике \href{\URLSTRLEN}{отсюда}).}
\EN{(the example is based on source code from \href{\URLSTRLEN}{there}).}
...
\RU{Далее мы загружаем по 16 байт в регистр \XMMONE при помощи команды \MOVDQA.}
\EN{Then, load next 16 bytes into the \XMMONE register using \MOVDQA instruction.}
...
\verb|_mm_setzero_si128()|\EMDASH\RU{это макрос, генерирующий \TT{pxor xmm0, xmm0} ~--- инструкция просто обнуляет регистр \XMMZERO.}
\EN{is a macro generating \TT{pxor xmm0, xmm0}~---instruction just clears the \XMMZERO register}

\verb|_mm_load_si128()|\EMDASH\RU{это макрос для \MOVDQA, он просто загружает 16 байт по адресу из указателя в \XMMONE.}
\EN{is a macro for \MOVDQA, it just loading 16 bytes from the address in the \XMMONE register.}
...
\RU{После исполнения \TT{pcmpeqb xmm1, xmm0}, регистр \XMMONE будет содержать:}
\EN{After \TT{pcmpeqb xmm1, xmm0} execution, the \XMMONE register shall contain:}
...
иначе нулем.}
\EN{This instruction will set first \EAX bit into 1 if most significant bit of the first byte in the \XMMONE is $1$.
In other words, if first byte of the \XMMONE register is \TT{0xff}, first \EAX bit will be set to 1 too.}

\RU{Если второй байт в регистре \XMMONE является \TT{0xff}, то второй бит в \EAX также будет единицей. 
...
\EN{If second byte in the \XMMONE register is \TT{0xff}, then second \EAX bit will be set to 1 too.
...
\RU{На вход может прийти 16 байт вроде}\EN{There might be 16 bytes on input like} \TT{hello\textbackslash{}x00garbage\textbackslash{}x00ab}
...
\RU{Если мы загрузим эти 16 байт в \XMMONE и сравним с нулевым \XMMZERO, то в итоге получим такое 
(я использую здесь порядок с MSB\MSBFOOTNOTE до LSB\LSBFOOTNOTE):}
\EN{If we load these 16 bytes into \XMMONE and compare them with zeroed \XMMZERO, we will get something like
...
\EN{Obviously, our function must consider only first zero bit and ignore the rest ones.}
