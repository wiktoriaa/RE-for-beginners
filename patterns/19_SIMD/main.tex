...
\EN{is a macro for \MOVDQA, it just loading 16 bytes from the address in the \XMM{1} register.}
...
Иными словами, инструкция отвечает на вопрос, \IT{какие из байт в \XMM{1} являются \TT{0xff}?}
...
In other words, the instruction is answer to the question \IT{which bytes in the \XMM{1} are \TT{0xff?}}
