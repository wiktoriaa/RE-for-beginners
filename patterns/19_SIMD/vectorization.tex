...
supercomputer line from 1988 when he played with its ``lite'' version Cray Y-MP EL
...
it is possible to get 4 64-bit \glspl{product}s at once.}
...
By the way, no exception is raised in case of overflow and no flags will be set, just the low 32 bits of the result will
be stored.
If one of \PADDD's operands is the address of a value in memory,
then the address must be aligned on a 16-byte boundary. If it is not aligned, an exception will be triggered
...
\EN{So, these SSE2-instructions will be executed only in case there are more than 4 pairs to work on
...
\EN{Also, if \TT{ar2} is aligned on a 16-byte boundary as well, this fragment of code will be executed:}
...
\EN{Otherwise, the value from \TT{ar2} will be loaded into \XMM{0} using \MOVDQU,
