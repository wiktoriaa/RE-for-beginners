...
\PTBR{Agora vamos extender o nosso exemplo \IT{\HelloWorldSectionName}~(\myref{sec:helloworld}),
...
\RU{Вот примерный скелет вызова функции}\EN{Here is a rough skeleton of the function call}\PTBR{Aqui está uma estrutura bem rústica da chamada da função}:
...
\section{\RU{Кстати}\EN{By the way}\PTBR{A propósito}}
...
\PTBR{A propósito, a diferença entre os argumentos passados em x86, x64, fastcall, ARM e MIPS  é uma boa demonstração do fato de como a CPU é indiferente sobre como os argumentos são passados para as funções.
...
\EN{The \ac{CPU} is not aware of calling conventions whatsoever.}%
\RU{\ac{CPU} не знает о соглашениях о вызовах вообще.}%
\PTBR{A \ac{CPU} não está ciente de convenções de chamada de funções.}%
...
\PTBR{Agora nós podemos também relembrar de dos programadores novatos de assembly passando argumentos para outras funções:
