\subsection{x86: \RU{3 аргумента}\EN{3 arguments}\PTBR{3 argumentos}}

\subsubsection{MSVC}

\RU{Компилируем при помощи MSVC 2010 Express, и в итоге получим:}%
\EN{When we compile it with MSVC 2010 Express we get:}%
\PTBR{Quando compilamos esse código com o MSVC 2010 Express temos:}%
...
\PTBR{Quase a mesma coisa, mas agora nós podemos ver que os argumentos da função \printf são empurrados na pilha na ordem reversa. O primeiro argumento é empurrado por último.}%
...
\PTBR{A propósito, variáveis do tipo \Tint em ambientes 32-bits tem 32-bits de largura, isso é 4 bytes.}%
...
\PTBR{Então, nós temos quatro argumentos aqui, $4*4=16$ --- eles ocupam exatamente 16 bytes na pilha: um ponteiro de 32-bits para uma string e três números do tipo \Tint.}%

\index{x86!\Instructions!ADD}
\index{x86!\Registers!ESP}
\index{cdecl}
\RU{Когда при помощи инструкции \INS{ADD ESP, X} корректируется \glslink{stack pointer}{указатель стека} \ESP 
после вызова какой-либо функции, зачастую можно сделать вывод о том, сколько аргументов 
у вызываемой функции было, разделив X на 4.}%
\EN{When the \gls{stack pointer} (\ESP register) has changed back by the \INS{ADD ESP, X}
instruction after a function 
call, often, the number of function arguments could be deduced by simply dividing X by 4.}%
\PTBR{Quando o ponteiro da pilha (registrador \ESP) volta para seu valor anterior pela instrução \INS{ADD ESP, X} depois de uma chamada de função,
geralmente o número de argumentos pode ser obtido simplismente por se dividir X, o argumento da função \ADD, por 4.}%

\RU{Конечно, это относится только к cdecl-методу передачи аргументов через стек, и только для 32-битной среды.}%
\EN{Of course, this is specific to the \IT{cdecl} calling convention, and only for 32-bit environment.}%
\PTBR{Lógico que isso é por causa da convenção de chamada do \IT{cdecl} e somente para ambientes 32-bits.}%

\ifx\LITE\undefined
\RU{См. также в соответствующем разделе о способах передачи аргументов через стек ~(\myref{sec:callingconventions}).}%
\EN{See also the calling conventions section ~(\myref{sec:callingconventions}).}%
\PTBRph{}%
\fi

\RU{Иногда бывает так, что подряд идут несколько вызовов разных функций, но стек корректируется только один раз, после последнего вызова:}%
\EN{In certain cases where several functions return right after one another, the compiler could merge multiple \q{ADD ESP, X} instructions into one, after the last call:}%
\PTBR{Em certos casos onde varias funções retornam uma depois da outro, o compilador pode multiplicar as instruções \q{ADD ESP, X} em uma só, depois da última chamada:}%
...
\RU{Вот пример из реальной жизни:}\EN{Here is a real-world example:}\PTBR{Aqui está um exemplo do mundo real:}
...
\RU{с опцией}\EN{with} \GTT{/MD} \RU{что означает линковать с библиотекой}\EN{option, which means to link 
with} \GTT{MSVCR*.DLL},
...
\RU{Самая первая точка останова в}\EN{The very first breakpoint is in} \GTT{ntdll.dll}, \RU{нажмите}\EN{press} 
...
\RU{Кликните на инструкции}\EN{Click on the} \INS{PUSH EBP}\RU{, нажмите}\EN{ instruction, press} F2 
...
\INS{CALL printf}\EN{ instruction}.
...
\RU{Регистр }\EN{Register }\EAX \RU{теперь содержит}\EN{now contains} \GTT{0xD} (13).
...
\INS{ADD ESP, 10}\EN{ instruction}:

\begin{figure}[H]
\centering
\includegraphics[scale=\FigScale]{patterns/03_printf/x86/olly3_4.png}
\caption{\olly: \RU{после исполнения инструкции}\EN{after} \INS{ADD ESP, 10}\EN{ instruction execution}}
...
\GTT{-g} \RU{означает генерировать отладочную информацию в выходном исполняемом файле}\EN{option instructs the compiler to include debug information in the executable file}.
...
\RU{Самый первый элемент это}\EN{The very first element is the} \ac{RA} (\GTT{0x0804844a}).
...
\RU{Две инструкции \INS{XCHG} это холостые инструкции, аналогичные \ac{NOP}.}%
\EN{The two \INS{XCHG} instructions are idle instructions, analogous to \ac{NOP}s.}

\RU{Второй элемент (\GTT{0x080484f0}) это адрес строки формата:}%
\EN{The second element (\GTT{0x080484f0}) is the format string address:}
...
\GTT{1.c} в строке 6}\EN{and the information that this expression is in the \GTT{1.c} file at the line 6}.
\RU{Действительно, файл \GTT{1.c} лежит в текущем директории и \ac{GDB} находит там эту строку}
\EN{Indeed, the \GTT{1.c} file is located in the current directory, and \ac{GDB} finds the string there}.
...
\INS{MOV EAX, 0}\EN{ instruction execution}. \EN{Indeed}
