...
with} \TT{MSVCR*.DLL},
\RU{чтобы импортируемые ф-ции были хорошо видны в отладчике.}
...
\RU{Теперь мы должны найти ф-цию}\EN{Now we have to find the} \main\EN{ function}.
...
That is correct, since \printf returns the number of characters printed. The
\RU{Значение }\EIP \RU{изменилось: действительно, теперь здесь адрес инструкции после}
\EN{value has changed: indeed, now it contains the address of the instruction coming after} \TT{CALL printf}.
...
\RU{Это должна делать вызывающая ф-ция}\EN{The \gls{caller} is responsible to do so}.
...
\RU{Остальные 3 элемента}\EN{Next 3 elements} (1, 2, 3) \RU{это аргументы ф-ции}\EN{are the} 
\printf\EN{ arguments}.
...
\RU{Это значит, исполнять все инструкции до самого конца ф-ции}\EN{The command instructs GDB to ``execute all instructions until the end of the function''}. 
