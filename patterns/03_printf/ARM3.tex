...
the 4 first arguments are passed in the \Reg{0}-\Reg{3} registers and remaining arguments~---via stack}.
...
{So, the first 4 arguments are passing via \Reg{0}-\Reg{0} arguments in this order:
pointer to the \printf format string in the \Reg{0}, then $1$ in R1, $2$ in R2 and $3$ in R3}.
...
is also in the
fact that there are no function prologue and epilogue (instructions which are saving \TT{\Reg{0}} and \LR registers values)}.
