...
\EN{TARM's traditional scheme for passing arguments (calling convention) behaves as follows:
...
\RU{Функция \printf, отработав, просто вернет управление по адресу, записанному в \ac{LR}}\EN{On completion, the \printf
function will simply return the control to the address stored in \ac{LR}}.
...
\EN{then the control from \printf will be returned to that point}.
...
\RU{Эта оптимизация часто используется в ф-циях, где последнее выражение --- это вызов другой ф-ции.}
\EN{This optimization is often used in functions where the last statement is a call to another function.}

\RU{Еще один похожий пример описан в секции}\EN{A similar example will be presented in} 
``\SwitchCaseDefaultSectionName'' 
\RU{, здесь}\EN{section, in}~(\myref{jump_to_last_printf}).
