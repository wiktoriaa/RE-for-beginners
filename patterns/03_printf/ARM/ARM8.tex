...
\RU{Самая первая инструкция}\EN{The very first} \INS{STR LR, [SP,\#var\_4]!} 
...
\RU{Вторая инструкция}\EN{The second} \INS{SUB SP, SP, \#0x14}
\RU{уменьшает \glslink{stack pointer}{указатель стека} \ac{SP}, но, на самом деле, эта процедура нужна для выделения в локальном стеке места размером \GTT{0x14} (20) байт.}
\EN{instruction decreases
\ac{SP} (the \gls{stack pointer}) in order to 
allocate \GTT{0x14} (20) bytes on the stack.}
...
\RU{Затем инструкция}\EN{Then, the} \INS{ADD R12, SP, \#0x18+var\_14} 
\RU{записывает в регистр \Reg{12} адрес места в стеке, куда будут помещены эти 4 значения}%
\EN{instruction writes the stack address where these 4 variables are to be stored, into the \Reg{12} register}.
\myindex{IDA!var\_?}
...
\RU{Так что в \Reg{12} будет записано \GTT{SP+4}.}
\EN{So, \GTT{SP+4} is to be stored into the \Reg{12} register.}
\myindex{ARM!\Instructions!STMIA}
\RU{Следующая инструкция}\EN{The next} \INS{STMIA R12, {R0-R3}} 
\RU{записывает содержимое регистров \Reg{0}-\Reg{3} по адресу в памяти, на который указывает \Reg{12}.}
\EN{instruction
writes registers \Reg{0}-\Reg{3} contents to the memory pointed by \Reg{12}.}
\RU{Инструкция }\INS{STMIA} \RU{означает}\EN{abbreviates} \IT{Store Multiple Increment After}. 
\IT{\q{Increment After}} 
\RU{означает, что \Reg{12} будет увеличиваться на 4 после записи каждого значения регистра.}
\EN{implies that \Reg{12} is to be increased by 4 after each register value is written.}
...
this value, with the help of the} \INS{STR R0, [SP,\#0x18+var\_18]} \RU{записывает его в стек}\EN{instruction is saved
on the stack}.
...
\RU{Инструкция}\EN{The} \INS{ADD SP, SP, \#0x14} \RU{возвращает \ac{SP} на прежнее место, 
...
\RU{Инструкция}\EN{The} \INS{LDR PC, [SP+4+var\_4],\#4} \RU{загружает в \ac{PC} 
...
analogous to} \INS{LDR PC, [SP],\#4}), \RU{затем}\EN{and then} \ac{SP} \RU{увеличивается 
...
\RU{Потому что она хочет показать разметку стека и тот факт, что}\EN{Because it wants to illustrate the stack layout and the fact that} \GTT{var\_4} \RU{выделена в локальном стеке именно для сохраненного
...
similar to} \INS{POP PC} \InENRU x86\footnote{\EN{It is impossible to
set \GTT{IP/EIP/RIP} value using \POP in x86, but anyway, you got the analogy right}\RU{В x86 
невозможно установить значение \GTT{IP/EIP/RIP} используя \POP, но будем надеятся, вы поняли аналогию}.}.
...
exception of} \INS{STMFA} (Store Multiple Full Ascending)%
\RU{~--- это синоним инструкции }\EN{ instruction, which is a synonym of}%
\INS{STMIB} (Store Multiple Increment Before)\EN{ instruction}. 
...
\RU{Например, значение в регистре \Reg{0} подготавливается в трех местах, по адресам \GTT{0x2918}, \GTT{0x2920} 
и \GTT{0x2928}, 
...
\RU{К примеру, инструкции}\EN{For example, instructions like} \INS{MOVT R0, \#0} \AndENRU 
\INS{ADD R0, PC, R0} \RU{не могут быть исполнены одновременно, потому что обе инструкции модифицируют 
регистр \Reg{0}}\EN{cannot be executed simultaneously since they both modify the \Reg{0} register}. 
\RU{А вот инструкции}\EN{On the other hand,} \INS{MOVT R0, \#0} \AndENRU \INS{MOV R2, \#4} 
