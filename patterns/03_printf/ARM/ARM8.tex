...
\EN{This implies that \ac{SP} will be decreased by 4 first, and then \ac{LR} will be saved at the address stored in \ac{SP}.}
...
\RU{Остальные 4 32-битных значения будут переданы через регистры}\EN{The other 4 32-bit values will be passed through registers}.
...
\EN{instruction writes the stack address where these 4 variables will be stored, into the \TT{R12} register}.
\index{IDA!var\_?}
...
\RU{Так что в \TT{R12} будет записано $SP+4$}\EN{So, $SP+4$ will be stored into the \TT{R12} register}.
...
\EN{implies that \TT{R12} will be increased by $4$ after each register value is written}.
...
\EN{thus the value from the \Reg{0} register ($4$) will be written to the address written in \ac{SP}}.
...
стек немного иначе: в начале $8$ за первый раз, затем $5$, $6$, $7$ за второй раз и $4$ за третий раз}\EN{The output is almost 
like in
