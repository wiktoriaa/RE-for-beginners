...
\EN{This means that first \ac{SP} will be decreased by 4, 
then \ac{LR} will be written at the address stored in \ac{SP}.}
...
\EN{they are written in the \Reg{0}, \Reg{1}, \Reg{2} and \Reg{3} registers respectively}.
...
\EN{instruction writes the address in the stack where these 4 variables will be written, into the \TT{R12} register}.
\index{IDA!var\_?}
...
\EN{which is created by \IDA to better show code accessing the stack}.
\RU{Макросы \IT{var\_?}, создаваемые \IDA, отражают локальные переменные в стеке}\EN{The \IT{var\_?} macros created
...
this value, with the help of the} \TT{``STR R0, [SP,\#0x18+var\_18]''} \RU{попадает в стек}\EN{instruction is saved
on the stack}.
...
\EN{so the value from the \Reg{0} register ($4$) will be written to the addrss written in \ac{SP}}.
...
\EN{There is no exclamation mark---indeed, first \ac{PC} is loaded from the address written in \ac{SP} }
\RU{Здесь нет восклицательного знака --- действительно,  в начале \ac{PC} загружается из места,
...
\RU{Почему}\EN{Why does} \IDA \RU{показывает инструкцию именно так}\EN{show the instruction like that}?
\RU{Потому что она хочет показать разметку стека и тот факт, что}\EN{Because it wants to show 
the stack layout and the fact that} \TT{var\_4} \RU{выделена в локальном стеке именно для сохраненного
значения}\EN{is allocated for saving the value of} \ac{LR}\EN{ in the local stack}.
...
set value of \TT{IP/EIP/RIP} using \POP in x86, but anyway, you get the idea, I hope}\RU{В x86 
...
we easily spot is that the instructions seem to be located randomly}.
\RU{Например, значение в регистре \Reg{0} подготавливается в трех местах, по адресам \TT{0x2918}, \TT{0x2920} 
и \TT{0x2928}, 
когда это можно было бы сделать в одном месте}\EN{For example, the value in the \Reg{0} register is prepared in three
...
\EN{Presumably, the compiler tries to generate code in such a way (where it is possible)}.
...
части регистров (W-).}
\EN{All other values have a \Tint 32-bit type, so they are written in the 32-bit part of 
the registers (W-).}
