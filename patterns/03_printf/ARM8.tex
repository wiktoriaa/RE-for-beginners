\section{ARM: \IFRU{8 аргументов}{8 arguments}}
...
\IFRU{сохраняет в стеке \ac{LR}, ведь нам придется использовать этот регистр для вызова \printf}
{instruction saves \ac{LR} on the stack, because we will use this register for the \printf call}.

\index{ARM!\Instructions!SUB}
\IFRU{Вторая инструкция}{The second} \TT{``SUB SP, SP, \#0x14''}
\IFRU{уменьшает \glslink{stack pointer}{указатель стека} \ac{SP}, но, на самом деле, эта процедура нужна для выделения в локальном стеке места размером \TT{0x14} ($20$) байт}
...
\IFRU{так что, значение из регистра \Reg{0} ($4$) запишется туда, куда указывает \ac{SP}}
{so, the value from the \Reg{0} register ($4$) will be written to the point where \ac{SP} is pointing to}.
...
\IFRU{Инструкция}{The} \TT{``ADD SP, SP, \#0x14''} \IFRU{возвращает \ac{SP} на прежнее место, 
аннулируя таким образом всё, что было записано в стеке}
{instruction returns the \ac{SP} pointer back to its former point,
...
\IFRU{Инструкция}{The} \TT{``LDR PC, [SP+4+var\_4],\#4''} \IFRU{загружает в \ac{PC} сохраненное значение \ac{LR} из стека,
обеспечивая таким образом выход из функции}
{instruction loads the saved \ac{LR} value from the stack into the \ac{PC} register, thus causing
...
\IFRU{Эта инструкция увеличивает \ac{SP} и только затем записывает в память значение очередного регистра, 
но не наоборот}{This
