...
% It might be useful to mention, to those who are not so familiar with ARM,
% that the `!' automatically changes the stack pointer afterwards.
% To somebody who doesn't know or forgot that fact, it might look like
% LR will be overwritten later:
% the stack pointer would be subtracted only by 0x14 (and not 0x18 total),
% and then -0x14 + 0x4 = -0x10 is used later,
% which only leaves space for the last 4 parameters, but not the LR.
% But actually, the 0x4 is already canceled because of the `!'.
