...
\EN{\scanf returns the result of its work in register \$V0, and it's checked at address 0x004006E4
by comparing the value in \$V0 with \$V1 (1 was stored in \$V1 earlier, at 0x004006DC).}
BEQ \EN{mean}\RU{означает} ``Branch Equal''\RU{ (переход если равно)}.
\RU{Если значения равны (т.е., в случае успеха), произойдет переход по адресу 0x0040070C.}
\EN{If the values are equal (i.e., success), the execution will jump to address 0x0040070C.}
