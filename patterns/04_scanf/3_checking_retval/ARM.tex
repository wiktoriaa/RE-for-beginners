...
\EN{is analogous to the x86 instruction with the same name, it subtracts one argument from the other and saves flags.}
...
if the result of the last computation was $0$, or if Z flag is $1$.}
...
where $0$ is written into the \Reg{0} as a value returned from the function, and then the function ends.}
...
\EN{Code flow here forks using the CMP/BNE (Branch if Not Equal) instruction pair.}
