...
\RU{Действительно, ведь что туда записала бы ф-ция \scanf}\EN{Indeed, what would \scanf write there}?
\RU{Она просто ничего не делала кроме возвращения нуля}\EN{It simply did nothing except returning zero}.
...
из стека}\EN{We now have 1 in \EAX, so the following check will execute as intended, and \printf will print
the value of the variable in the stack}.

\RU{Запускаем}\EN{When we run the program} (F9) \RU{и видим в консоли следующее}\EN{we will see the following in the console window}:
