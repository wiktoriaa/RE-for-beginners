...
\EN{So the simplest thing to do is to pass an address of array or structure to \gls{callee} function,
and let \gls{callee} change what must be changed.}
...
on x64-architecture require twice as more space.}
...
Data types is not even important to know, only block size matters.}
...
(\RU{мы еще вернемся к этому в будущем}\EN{we will back to this further}~(\ref{label_pointers})).
