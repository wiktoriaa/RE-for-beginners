...
\RU{GP указыает на середину буфера, так что такие смещения могут нам подсказать что адреса всех трех ф-ций,
...
\RU{Немного изменим наш пример и сделам чтобы у $x$ было значение по умолчанию:}
\EN{Let's alter our example to make $x$ variable some default value:}

\lstinputlisting{patterns/04_scanf/2_global/MIPS/default_value_\LANG.c}
