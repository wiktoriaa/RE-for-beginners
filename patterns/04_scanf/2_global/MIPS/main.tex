...
\EN{IDA displays the $x$ variable in the .sbss ELF section (remember the ``Global Pointer''? \myref{MIPS_GP}),
...
\RU{Еще нужно отметить что функция заканчивается двумя \ac{NOP}-ами (\TT{MOVE \$AT,\$AT} --- 
...
\EN{In our case those are LUI (``Load Upper Immediate'') and ADDIU (``Add Immediate Unsigned Word'').}
...
the \$S0 register, and it is possible to encode the offset in a LW (``Load Word'') instruction, so one single LW is enough 
...
содержимое которых должно быть сохранено в других функциях (т.е., ``saved'').}
\EN{Registers holding temporary data are prefixed with T-, but here we also see some prefixed with S-, 
the contents of which is need to be preserved before use in other functions (i.e., ``saved'').}
% FIXME:
...
\RU{Ф-ция \scanf не изменяет это значение.}\EN{The \scanf function does not change its value.}
