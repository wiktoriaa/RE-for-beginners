...
а именно сегменте данных}\EN{So, the \TT{x} variable is now global and somehow 
...
\EN{Because it is a variable, and by definition it can be changed. And it's possible that it will change 
very often.}
\index{\RAM}
\index{\ROM}
...
\EN{The code segment can sometimes be located in a ROM chip (remember, we now deal
...
\EN{Furthermore, constant variables in RAM must be initialized, 
since after powering on, the RAM, obviously, contains random information.}
...
\EN{Onwards, we see a pointer to the \TT{x} (\TT{off\_2C}) variable in the code segment, and all
operations with the variable occurr via this pointer.}
...
\EN{This is because the \TT{x} variable can be located somewhere far from this code fragment, so its address
...
\EN{The \TT{LDR} instruction in thumb mode can only address variables in range of 1020 bytes its location, }
...
\EN{So, address of the \TT{x} variable
must be located somewhere in close proximity, because there is no guarantee that the linker will able to place
this variable near the code, it could be even in an external memory chip!}
...
с сегментом кода.}
\EN{Perhaps, thereafter, the linker will able to place this segment in ROM too, along with the code segment.}
...
(строки 21 и 25).}
\EN{Now the $x$ variable is declared as global and its address is calculated using 
