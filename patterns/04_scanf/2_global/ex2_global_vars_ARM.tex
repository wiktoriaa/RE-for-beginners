...
\EN{Segment of code can sometimes be located in microcontroller ROM (remember, we now deal
...
\EN{Furthermore, constant variables in RAM must be initialized before, 
since after RAM turning on, it, obviously, contain random information.}
...
\EN{Onwards, we see a pointer to the \TT{x} (\TT{off\_2C}) variable in code segment, and all
...
\EN{Same instruction in ARM-mode~---variables in range of $\pm{}4095$ bytes.}
\RU{Таким образом,
...
\EN{This, address of the \TT{x} variable
...
\lstinputlisting[caption=\NonOptimizing GCC 4.9.1 ARM64,numbers=left]{patterns/04_scanf/2_global/ARM64_GCC491_O0.s}

\index{ARM!\Instructions!ADRP/ADD pair}
\RU{Теперь $x$ это глобальная переменная, и её адрес вычисляется при помощи пары инструкций ADRP/ADD 
(строки 21 и 25).}
\EN{Now the $x$ variable is declared as global variable and its address calculated using 
ADRP/ADD instructions pair (lines 21 and 25).}
