...
\EN{A pointer
to an \Tint-typed variable must be passed to \scanf so it can return value via it.}
...
has named it \IT{var\_8}, however, it is not necessary to allocate it since \ac{SP} (\gls{stack pointer})
is already pointing to the space and it can be used directly.}
...
по этому адресу.}
\EN{The address is passed to \scanf, which just stores value the user entered in the memory at this address.}
