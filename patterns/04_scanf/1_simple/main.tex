...
\RU{Согласен, использовать \scanf в наши времена для того, чтобы спросить у пользователя что-то~--- 
...
\EN{In addition if the callee function needs to modify something in the large array or structure received as a parameter and return back the entire structure then the situation is close to absurd.}
...
\RU{Указатель в}\EN{A pointer in} \CCpp\RU{~--- это просто адрес какого-либо места в памяти.}%
...
\EN{By the way, that is the reason behind some people's indignation related to switching to x86-64~---all pointers
in the x64-architecture require twice as much space.}
...
\RU{Функция }\IT{scanf()}\RU{~--- это как раз такой случай}\EN{ is such a case}. 
