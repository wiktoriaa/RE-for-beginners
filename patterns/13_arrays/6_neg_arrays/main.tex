...
\RU{Элементы массивов в \CCpp индексируются начиная с 0, но в некоторы \ac{PL}, первый элемент индексируется через
1.}
\EN{This is at least}\RU{Это как минимум} FORTRAN.
\EN{Programmers may have this habit, so using this little trick, it's possible to address first element
