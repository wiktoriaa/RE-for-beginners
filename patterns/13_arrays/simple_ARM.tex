...
\lstinputlisting{patterns/13_arrays/simple_Keil_ARM_O0_\IFRU{ru}{en}.asm}
...
{instruction in function epilogue allocates exactly this amount of space in local stack.}

\IFRU{И в первом и во втором цикле, итератор цикла $i$ будет постоянно находится в регистре \Reg{4}.}
{In both first and second loops, $i$ loop iterator will be placed in the \Reg{4} register.}
...
\TT{``STR R0, [SP,R4,LSL\#2]''} \IFRU{записывает содержимое \Reg{0} в массив}{writes \Reg{0} contents into array}.
\IFRU{Указатель на элемент массива вычисляется так: \SP указывает на начало массива, \Reg{4} это $i$.}
{Here is how a pointer to array element is to be calculated: \SP pointing to array begin, \Reg{4} is $i$.}
...
\lstinputlisting{patterns/13_arrays/simple_Keil_thumb_O3_\IFRU{ru}{en}.asm}
