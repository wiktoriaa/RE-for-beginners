...
\EN{We will work with an array of type \Tchar, which implies that each element requires only one 
byte in memory.}
...
\RU{Заполняем вторую строку значениями}\EN{Let's fill the second row with these values:} $0 \ldots 3$:
...
\RU{Я обвел красным все три строки}\EN{I have marked all three rows with red}. 
...
\RU{Здесь я также обвел красным три строки}\EN{I have also marked the three rows in red here}. 
