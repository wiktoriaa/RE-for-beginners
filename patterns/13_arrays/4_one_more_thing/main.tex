...
GCC может это сделать выделяя место под массив динамически в стеке (как alloca()~(\myref{alloca}))}}
...
GCC actually does this by allocating an array dynamically on the stack (like alloca()~(\myref{alloca}))}}:
...
но внутри это очень похоже на alloca()~(\myref{alloca})}
...
but it looks like alloca()~(\myref{alloca}) internally.}
