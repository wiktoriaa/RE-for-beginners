...
\dots since \IT{month[3]} expression has a \IT{const char*} type.
And then, 4th character is taken from that expression by adding 5 bytes to its address.

By the way, arguments list passed to \IT{main()} function has the same data type:
