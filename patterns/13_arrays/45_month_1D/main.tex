...
значениями.}
\EN{The reason for the sign extension is that this 32-bit value will be used in calculations with
with other 64-bit values.}
\RU{Таким образом, оно должно быть расширено до 64-битного}
\EN{Hence, it should be promoted to 64-bit}
...
(negative array indices will be explained later: \myref{negative_array_indices}). 
And if this happens, the negative input \Tint value will be sign-extended correctly 
...
\EN{And that's why to pick a specific element, $month*8$ bytes should be skipped from the start.}
...
\EN{For 1, an element would be a pointer to a string that contains ``February'', etc.}
...
\EN{``0+'' was left in the listing because GCC assembler output is not tidy enough to eliminate it}.
...
sign-extends it (this is what ``sxtw'' suffix means) and adds to X0.}
...
случайным образом.}
\EN{But I should to note that there might be really \IT{anything} that linker has decided to put by chance.}

\RU{Так что будет если 12 будет передано в ф-цию?}\EN{So what if 12 is passed to the function?}
...
\EN{Let's see how the CPU will treat the bytes there as a 64-bit value:}
...
\EN{Most likely it will crash, because this value does't look like a valid address.}
...
\EN{There exists the philosophy that says ``fail early and fail loudly'' or ``fail-fast'', 
...
\EN{The assertion macro will check for valid values at every function start and fail if the expression is false.}
...
name to another function which will show this information to the user.}
...
\EN{Microsoft \gls{Windows NT} kernels come in ``checked'' and ``free'' builds}
...
\EN{The first has validation checks (hence, ``checked''), the second one doesn't (hence, ``free'' of checks).}
