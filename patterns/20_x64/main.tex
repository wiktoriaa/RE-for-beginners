...
\EN{In Win64, the function calling convention is slightly different, somewhat resembling fastcall~(\ref{fastcall}).
...
\EN{See also the section on calling conventions}~(\ref{sec:callingconventions}).
...
For us this means that the emitted code will contain less local variables.}
...
\EN{There are a lot of local variables. Of course, not all those will be in the local stack.
...
\EN{How float point numbers are processed in x86-64 is explained here: \ref{floating_SIMD}.}
