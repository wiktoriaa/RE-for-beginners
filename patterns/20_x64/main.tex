...
\EN{From the reverse engineer's perspective, most important differences are:}
...
\EN{SIMD-registers number are doubled: from 8 to 16:} \TT{XMM0-XMM15}.
...
\item
\RU{Сишный \Tint остается 32-битным для совместимости.}
\EN{C \Tint type is still 32-bit for compatibility.}
...
\EN{Since now registers number are doubled, compilers has more space now for maneuvering calling 
...
\RU{64-битные инструкции в ARM появились в}\EN{In ARM, 64-bit instructions are appeared in} ARMv8.
