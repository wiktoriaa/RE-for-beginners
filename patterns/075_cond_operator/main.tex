\chapter{\RU{Условный оператор}\EN{Conditional operator}}
\label{chap:cond}
% FIXME: should be in 07_jcc folder...
...
\lstinputlisting{patterns/075_cond_operator/cond.c}
...
\lstinputlisting[caption=\NonOptimizing MSVC 2008]{patterns/075_cond_operator/MSVC2008.asm}

\lstinputlisting[caption=\Optimizing MSVC 2008]{patterns/075_cond_operator/MSVC2008_Ox.asm}

\RU{Более новые компиляторы могут быть более краткими}\EN{Latest compilers may be more concise}:

\lstinputlisting[caption=\Optimizing MSVC 2012 x64]{patterns/075_cond_operator/MSVC2012_Ox_x64.asm}
...
\lstinputlisting[label=cond_Keil_ARM_O3,caption=\OptimizingKeilVI (\ARMMode)]{patterns/075_cond_operator/Keil_ARM_O3.s}
...
\lstinputlisting[caption=\OptimizingKeilVI (\ThumbMode)]{patterns/075_cond_operator/Keil_thumb_O3.s}

\section{ARM64}

\Optimizing GCC (Linaro) 4.9 \ForENRU ARM64 \RU{также использует условные переходы}\EN{also use conditional jumps}:

\lstinputlisting[label=cond_ARM64,caption=\Optimizing GCC (Linaro) 4.9]{patterns/075_cond_operator/ARM64_GCC_O3.s}
...
\section{\RU{Перепишем используя обычный \TT{if/else}}\EN{Let's rewrite it in \TT{if/else} way}}

\lstinputlisting{patterns/075_cond_operator/cond2.c}
...
\lstinputlisting[caption=\Optimizing GCC 4.8]{patterns/075_cond_operator/cond2_GCC_O3.s}
