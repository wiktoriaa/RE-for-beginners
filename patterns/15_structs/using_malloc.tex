\section{\RU{Выделяем место для структуры через malloc()}
\EN{Let's allocate space for structure using malloc()}}
...
\lstinputlisting{patterns/15_structs/systemtime_malloc.c}
...
\lstinputlisting[caption=\Optimizing MSVC]{patterns/15_structs/systemtime_malloc.asm}
...
с регистрами.}
\EN{New instruction~---\MOVZX (\IT{Move with Zero eXtent}).
...
\RU{В этом примере я тоже могу представить структуру как массив WORD-ов}\EN{In this example, I can represent
structure as array of WORD-s}:

\lstinputlisting{patterns/15_structs/systemtime_malloc2.c}

\RU{Получим такое}\EN{We got}:

\lstinputlisting[caption=\Optimizing MSVC]{patterns/15_structs/systemtime_malloc2.asm}
...
\EN{And again I should note, one should not do this in practice}.
