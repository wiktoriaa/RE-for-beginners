...
\EN{Passing structure as function argument (instead of passing pointer to structure) is the same
as passing all structure fields one by one.}
\RU{Передача структуры как аргумент ф-ции (вместо передачи указателя на структуру) это то же
что и передача всех полей структуры по одному.}
\EN{If structure fields packed by default, f() function can be rewritten as:}
\RU{Если поля в структуре пакуются по умолчанию, то ф-цию f() можно переписать так:}
...
\RU{И в итоге будет такой же код}\EN{And that will result in the same code}.
