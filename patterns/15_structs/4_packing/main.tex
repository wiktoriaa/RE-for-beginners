...
\RU{Кстати, подобная реализация копирования при помощи \MOV взамен вызова ф-ции memcpy(), например, это
очень распространенная практика, потому что это в любом случае работает быстрее чем вызов memcpy() ---
если речь идет о коротких блоках, конечно.}
\EN{By the way, such copy implementation using \MOV instead of memcpy() function calling is widely
used, because it's faster then to call memcpy()---if to talk about short blocks, of course.}
...
\figname \ref{fig:packing_olly_4}.
...
\figname \ref{fig:packing_olly_1}.
