...
{patterns/15_structs/4_packing/packing.asm}
...
\lstinputlisting[caption=MSVC 2012 /GS- /Zp1,label=src:struct_packing_1]
{patterns/15_structs/4_packing/packing_msvc_Zp1.asm}
...
потому что значения для нее готовятся при помощи инструкции \TT{MOVSX}, которая загружает
из памяти байты а не слова}
\EN{This garbage influence \printf output in no way, because values for it is prepared
by \TT{MOVSX} instruction, which takes bytes, but not words}: 
