...
\lstinputlisting[caption=\Optimizing MSVC,style=customasm]{patterns/15_structs/2_using_malloc/systemtime_malloc.asm}
...
\lstinputlisting[caption=\Optimizing MSVC,style=customasm]{patterns/15_structs/2_using_malloc/systemtime_malloc2.asm}

\RU{И снова мы получаем идентичный код, неотличимый от предыдущего.}
\EN{Again, we got the code that cannot be distinguished from the previous one.}
