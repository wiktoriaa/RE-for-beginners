\subsubsection{\RU{Структура как набор переменных}\EN{Structure as a set of values}}
...
\EN{Usually, non-optimizing compiler allocated variables in local stack in the 
same order as they were declared in function.}
...
\EN{Indeed, compiler is not aware these will be filled when calling to} \TT{localtime\_r()}.
...
со структурой \TT{SYSTEMTIME}}\EN{This will not work if structure fields have 16-bit size (\TT{WORD}), 
as in case of \TT{SYSTEMTIME} structure}\EMDASH{}\TT{GetSystemTime()} 
\RU{заполнит их неверно}\EN{will fill them incorrectly} 
(\RU{потому что локальные переменные будут выровнены по 32-битной границе}\EN{because local 
variables will be aligned on 32-bit border}).
\RU{Читайте об этом в следующей секции}\EN{Read more about it in next section}: 
``\StructurePackingSectionName'' (\ref{structure_packing}).
...
\subsubsection{\RU{Структура как массив 32-битных слов}\EN{Structure as an array of 32-bit words}}

\lstinputlisting{patterns/15_structs/3_tm_linux/GCC_tm3.c}

\RU{Я просто привожу (cast) указатель на структуру к массиву \Tint{}-ов}\EN{I just cast pointer to structure 
to array of \Tint{}'s}.
\RU{И это работает}\EN{And that works}!
\RU{Я запустил пример}\EN{I run example at} 23:51:45 26-July-2014.
...
\RU{Переменные здесь в том же порядке, в котором они перечислены в определении структуры}\EN{Variables here 
are just in the same order as they are enumerated in structure definition}: \ref{struct_tm}.

\RU{Вот как это компилируется}\EN{Here is how it was compiled}:

\lstinputlisting[caption=\Optimizing GCC 4.8.1]{patterns/15_structs/3_tm_linux/GCC_tm3.lst}

\RU{И действительно: место в локальном стеке в начале используется как структура, затем как массив.}
\EN{Indeed: the space in local stack is first treated as structure, then it's treated as array.}

\RU{Возможно даже модифицировать поля структуры через указатель.}
\EN{It's even possible to modify structure fields through this pointer.}

\RU{И снова, это сомнительный хакерский способ, который не рекомендуется использовать в настоящем коде.}
\EN{And again, it's dubious hackish way to do things, which is not recommended to use in production code.}
...
\EN{As an exercise, try to modify (increase by 1) current month number treating structure as 
array.}

\subsubsection{\RU{Структура как массив байт}\EN{Structure as an array of bytes}}

\RU{Я могу даже больше. Можно привести (cast) указатель к массиву байт и вывести его:}
\EN{I can do even more. Let's cast the pointer to array of bytes and dump it:}

\lstinputlisting{patterns/15_structs/3_tm_linux/GCC_tm4.c}
...
\RU{Я также запустил этот пример}\EN{I run this example also at} 23:51:45 26-July-2014.
\RU{Переменные точно такие же как и в предыдущем выводе}\EN{The values are just the same as in previous dump} 
(\ref{GCC_tm3_output}), \RU{и конечно, младший байт идет в самом начале, потому что это архитектура 
little-endian}\EN{and of course, lower byte goes first, because this is little-endian architecture} 
(\ref{sec:endianness}).

\lstinputlisting[caption=\Optimizing GCC 4.8.1]{patterns/15_structs/3_tm_linux/GCC_tm4.lst}
