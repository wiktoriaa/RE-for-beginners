...
\EN{First, let's see the binary form of the constants we will be using}:
...
That's why the \TT{``or ah, 40h''} instruction occupies only 3 bytes.
...
It is worth noting that both the function prologue (\TT{``push ebp / mov ebp,esp''}) and epilogue (\TT{``pop ebp''})
