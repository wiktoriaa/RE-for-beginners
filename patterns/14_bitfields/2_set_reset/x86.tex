...
\EN{The \OR instruction adds one more bit to value while ignoring the rest.}
...
\EN{\ANDIns resets one bit. It can be said that \ANDIns just copies all bits except one.
Indeed, in the second \ANDIns operand only the bits that need to be saved are set,
...
\clearpage
\ANDIns \RU{исполнилась}\EN{got executed}:
...
Поэтому, инструкция \TT{\q{or ah, 40h}} занимает только 3 байта. 
Было бы логичнее сгенерировать здесь \TT{\q{or eax, 04000h}}, но это уже 5 байт, или даже 6 
...
That's why the \TT{\q{or ah, 40h}} instruction occupies only 3 bytes.
It would be more logical way to emit here \TT{\q{or eax, 04000h}}
...
Интересно, что и пролог функции (\TT{\q{push ebp / mov ebp,esp}}) и эпилог (\TT{\q{pop ebp}}) 
...
It is worth noting that both the function prologue (\TT{\q{push ebp / mov ebp,esp}}) and epilogue (\TT{\q{pop ebp}})
