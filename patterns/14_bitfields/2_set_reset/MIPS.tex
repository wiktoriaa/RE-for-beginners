...
\RU{ORI это, конечно, операция \q{ИЛИ}, \q{I} в имени инструкции означает что значение встроено в машинный код.}
\EN{ORI is,of course, the OR operation. \q{I} in the instruction name mean that the value is embedded in the machine code.}
...
\EN{But after that we have AND. There was no way to use ANDI because it's not possible to embed the 0xFFFFFDFF number
in a single instruction, so the compiler has to load 0xFFFFFDFF into register \$V0 first and then generate
