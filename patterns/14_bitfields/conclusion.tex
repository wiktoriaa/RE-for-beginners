...
(которые называются ``data processing instructions'').}
\EN{It's also possible to add shifting suffix to some instructions 
(which are called ``data processing instructions'').}
...
\EN{ARM in ARM mode has BIC instruction, which works like NOT+AND instruction pair:}
