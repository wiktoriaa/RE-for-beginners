...
\EN{and that is value is used as the second argument for} \TT{CreateFile()}\footnote{\href{http://go.yurichev.com/17065}{MSDN: CreateFile function}} function.
...
\EN{Let's take a look into \TT{open()} function in the \TT{libc.so.6} library, but there is only syscall:}
...
\EN{Of course, it is easy to download both Glibc and Linux kernel source code, 
