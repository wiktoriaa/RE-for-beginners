...
; §EDX здесь это "новая версия rt", которая будет записана в переменную rt, если последний бит был 1§
	shr	rsi, cl		; RSI=RSI>>CL
	and	esi, 1		; ESI=ESI&1
; §последний бит был 1? Тогда записываем "новую версию rt" в EAX§
	cmovne	eax, edx
	add	rcx, 1		; RCX++
	cmp	rcx, 64
	jne	.L3
