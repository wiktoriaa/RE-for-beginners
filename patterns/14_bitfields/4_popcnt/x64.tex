...
\lstinputlisting{patterns/14_bitfields/4_popcnt/shifts64.c}
...
\lstinputlisting[caption=\Optimizing GCC 4.8.2]{patterns/14_bitfields/4_popcnt/shifts64_GCC_O3.s}
...
определнного бита с единицей, но здесь ``rt'' увеличивется на 1 до этого (строка 7), записывая новое значение
в регистр \EDX.}
\EN{While all examples we saw so far, incrementing ``rt'' value after comparing specific bit with one,
the code here incrementing ``rt'' before (line 7), writing new value into \EDX register.}
...
\RU{И это может работать быстрее на современных CPU с предсказателем переходов: здесь для предсказателей
меньше работы.}
\EN{And that might work faster on the modern CPUs with branch predictors: there are less work for predictors.}
\RU{Так что современные компиляторы могут иногда избавляться от условных переходов, если это возможно.}
\EN{So the modern compilers sometimes try to do get rid of conditional jumps, if possible.}
