...
\EN{\TT{O\_CREAT} bit is checked differently in Linux kernel 3.8.0.}
...
\EN{Here is how kernel compiled for ARM mode looks like in \IDA:}
...
\TT{TST} \RU{это аналог инструкции \TEST в x86.}\EN{is analogical to a \TEST instruction in x86.}

\RU{Мы можем ``узнать'' визуально этот фрагмент кода по тому что в одном случае исполнится 
функция \TT{lookup\_fast()}, а в другом \TT{complete\_walk()}.}
\EN{We can ``spot'' visually this code fragment by the fact the 
...
\EN{This is corresponding to the \TT{do\_last()} function source code.}

\RU{Макрос }\TT{O\_CREAT} \RU{здесь так же равен \TT{0x40}.}\EN{macro is equals to \TT{0x40} here too.}
