...
\section*{\RU{Разница между неоптимизирующей (debug) и оптимизирующей (release) версиями}\EN{The Difference between non-optimized (debug) and optimized (release) versions}}

\RU{Неоптимизирующий компилятор работает быстрее, генерирует более понятный (хотя и более объемный) код. Оптимизирующий (release) компилятор работает медленнее и старается сгенерировать более быстрый (хотя и не обязательно краткий) код.}
\EN{A non-optimizing (debug) compiler is faster and produces more understandable (albeit verbose) code, whereas an optimizing (release) compiler is slower and tries to produce code that runs faster (but is not necessarily more compact).}
...
\EN{One of the important features of the ´debug' code is that there might be debugging information which shows the connections between each line of the source code and the respective machine code addresses.}
\RU{Оптимизирующие компиляторы обычно генерируют код где целые строки из исходного кода могут быть соптимизированы и не присутствовать в итоговом машинном коде.}
\EN{Optimizing compilers, on the other hand, tend to produce output where entire lines of source code can be optimized away and thus not even be present in the resulting machine code.}

\RU{Практикующий reverse engineer обычно сталкивается с обоими версиями, потому что некоторые разработчики
включают оптимизацию, некоторые другие --- нет. Вот почему я стараюсь дать примеры для обоих версий.}
...
\EN{The} \ac{CPU} \RU{это собственно устройство исполняющее все программы}\EN{is the device which executes the machine code a program consists of}.
...
\EN{One might wonder why there needs to be a difference between machine code and a \ac{PL}. The answer lies in the fact that humans and \ac{CPU}s are not alike -}
...
без мелких ошибок}
\EN{In a similar fashion, it is very inconvenient for humans to write in assembly language due to it being so low-level and difficult to write in without making a huge amount of annoying mistakes}.
...
\ifx\LITE\undefined
\section{\RU{Несколько слов о разнице между ISA}\EN{A couple of words about different ISAs}
...
\EN{By the way, fixed-length instructions are handy due to that one can calculate the next (or previous) 
instruction's address without effort. This feature will be discussed in the switch()~(\myref{sec:SwitchARMLot}) section.}
}
\fi
.
\RU{Это то что сейчас называется ``режим ARM''}\EN{This is now referred to as ``ARM mode''}.
...
compete with ARM mode - a goal that was clearly achieved, as the majority of applications for \idevices are compiled for the Thumb-2 instruction set (admittedly, largely due to the fact that that Xcode does this by default) .}
\RU{По поводу богатства возможностей, Thumb-2 теперь может конкурировать с режимом ARM. Основное количество приложений для \idevices скомпилировано для набора инструкций Thumb-2, потому что Xcode
