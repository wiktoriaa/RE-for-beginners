...
\section{\RU{Несколько слов о x86 и ARM}\EN{Couple words about x86 and ARM}}
...
\RU{Так что в самом начале, инструкции ARM кодировались 4-мя байтами.}
\EN{So at very start, ARM has instructions all encoded in 4 bytes.}
...
\RU{На самом деле, самые используемые инструкции процессора на практике могут быть закодированы
используя меньшее количество информации.}
\EN{In fact, most used \ac{CPU} instructions in real world applications can be encoded using 
lesser ammount of information.}
...
\RU{Затем создатели ARM решили что Thumb можно расширить: так появился Thumb-2.}
\EN{Then ARM creators thought Thumb can be extended: Thumb-2 was appeared. }
...
\RU{Следовательно, в этой книге я стараюсь добавлять фрагменты кода на всех трех ARM \ac{ISA}, где это нужно.}
\EN{Therefore, I try to add fragments of code in all 3 ARM \ac{ISA}'s in this book, wherever it's needed.}
...
\input{patterns/075_cond_operator/main}
