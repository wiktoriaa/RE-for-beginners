...
\EN{Sometimes, I use ancient compilers, in order to get shortest (or simplest) possible
code snippet.}
...
\EN{When I learned assembly language, I also often compile small C-functions and then rewrite
...
лучше.}
\EN{This probably not worth doing today in real-world scenarios 
...
\EN{Therefore, you can try to take any assembly code from this book and try to make it shorter.}
...
\EN{Non-optimizing compiler work faster and produce more understandable (verbose, though) code.}
...
\EN{Optimizing (release) compiler work slower and try to produce faster (but not necessarily smaller) code.}
...
\EN{One important feature of debugging code is that there might be debugging information showing connections
between each line in source code and address in machine code.}
...
\EN{Optimizing compilers tend to produce such code where whole source code lines may be optimized away 
and not present in resulting machine code.}
...
\EN{When practicing reverse engineering you will usually encounter both versions, 
because some developers turn on optimization switches, some others do not.}

\RU{Вот почему я стараюсь дать примеры для обоих версий.}
\EN{That's why I try to give examples of both versions of code.}

\chapter{\RU{Краткое введение в CPU}\EN{Short introduction to the CPU}}
...
x64 extensions did not affect \ac{ISA} very much.}
\RU{В x86 до сих пор есть масса инструкций появившихся в 16-битном 8086 и присутствующих в самых последних
процессорах.}
\EN{x86 have a lot of instructions that appeared in 16-bit 8086 CPU and are still present in latest CPUs.}\\
\\
\index{ARM!\ARMMode}
\index{ARM!\ThumbMode}
...
\EN{So at the very start, ARM has all instructions encoded in 4 bytes}\footnote{\RU{Кстати, 
...
\EN{However, not all ARM instructions could be encoded in just 2 bytes, so Thumb instruction set is somewhat limited.}
...
\EN{Then ARM creators thought Thumb could be extended: Thumb-2 was appeared (in ARMv7). }
