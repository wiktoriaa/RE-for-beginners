...
\EN{A practicing reverse engineering will usually encounter both versions,
...
\EN{So at the very start, ARM had all instructions encoded in 4 bytes}\footnote{\RU{Кстати,
...
\EN{On instruction set richness, Thumb-2 can now compete with the original ARM mode.}
...
\EN{Then the 64-bit ARM came, this \ac{ISA} has 4-byte opcodes again, without any additional Thumb mode.}
...
вариации одного.}
\EN{These \ac{ISA}s intersect partially, but I would say that they are more different
...
\EN{There are more \ac{RISC} \ac{ISA}s with fixed length 32-bit opcodes, for example}
