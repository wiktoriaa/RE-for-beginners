\part{\IFRU{Образцы кода}{Code patterns}}
...
\IFRU{Здесь много примеров и для x86/x64 и для ARM}{There are a lot of examples for both x86/x64 
and ARM}. 
\IFRU{Те, кто уже хорошо знаком с одной из архитектур, могут легко пролистывать страницы}
{Those who already familiar with one of architectures, may freely skim over pages}.

\chapter{\IFRU{Краткое введенение в CPU}{Short introduction to the CPU}}

\EN{The} \ac{CPU} \IFRU{это собственно устройство исполняющее все программы}{is the unit which executes all of the programs}.
