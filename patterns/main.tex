...
Он делал это такое количество раз, что связь между кодом на \Cpp и тем, что генерирует компилятор, вбилась в его подсознание достаточно глубоко.
...
He did it so many times that the relationship between the \Cpp code and what the compiler produced was imprinted deeply in his mind. 
...
\RU{Человеку проще писать на \ac{PL} высокого уровня вроде \Cpp, Java, Python, 
а \ac{CPU} проще работать с абстракциями куда более низкого уровня}%
\EN{it is much easier for humans to use a high-level \ac{PL} like \Cpp, Java, Python, etc., 
