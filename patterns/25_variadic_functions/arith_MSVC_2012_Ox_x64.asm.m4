...
v$ = 8
arith_mean PROC
	mov	DWORD PTR [rsp+8], ecx   ; _RU(`первый аргумент')_EN(`1th argument')
	mov	QWORD PTR [rsp+16], rdx  ; _RU(`второй аргумент')_EN(`2nd argument')
	mov	QWORD PTR [rsp+24], r8   ; _RU(`третий аргумент')_EN(`3rd argument')
	mov	eax, ecx                 ; sum = _RU(`первый аргумент')_EN(`1st argument')
	lea	rcx, QWORD PTR v$[rsp+8] ; _RU(`указатель на второй аргумент')_EN(`pointer to the 2nd argument')
	mov	QWORD PTR [rsp+32], r9   ; _RU(`четвертый аргумент')_EN(`4th argument')
