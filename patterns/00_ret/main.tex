...
\RU{И вот что делает оптимизирующий GCC}\EN{And that's what the optimizing GCC compiler does}:
...
\EN{There are just two instructions: the first places the value 123 into the \EAX register, which is used for return
...
\EN{ARM uses \Reg{0} the register for returning results, so 123 is placed into \Reg{0} here.}
...
\EN{Assembly output done by GCC shows registers by number:}
...
\EN{So the \$2 (or \$V0) register is used for value returning.}
...
\EN{By why the load instruction (LI) and the jump instruction (J or JR) are swapped?}
\index{MIPS!Branch delay slot}
\RU{Это артефакт \ac{RISC} и называется он}
\EN{This is merely \ac{RISC} artifact and called} ``branch delay slot''.
\RU{На самом деле, нам не нужно вникать в эти детали}\EN{Actually, we don't need to get into it}.
\RU{Нужно просто запомнить: в MIPS инструкция после инструкции перехода исполняется \IT{перед} 
инструкцией перехода.}
\EN{We should just memorize: in MIPS, the instruction after jump or branch instruction
is executed \IT{before} the jump instruction.}
\RU{Таким образом, инструкция перехода всегда поменена местами с той, которая должна быть исполнена перед ней.}
\EN{Hence, jump instruction is always swapped with the one, which should be executed before.}
