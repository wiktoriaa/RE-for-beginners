...
\lstinputlisting[caption=MSVC 2010]{patterns/08_switch/2_lot/lot_msvc_EN.asm}
...
\TT{npad} (\myref{sec:npad}) is assembly language macro that aligning the next label so that it is to be stored at an address aligned on a 4 byte
(or 16 byte) boundary.
