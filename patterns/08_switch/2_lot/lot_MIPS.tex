...
\EN{There are new instruction to us is SLTIU (``Set on Less Than Immediate Unsigned'').}
...
\EN{In fact, this is the same as SLTU (``Set on Less Than Unsigned''), but ``I'' means ``immediate'', 
i.e., a number is to be specified in the instruction itself.}

\index{MIPS!\Instructions!BNEZ}
BNEZ \RU{это}\EN{is} ``Branch if Not Equal to Zero''\RU{ (переход если не равно нулю)}.

\RU{Код очень похож на код для других \ac{ISA}.}\EN{Code is very same as for other \ac{ISA}s.}
...
\EN{MIPS is 32-bit CPU after all, so all addresses in \IT{jumptable} are 32-bit ones.}
