...
\EN{This code makes use of the ARM mode feature in which all instructions has fixed size of 4 bytes.}
...
so that is why \ac{PC} pointing there. This fact should be memorized.}
...
$PC+8+a*4 = PC+8+1*4 = PC+12 = 0x184$\RU{, это адрес метки \IT{loc\_184}}\EN{ will be written to the \ac{PC},
...
\EN{It is too complex for describing it here now, so I will omit all elaborations.}
...
\EN{It is also worth noting the code is generated as a separate function in order to reuse it, 
so compiler will not generate the very same code for each switch() statement.}
