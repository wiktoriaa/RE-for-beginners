\chapter{\SwitchCaseDefaultSectionName}
\index{\CLanguageElements!switch}

\section{\IFRU{Если вариантов мало}{Few number of cases}}

\lstinputlisting{patterns/08_switch/8_1.c}

\input{patterns/08_switch/few_x86}

\input{patterns/08_switch/few_ARM}

\section{\IFRU{И если много}{A lot of cases}}

\IFRU{А если ветвлений слишком много, то конечно генерировать слишком длинный код с многочисленными \JE/\JNE 
уже не так удобно.}
{If \TT{switch()} statement contain a lot of case's, it is not very convenient for compiler to emit too large code
with a lot \JE/\JNE instructions.}

\lstinputlisting{patterns/08_switch/8_4.c}

\input{patterns/08_switch/lot_x86}

\input{patterns/08_switch/lot_ARM}

