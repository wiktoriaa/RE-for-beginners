...
\IFRU{Затем полученное}{Then,} $R0*4$ \IFRU{прибавляется к текущему значению \ac{PC}}{value we got, is added to
...
\IFRU{содержимое \ac{PC} на 8 байт больше}{value in the \ac{PC} is 8 bytes ahead} (\TT{0x180}) 
...
\IFRU{В случае, если $a=0$, тогда к \ac{PC} ничего не будет прибавлено, 
...
{If $a=0$, then nothing will be added to the value in the \ac{PC},
...
\IFRU{В случае, если}{In case of} $a=1$, \IFRU{тогда в \ac{PC} запишется}{then} 
$PC+8+a*4 = PC+8+1*4 = PC+16 = 0x184$\IFRU{, это адрес метки \IT{loc\_184}}{will be written to the \ac{PC},
this is the address of the \IT{loc\_184} label}.

\IFRU{При каждой добавленной к $a$ единице, итоговый \ac{PC} увеличивается на $4$.}
{With every $1$ added to $a$, resulting \ac{PC} increasing by $4$.}
...
\IFRU{Но можно отметить, что эта функция использует регистр \ac{LR} как указатель на таблицу.}
{But it is interesting to note the function uses \ac{LR} register as a pointer to the table.}
\IFRU{Действительно, после вызова этой функции, в \ac{LR} был записан
адрес после инструкции}{Indeed, after this function calling, \ac{LR} will contain address after} \\ 
