...
\EN{may also turn on the CF flag, which will be checked in the subsequent \TT{SBB} instruction}:
\RU{если флаг переноса включен, то от результата отнимется единица}\EN{if the carry flag is on, then 1 will also be subtracted
from the result}. \\
\\
...
с переносом).}
\EN{ARM has the ADC instruction as well (which counts carry flag) and SBC (``subtract with carry'').}
...
\EN{The -S suffix stands for ``set flags'', and flags (esp. carry flag) is what consequent ADC/SBC instructions
definitely need.}
...
\EN{To know if the carry flag would be set, a comparison (using ``SLTU'' instruction) also occurs, 
