...
\RU{Новые инструкции это}\EN{The new instructions here are}:
...
\EN{loads a 32-bit word into a register of the first coprocessor (hence ``1'' in instruction name).}
...
(``.D'' \RU{в суффиксе означает двойную точность}\EN{in the suffix stands for double precision}, 
``.S'' \RU{означают одинарную точность}\EN{would stands for single precision})
