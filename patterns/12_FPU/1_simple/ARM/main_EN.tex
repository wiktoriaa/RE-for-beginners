...
\lstinputlisting[label=ARM_leaf_example10,caption=\OptimizingXcodeIV (\ARMMode)]{patterns/12_FPU/1_simple/ARM/Xcode_ARM_O3_EN.asm}
...
\INS{VLDR} \AndENRU \INS{VMOV}, as it can be easily deduced, are analogous to the \INS{LDR} and \MOV instructions,
...
\INS{VMOV R0, R1, D16} is the inverse operation: what was in \GTT{D16} 
is split in two registers, \Reg{0} \AndENRU \Reg{1}, because a double-precision number 
...
\lstinputlisting{patterns/12_FPU/1_simple/ARM/Keil_O3_thumb_EN.asm}
...
\lstinputlisting[caption=\Optimizing GCC (Linaro) 4.9]{patterns/12_FPU/1_simple/ARM/ARM64_GCC_O3_EN.s}

\subsection{ARM64: \NonOptimizing GCC (Linaro) 4.9}

\lstinputlisting[caption=\NonOptimizing GCC (Linaro) 4.9]{patterns/12_FPU/1_simple/ARM/ARM64_GCC_O0_EN.s}
