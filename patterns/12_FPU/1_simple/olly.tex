...
\EN{In reality, the \ac{FPU} has just 8 registers and a pointer (called \GTT{TOP}) which contains a register number,
which is the current \q{top of stack}.}
\RU{При заталкивании значения в стек регистр \GTT{TOP} меняется, и указывает на свободный регистр. 
Затем значение записывается в этот регистр.}
\EN{When a value is pushed to the stack, \GTT{TOP} is pointed to the next available register,
...
\RU{Но в реальности, эта инструкция сохранила сумму и затем передвинула регистр \GTT{TOP}.}
\EN{But in fact, this instruction saved the sum and then shifted \GTT{TOP}.}
