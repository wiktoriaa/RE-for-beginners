...
\EN{I have marked red 2 pairs of 32-bit words in the stack. Each pair is a double-number in
IEEE 754 format and is passed from \main.}
...
(used internally in the FPU), here we see $1.999...$, which is close to $1.2$.}
...
\FDIV \RU{исполнилась}\EN{was executed}, \RU{теперь}\EN{now} \ST{0} \RU{содержит}\EN{contains} $0.382...$ 
(\gls{quotient}):
...
\EN{was executed, loading $3.4$ into \ST{0} (here we see the approximate value, $3.39999...$)}: 
...
instruction}: \FMUL. 
\RU{Она загружает константу}\EN{It loads the constant} $4.1$ \RU{из памяти, так что \olly тоже показывает её здесь}
...
\EN{\main will take this value from the register later.}\\
\\
\RU{Мы также видим кое-что необычное: значение $13.93...$ теперь находится в \ST{7}.}
\EN{We also see something unusual: the $13.93...$ value is now located in \ST{7}.}
\RU{Почему}\EN{Why}?

\label{FPU_is_rather_circular_buffer}
\RU{Я писал, что регистры в \ac{FPU} представляю собой стек}\EN{As I wrote before, the \ac{FPU} registers
are a stack}: \myref{FPU_is_stack}. 
