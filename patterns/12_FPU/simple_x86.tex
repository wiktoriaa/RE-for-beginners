...
\lstinputlisting[caption=MSVC 2010]{patterns/12_FPU/12_1_\IFRU{ru}{en}.asm}

\IFRU{\FLD берет 8 байт из стека и загружает их в регистр \STZERO, автоматически конвертируя во внутренний 
...
{\FDIV divides value in the \STZERO register by number storing at address 
...
{By the way, there is also \FDIVP instruction, which divides \STONE by \STZERO, 
...
{The function must return result in the \STZERO register, so,
...
\lstinputlisting[caption=\Optimizing GCC 4.4.1]{patterns/12_FPU/12_2_\IFRU{ru}{en}.asm}
