...
\lstinputlisting[caption=\Optimizing GCC 4.4.5 (IDA)]{patterns/12_FPU/3_comparison/MIPS_O3_IDA_EN.lst}
...
\GTT{T} mean that the jump is to be taken if the bit is set (\q{True}).
There is also the instruction \q{BC1F} which jumps if the bit is cleared (\q{False}).
