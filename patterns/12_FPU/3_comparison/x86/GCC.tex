...
but if one tries to do any operation with ``signaling'' NaNs, an exception will be raised.}
...
\EN{\FCOM will raise an exception if any operand is \gls{NaN}. 
\FUCOM will raise an exception only if any operand is a signaling \gls{NaN} (SNaN).}
...
\EN{Let's recall that \FNSTSW is moves the bits that interest us ~--- \CThreeBits ~--- into \AH 
...
\EN{If $a$ is greater than $b$ in our example, then \CThreeBits will be set to:} 0, 0, 0.
\item
\RU{Если $a$ меньше $b$, то биты будут выставлены так:}\EN{if $a$ is less than $b$, then the bits will be set to:} 0, 0, 1.
\item
...
\EN{In other words, after three \FUCOMPP/\FNSTSW/\SAHF instructions, we will have these states for the CPU flags:}

\begin{itemize}
\item
\RU{Если $a>b$ в нашем случае, то флаги будут выставлены так:}
\EN{If $a>b$, the CPU flags will be set as:} \TT{ZF=0, PF=0, CF=0}.
\item
\RU{Если $a<b$, то флаги будут выставлены так:}\EN{If $a<b$, then the flags will be set as:} \TT{ZF=0, PF=0, CF=1}.
\item
...
\EN{Depending on the CPU flags and conditions, \SETNBE will store store $1$ or $0$ to AL. 
...
\SETNBE stores $1$ only if \TT{CF=0} and \TT{ZF=0}. If it is not true, $0$ will be stored into \AL.}
...
\EN{Then $1$ will be stored in \AL, the subsequent \JZ will not be triggered and the function will 
