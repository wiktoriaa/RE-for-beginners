...
\RU{проверяющих биты в регистре статуса сопроцессора}\EN{which can check bits in coprocessor status register}, 
...
\EN{instruction for D-registers, to be executed if one operand is greater than other while comparing} 
...
\EN{$a$ value will stay in the \TT{D16} register.}
...
\EN{Penultimate instruction \TT{VMOV} will prepare value in the \TT{D16} register for returning it via \Reg{0} and \Reg{1}
registers pair.}
...
\EN{Almost the same as in previous example, however slightly different.}
\RU{Как мы уже знаем, многие инструкции в режиме ARM можно дополнять условием.}
\EN{As we already know, many instructions in ARM mode can be supplied by condition predicate.}
...
\EN{There is no place in 16-bit instructions for spare 4 bits in which condition can be encoded.}
...
\index{Angry Birds}
\RU{Еще чуть сложнее}\EN{Slightly harder}, 
\RU{и снова этот фрагмент из}\EN{and this fragment is also from} ``Angry Birds'':
...
\RU{А если бы здесь было, например,}\EN{And if there will be, for example,}
...
\EN{Compilers usually not generating all possible combinations.}
\index{Angry Birds}
...
\EN{It is possible in \IDA to produce listing files, so I did it, but I also set in options 
...
\EN{but too much redundant code because of $a$ and $b$ variables stored in the local stack, as well
as returning value.}
...
\EN{Keil doesn't generate FPU-instructions since it cannot rely it will
...
\EN{Comparison result is to be left in flags by this function, so the following}
