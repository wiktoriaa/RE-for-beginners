...
\RU{Входные величины помещаются в}\EN{The input values are placed into the} \GTT{D17} \AndENRU \GTT{D16} 
\RU{и сравниваются при помощи инструкции}\EN{registers and then compared using the} 
\INS{VCMPE}\EN{ instruction}.
...
\RU{Поэтому имеется инструкция}\EN{So there is} \INS{VMRS}%
...
\INS{VMOVGT} \RU{это аналог}\EN{is the analog of the} \INS{MOVGT}, 
...
\RU{в \GTT{D16} запишется значение $b$}\EN{the value of $b$ is to be written into \GTT{D16}}%
\RU{, лежащее в тот момент в}\EN{(that is currently stored in in} \GTT{D17}\EN{)}.

\RU{В обратном случае}\EN{Otherwise} 
\RU{в \GTT{D16} остается значение $a$.}
\EN{the value of $a$ stays in the \GTT{D16} register.}

\myindex{ARM!\Instructions!VMOV}
\RU{Предпоследняя инструкция \INS{VMOV} готовит то, что было в \GTT{D16}, для возврата через 
пару регистров \Reg{0} и \Reg{1}.}
\EN{The penultimate instruction \INS{VMOV} prepares the value in the \GTT{D16} register for returning it via the \Reg{0} and \Reg{1}
register pair.}
...
\RU{В листинге, сгенерированном при помощи \IDA, мы видим инструкцию \INS{VMOVGT}, 
такую же как и в предыдущем примере.}
\EN{Here, in the \IDA-generated listing, we see the \INS{VMOVGT} instruction, as in previous example.}

\RU{В реальности}\EN{In fact,} 
\RU{там закодирована обычная инструкция \INS{VMOV}}%
\EN{the usual \INS{VMOV} is encoded there}, 
\RU{просто \IDA добавила суффикс \GTT{-GT} к ней}%
\EN{but \IDA adds the \GTT{-GT} suffix to it}, 
\RU{потому что перед этой инструкцией стоит \INS{IT GT}.}
\EN{since there is a \INS{IT GT} instruction placed right before it.}

\label{ARM_Thumb_IT}
\myindex{ARM!\Instructions!IT}
\myindex{ARM!if-then block}
\EN{The}\RU{Инструкция} \INS{IT} \RU{определяет так называемый}\EN{instruction defines a so-called} \IT{if-then block}. 
...
\RU{В нашем примере}\EN{In our example,} \INS{IT GT} \RU{означает,}\EN{implies}
...
\INS{ITE} \RU{означает}\EN{stands for} \IT{if-then-else} 
...
\EN{The first instruction executes if the condition encoded in} \INS{ITE} (\IT{NE, not equal}) 
...
(\RU{Обратное условие от}\EN{The inverse condition of} \GTT{NE} \RU{это}\EN{is} \GTT{EQ} (\IT{equal})).
...
\EN{That's why \IDA adds the} \GTT{-EQ}\EN{ suffix
to each one of them}. 
...
\RU{означает, что первая и вторая инструкции исполнятся, если условие \GTT{LE} (\IT{Less or Equal})
справедливо, а третья~--- если справедливо обратное условие (\GTT{GT}\EMDASH\IT{Greater Than}).}
\EN{implies that the 1st and 2nd instructions are to be executed if the \GTT{LE} (\IT{Less or Equal})
...
\RU{встречаются только такие варианты инструкции \INS{IT}}\EN{only these variants of the \INS{IT} instruction are used}: 
...
\RU{Затем, зная что старшая часть 16-битного опкода (\INS{IT} это \GTT{0xBF}), сделаем при помощи \GTT{grep} это:}
\EN{Then, knowing the high part of the 16-bit opcode (\INS{IT} is \GTT{0xBF}), we do the following using \GTT{grep}:}
...
function to do the comparison:} \GTT{\_\_aeabi\_cdrcmple}. 
...
\INS{BCS} (\IT{Carry set\RU{~}---\RU{ }Greater than or equal})
