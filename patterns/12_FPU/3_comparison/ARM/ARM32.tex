...
\RU{потому что перед этой инструкцией стоит \TT{\q{IT GT}}}
...
\RU{В нашем примере}\EN{In our example}, \TT{\q{IT GT}} \RU{означает,}\EN{implies}
...
\q{Angry Birds} (\RU{для}\EN{for} iOS):
...
\RU{и снова этот фрагмент из}\EN{which is also from} \q{Angry Birds}:
...
\index{Angry Birds}
\RU{Еще фрагмент из}\EN{Another fragment from} \q{Angry Birds}:
...
\index{Angry Birds}
\RU{Например, в вышеупомянутой игре \q{Angry Birds} (версия \IT{classic} для iOS)}
\EN{For example, in the mentioned \q{Angry Birds} game (\IT{classic} version for iOS)}
