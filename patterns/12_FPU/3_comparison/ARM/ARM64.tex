...
(\RU{а это первый и второй аргументы ф-ции}\EN{which are the first and second arguments of the function})
...
the compiler is not thinking yet about the more convenient} \TT{FCSEL} \RU{так что он работает по старым 
методам}\EN{instruction, so it proceed using old methods}: 
...
into} \RegX{0}.
\RU{В другом случае}\EN{In the other case} ($a<=b$), \EN{the value of}\RU{значение} $b$ \RU{загружается в}\EN{gets loadedin} 
\RegX{0}.
