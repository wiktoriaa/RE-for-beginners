...
instead of} \ac{FPSCR}.
\RU{Очень удобная инструкция, если сравнивать с тем набором инструкций, что был в процессорах раньше}\EN{Very 
convenient instruction, if to compare to the instruction set in previous CPUs}.
...
\RU{В начале, код сохраняет значения входных аргументов в локальном стеке}\EN{First, code saves input argument 
values in the local stack} (\IT{Register Save Area}).
...
manually my removing redundant instructions, but do not introduce new ones (including \TT{FCSEL})}.
...
\EN{So numbers of \Tfloat type is passed in 32-bit S-registers (which are lower parts of 64-bit D-registers).}
