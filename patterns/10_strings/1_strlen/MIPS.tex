...
\RU{В MIPS нет инструкции \q{НЕ}, но есть NOR, это операция \q{ИЛИ + НЕ}.}
\EN{MIPS lacks a \q{NOT} instruction, but has \q{NOR} which is OR + NOT operation.}
\RU{Эта операция широко применяется в цифровой электронике\footnote{ИЛИ-НЕ называют \q{универсальным элементом}.
...
\RU{Так что операция \q{НЕ} реализована здесь как}\EN{So, the NOT operation is implemented here as} 
\q{NOR DST, \$ZERO, SRC}.
