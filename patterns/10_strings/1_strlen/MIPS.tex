...
\RU{В MIPS нет инструкии ``НЕ'', но есть NOR, это операция ``ИЛИ + НЕ''.}
\EN{MIPS lacks NOT instruction, but has NOR which is OR + NOT operation.}
\RU{Эта операция широко применяется в цикрофой электронике\footnote{ИЛИ-НЕ называют ``универсальным элементом''.
