...
\EN{You can also read about it in} ``\IT{\SignedNumbersSectionName}''\EN{ section}~(\myref{sec:signednumbers}).
...
will set the \ZF flag to $0$, but \SETNZ, if \TT{ZF==0} (\IT{NZ} stands for \IT{not zero}) will set \AL to $1$.
...
\RU{См. также раздел:}\EN{See also:} ``\SignedNumbersSectionName''~(\myref{sec:signednumbers}).
