\sectionold{Pseudo-random number generator example}
\label{FPU_PRNG}
...
The exponent cannot be zero (the floating number is denormalized in this case), so we are storing \GTT{01111111} 
...
\lstinputlisting{patterns/17_unions/FPU_PRNG/FPU_PRNG_EN.cpp}

\subsectionold{x86}

\lstinputlisting[caption=\Optimizing MSVC 2010]{patterns/17_unions/FPU_PRNG/MSVC2010_Ox_Ob0_EN.asm}
...
\lstinputlisting[caption=\Optimizing GCC 4.4.5]{patterns/17_unions/FPU_PRNG/MIPS_O3_IDA_EN.lst}
...
\subsectionold{ARM (\ARMMode)}

\lstinputlisting[caption=\Optimizing GCC 4.6.3 (IDA)]{patterns/17_unions/FPU_PRNG/raspberry_GCC_O3_IDA_EN.lst}
