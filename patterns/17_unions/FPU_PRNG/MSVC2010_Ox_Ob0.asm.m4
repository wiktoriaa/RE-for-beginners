include(`commons.m4')$SG4238	DB	'%f', 0aH, 00H
...
; _EN(`store value we got into local stack and reload it:')_RU(`сохранить полученное значение в локальном стеке и перезагрузить его')
	fstp	DWORD PTR tv130[esp+4] ; \  _EN(`these instructions are redundant')_RU(`эти инструкции избыточны')
