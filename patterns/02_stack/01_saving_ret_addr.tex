...
\RU{Как следствие, ``leaf''-функция не использует регистр \ac{LR}}
\EN{As a consequence, leaf functions do not use the \ac{LR} register}.
...
\RU{Это может быть быстрее чем в x86, ведь внешняя память для стека не используется}
\EN{This can be faster than on x86 because external RAM is not used for the stack}
