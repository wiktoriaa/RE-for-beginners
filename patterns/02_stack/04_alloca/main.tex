...
\EN{One of the reason there is a separate function instead of just a couple instructions in the code,
is because the \ac{MSVC} implementation
of the alloca() function also has code which reads from the memory just allocated, in order to let the \ac{OS} map
