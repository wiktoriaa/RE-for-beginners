...
вернет \ESP назад в изначальное состояние и выделенная память просто \IT{выкидывается}.}
\EN{The allocated memory chunk does not need to be freed via a \TT{free()} function call, since the 
function epilogue~(\myref{sec:prologepilog}) will return \ESP back to its initial state and 
the allocated memory will be just \IT{dropped}.}
...
console), it writes it to the \TT{buf} buffer. \puts copies \TT{buf} contents to \gls{stdout}. Of course, these two
...
\footnote{
\RU{Это потому, что alloca() ~--- это не сколько функция, сколько т.н. \IT{compiler intrinsic} (\myref{sec:compiler_intrinsic})}
\EN{It is because alloca() is rather a compiler intrinsic (\myref{sec:compiler_intrinsic}) than a normal function}.
...
\RU{ ~--- это аналог}\EN{corresponds to} \TT{mov DWORD PTR [esp+20], 3} \RU{в Intel-синтаксисе:}
\EN{ in Intel-syntax.}\RU{ при адресации памяти в виде} \EN{In AT\&T syntax when addressing memory using} \IT{\RU{регистр+смещение}\EN{register+offset}} \EN format, 
\RU{это записывается в AT\&T синтаксисе как}\EN{it is notated as} 
