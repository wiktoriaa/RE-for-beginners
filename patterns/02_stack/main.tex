...
\RU{Стек в компьютерных науках ~--- это одна из наиболее фундаментальных вещей}
\EN{A stack is one of the most fundamental data structures in computer science}
\footnote{\href{http://go.yurichev.com/17119}{wikipedia}}.

\RU{Технически, это просто блок памяти в памяти процесса + регистр \ESP или \RSP в x86 или x64, либо \ac{SP} в ARM, который указывает где-то в пределах этого блока.}
\EN{Technically, it is just a block of memory in process memory along with the \ESP or \RSP register in x86 or x64, or the \ac{SP} register in ARM, as a pointer within the block.}
...
\EN{\POP is the reverse operation: get the data from memory pointed to by \ac{SP}, 
...
\EN{After stack allocation the \gls{stack pointer} points to the end of stack.}
...
\EN{The end of the stack is actually at the beginning of the memory allocated for the stack block. 
...
сторону увеличения.}
\EN{Besides support of descending stacks, ARM has also support of ascending stacks.} \\
...
\RU{предназначены для descending-стека, т.е. уменьшающегося}\EN{instructions are intended to deal with 
a descending stack}.
\RU{Инструкции}\EN{The}
\ac{STMFA}/\ac{LDMFA}, \ac{STMEA}/\ac{LDMEA} 
\RU{предназначены для ascending-стека, т.е. увеличивающегося}\EN{instructions are intended to deal with 
an ascending stack}.

\section{\RU{Почему стек растет в обратную сторону?}\EN{Why does the stack grow backward?}}

\RU{Интуитивно мы можем подумать, что, как и любая другая структура данных, стек мог бы расти вперед, 
т.е. в сторону увеличения адресов}\EN{Intuitively, we might think that, like any other data structure, 
the stack may grow upward, i.e., towards higher addresses}.

\RU{Причина, почему стек растет назад, вероятно, историческая}
\EN{The reason the stack grows backward is probably historical}.
\RU{Когда компьютеры были большие и занимали целую комнату, было очень легко разделить сегмент на две части,
для \glslink{heap}{кучи} и стека}\EN{When computers were big and occupied a whole room, 
it was easy to divide memory into two parts, one for the \gls{heap} and one for the stack}.
\RU{Конечно, ведь заранее было неизвестно, насколько большой может быть \glslink{heap}{куча} или стек, 
