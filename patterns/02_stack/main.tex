...
%I've added some extra EN text in parenthesis clarifying the meaning of "decsending" and "ascending"
%because the terms were not introduced till that point. Those could go in footnote as well. Sorry that I am
%not includding a russian version but I my russian is not that good.
\RU{предназначены для descending-стека, т.е. уменьшающегося}\EN{instructions are intended to deal with 
a descending stack (grows downwards, starting with a high address and progressing to a lower one)}.
...
an ascending stack (grows upwards, starting from a low address and progressing to a higher one)}.
