\subsection{\IFRU{Передача параметров для функции}{Passing function arguments}}
...
{Functions with a variable number of arguments (like \printf) determine the number by specifiers
(which begin with a \% sign) in the format string.}
\IFRU{Если написать что-то вроде}{If we write something like} 
...
{That's why it is not very important how we declare the \main function}: \IFRU{как}{as} \main, 
\TT{main(int argc, char *argv[])} 
\IFRU{либо}{or} \TT{main(int argc, char *argv[], char *envp[])}.

\IFRU{В реальности, \ac{CRT}-код вызывает \main примерно так:}
{In fact, \ac{CRT}-code is calling \main roughly as:}
...
{If you'll declare \main as \main without arguments, they are, nevertheless, still present in the stack, but
not used.}
