\section{\RU{Разметка типичного стека}\EN{A typical stack layout}\PTBR{Um modelo típico de pilha}}
...
\PTBR{Um modelo típico de pilha em um ambiente 32-bits no início de uma função,
...
\dots & \dots \\
\hline
ESP-0xC & \RU{локальная переменная}\EN{local variable}\PTBR{Variável local} \#2, \MarkedInIDAAs{} \TT{var\_8} \\
\hline
ESP-8 & \RU{локальная переменная}\EN{local variable}\PTBR{Variável local} \#1, \MarkedInIDAAs{} \TT{var\_4} \\
\hline
ESP-4 & \RU{сохраненное значение}\EN{saved value of}\PTBR{Valor salvo de} \EBP \\
\hline
