...
\RU{Результат первого сравнения}\EN{First comparison results}: \figname \ref{fig:jcc_olly_unsigned_1}.
...
\RU{Следующий переход}\EN{The next conditional jump}:\figname \ref{fig:jcc_olly_unsigned_2}.
...
\RU{Третий переход}\EN{The third conditional jump} \JNB: \figname \ref{fig:jcc_olly_unsigned_3}.
...
\RU{он срабатывает если}\EN{it does if} ZF=0 (zero flag): \figname \ref{fig:jcc_olly_signed_2}.
...
\RU{что в нашем случае не так}\EN{and that is not true in our case}: \figname \ref{fig:jcc_olly_signed_3}.
...
\RU{Вот как она выглядит в}\EN{Here is how it looks in} Hiew: \figname \ref{fig:jcc_hiew_1}.
...
\RU{Что и делаем}\EN{That's what we do}: \figname \ref{fig:jcc_hiew_2}.
...
если флаги всегда одни и те же}\EN{Observant reader may ask, why to execute \CMP so many times, 
...
\RU{Нужно сказать что x86-код такого типа редок}\EN{Needless to say, that type of x86 code is rare}.
MSVC 2012, \RU{как видно, не может этого делать}\EN{as it seems, can't do that}.
\RU{С другой стороны, программисты на ассемблере прекрасно осведомлены о том, что инструкции}\EN{On the other case, 
