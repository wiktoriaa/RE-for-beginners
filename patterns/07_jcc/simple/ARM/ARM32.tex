...
\EN{For instance, the \ADD instruction is in fact \TT{ADDAL} internally, where \TT{AL} means
...
conditional jump, but has \TT{AL} in the \IT{condition field}, and it means execute always, ignoring flags.}
...
\EN{The \TT{ADRGT} instructions works just like \TT{ADR} but will execute only in the case when the previous \CMP
...
triggered only if the result of the comparison was the same} (\IT{Greater Than}). 
...
\EN{Consequently, these instructions with suffix \TT{-GT} will be executed only in the case when
...
but will be triggered only when one value was greater or equal to the other}
(\IT{Greater or Equal}).
...
функции закончится.}
\EN{instruction is that is like a function epilogue, but it will be triggered only if $a>=b$, only then the function 
...
\EN{But if it is not true, i.e., $a<b$, then the control flow will continue to the next \TT{``LDMFD SP!, \{R4-R6,LR\}''} instruction,
...
секции}\EN{We already examined an unconditional jump to the \printf function instead of function return, in} <<\PrintfSeveralArgumentsSectionName>>\RU{, здесь}\EN{ section, here}~(\ref{ARM_B_to_printf}).
