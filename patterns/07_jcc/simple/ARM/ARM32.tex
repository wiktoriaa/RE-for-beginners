...
other during last comparison.}
\RU{Перед ними расположен еще один \CMP (потому что вызов \printf мог испортить состояние флагов).}
\EN{Another \CMP is located before them (because \printf call may tamper state of flags).}
...
\EN{There is no such feature in x86, if not to consider \TT{CMOVcc} instruction, it is the same as \MOV,
...
там используются 
инструкции}\EN{function is just likewise, but other instructions are used while dealing 
