...
\EN{But if it is not true, i.e., $a<b$, then the control flow will next to the \TT{``LDMFD SP!, \{R4-R6,LR\}''} instruction,
...
секции}\EN{We already examined ann unconditional jump to the \printf function instead of function return, in} <<\PrintfSeveralArgumentsSectionName>>\RU{, здесь}\EN{ section, here}~(\ref{ARM_B_to_printf}).
