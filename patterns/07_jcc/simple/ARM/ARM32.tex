...
\EN{But if that condition is not satisfied, i.e., $a<b$, then the control flow will continue to the next \TT{``LDMFD SP!, \{R4-R6,LR\}''} instruction, which is one more function epilogue. This instruction restores not only the \TT{R4-R6} registers state, but also \ac{LR} instead of \ac{PC}, thus, it does not returns from the function.}
...
там используются 
инструкции}\EN{is similar, only other instructions are used while dealing 
