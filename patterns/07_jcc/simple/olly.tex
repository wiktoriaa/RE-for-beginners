...
флаги будут три раза одни и те же каждый раз}\EN{\CMP is executed thrice here, but for the same arguments, so
the flags will be the same each time}.
...
\olly \RU{подсказывает, что первый переход}\EN{gives a hint that the} (\JBE) \RU{сработает}\EN{jump will be triggered}.
...
\clearpage
\RU{Второй переход}\EN{The second} \JNZ \RU{сработает}\EN{conditional jump will trigger}: 
