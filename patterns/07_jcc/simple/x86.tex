...
\EN{See also the section about signed number representations}~(\ref{sec:signednumbers}).
...
\EN{produces almost the same code, but with} \puts~(\ref{puts}) \RU{вместо}\EN{instead of} \printf.
...
\RU{Этот прием будет описан немного позже}\EN{This kind of trick will be explained later}: \ref{JMP_instead_of_RET}.
