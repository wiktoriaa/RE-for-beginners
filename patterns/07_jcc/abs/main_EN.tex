\sectionold{Calculating absolute value}
\label{sec:abs}
...
\subsectionold{\NonOptimizing GCC 4.9 (ARM64)}
...
There is also the \INS{NEGU} pseudoinstruction, which just does subtraction from zero.
The \q{U} suffix in both \INS{SUBU} and \INS{NEGU} implies that no exception to be raised in case of integer overflow.

\subsectionold{Branchless version?}
