\section{\RU{Тернарный оператор}\EN{Ternary operator}}
\label{chap:cond}

\RU{Тернарный оператор (ternary operator) в}\EN{The ternary operator in} \CCpp \RU{это}\EN{has the following syntax}:
...
\EN{It has, however, ``Conditional SELect'' instruction (\TT{CSEL}), but GCC 4.9 does not seem to be smart enough to use it in such piece of code.}
\fi
...
\RU{убрав все инструкции условного перехода и использовав инструкцию \TT{CSEL}}\EN{by removing all 
