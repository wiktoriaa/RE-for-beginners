...
\RU{Место в стеке для ``shadow space'' выделяет именно \gls{caller}}\EN{It up to the 
\gls{caller} to allocate ``shadow space'' in the stack}.
...
\EN{There are is ``shadow space'' requirement in System V *NIX\cite{SysVABI}, but the \gls{callee} may need to save
