...
is called then.}
\RU{Внутри \TT{f()}, доступ к аргументам, также как и к локальным переменным, происходит через макросы: 
...
\EN{Argument access inside \TT{f()} is organized with the help of macros like: \TT{\_a\$ = 8}, 
...
\EN{Then \TT{a} value is stored into \EAX. After \IMUL instruction execution, value in the \EAX is 
...
\RU{Когда мы протрассируем до первой инструкции в \TT{f()}, которая использует какой-то из аргументов
