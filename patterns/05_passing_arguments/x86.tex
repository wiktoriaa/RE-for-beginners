...
\EN{What we see is that the \main function pushes 3 numbers in the stack stack calls \TT{f(int,int,int).}} 
...
So, we're addressing the \IT{outer} side of the \gls{stack frame} by adding the \TT{\_a\$} macro to the value in the \EBP register.}
...
a \gls{product} of the value in \EAX and what is stored in \TT{\_b}.}
...
Returning to \gls{caller}~---it will take value from the \EAX and used it as an argument to \printf.}
...
инструкция}\EN{The \gls{stack pointer} is will not go back after both function execute, because the penultimate}
\TT{LEAVE} (\myref{x86_ins:LEAVE}) \RU{сделает это за один раз, в конце исполнения}\EN{instruction will do this
at the end}.
