...
\RU{Имеем в итоге}\EN{Here is what we get after compilation} (MSVC 2010 Express):
...
On returning to \gls{caller}, it will take the \EAX value and use it as an argument to \printf.}
...
инструкция}\EN{The \gls{stack pointer} is not set back after the two function calls(f and printf), because the penultimate}
\TT{LEAVE} (\myref{x86_ins:LEAVE}) \RU{сделает это за один раз, в конце исполнения}\EN{instruction will take care of this at the end}.