...
\EN{The \ttf{} function is just the same, but whole 64-bit X-registers are now used.}
...
\EN{Long 64-bit values are loaded into the registers by parts, I have described it also here}: \ref{ARM_big_constants_loading}.
...
for a case if someone (or something) in this function will need to use the \TT{W0...W2} 
registers, this overwriting original
...
\RU{хотя, вызываемая ф-ция не обязана сохранять их}\EN{, however, the callee is not obliged to save them}.
...
that the function arguments will not be needed in the future and registers \TT{W0...W2} also will not be used.}
