...
\RU{Далее вызывается два раза \TT{dump()}.}\EN{\TT{dump()} function called two times after.}
...
\EN{In fact, internally, constructors are returns pointer to the newly created object, i.e.}, \ITthis.
...
\EN{This is all because here used thiscall~(\ref{thiscall}) calling convention, the method of passing values through the
...
\lstinputlisting[caption=MSVC 2012 x64 /Ox]{patterns/cpp/classes/simple_MSVC_x64.asm}
...
\EN{It is almost the same situation in GCC 4.4.1, with a few exceptions.}
