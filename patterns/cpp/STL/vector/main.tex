...
\IFRU{Я бы назвал}{I would call} \TT{std::vector} ``\IFRU{безопасной оболочкой (wrapper)}{safe wrapper}'' \EN{of }\ac{PODT} \IFRU{массива в Си}{C array}.
...
\RU{Метод }\TT{.at()} \IFRU{с другой стороны, проверяет, и подкидывает исключение}{method, however, does checking and 
...
\RU{Метод }\TT{.reserve()} \IFRU{точно так же вставлен прямо в код \main}{method is inlined as well}.
