...
\IFRU{Это также циркулярный список, что означает что последний элемент имеет указатель на первый 
и наоборот: первый имеет указатель на последний}
{This is also a circular list, meaning that the last element has a pointer to the first and 
...
\IFRU{Обратные итераторы}{Reverse iterators} (.rbegin, .rend) \IFRU{работают точно так же,
только наоборот}
...
\lstinputlisting[caption=GCC 4.8.1 -O3 -fno-inline-small-functions]{patterns/cpp/STL/list/GCC.asm}

\lstinputlisting[caption=\IFRU{Весь вывод}{The whole output}]{patterns/cpp/STL/list/GCC.txt}
...
\lstinputlisting[caption=\IFRU{Весь вывод}{The whole output}]{patterns/cpp/STL/list/MSVC.txt}
