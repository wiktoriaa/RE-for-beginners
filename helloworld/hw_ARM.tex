...
\newcommand{\LRwithfootnote}{\TT{LR}\footnote{link register}\xspace}
\newcommand{\PCwithfootnote}{\TT{PC}\footnote{program counter}\xspace}
\newcommand{\SPwithfootnote}{\TT{SP}\footnote{stack pointer}\xspace}
...
\subsubsection{\IFRU{Неоптимизирующий}{Non-optimizing} Keil: \IFRU{режим ARM}{ARM mode}}
...
\subsubsection{\IFRU{Неоптимизирующий}{Non-optimizing} Keil: \IFRU{режим thumb}{thumb mode}}
...
\subsubsection{\IFRU{Оптимизирующий}{Optimizing} Xcode: \IFRU{режим ARM}{ARM mode}}

Xcode 4.6.3 \IFRU{без включенной оптимизации выдает слишком много лишнего кода, поэтому остановимся на той версии, где как можно меньше инструкций}{without optimization turned on, produces a lot of redundant code, so we'll study that version where instruction count as small as possible}: \TT{(-O3)}.
...
\subsubsection{\IFRU{Оптимизирующий}{Optimizing} Xcode: \IFRU{режим thumb-2}{thumb-2 mode}}
...
Инструкции \TT{BL} и \TT{BLX} в thumb, как мы помним, обычно кодируются как пара 16-битных инструкций, 
...
Thumb-2 инструкция, например, \TT{``MOVW R0, \#0x13D8''} ~--- она записывает 16-битное число в 
