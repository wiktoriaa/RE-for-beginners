...
{would simplify output code but suppressing RIP-relative addressing details, so I'll run \IT{objdump} instead to see the details}:
...
to another place, it ``patches'' DLL in memory (at the \IT{FIXUP} places) in order to correct 
all addresses.}
...
blocks ~--- because each loaded into memory DLL instance \IT{fixed} to be work only at these addresses..}
