...
That's because \printf requires 32-bit \Tint, but we got WORD in structure ~--- that is 16-bit unsigned type.
That's why by copying value from WORD into \Tint{}, bits from 16 to 31 should also be cleared, 
...
from previous}.
\IFRU{Но и снова я должен отметить, что в реальности так лучше не делать}{And again I should note, one should not do
this in practice}.
