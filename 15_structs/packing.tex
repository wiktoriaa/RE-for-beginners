...
{As we may recall, here a structure passed instead of pointer to structure, and because first 4 function arguments 
in ARM are passed via registers, so then structure fields are passed via} \TT{R0-R3}.
...
\IFRU{Она здесь применяется дла загрузки полей}{Here it's used for loading fields} $a$ \IFRU{и}{and} $c$ 
...
{Also, it's located nearly (take a look on addresses).}
...
\IFRU{Keil решил сделать так, вероятно, из-за экономии.}{Apparently, Keil decides to do this because of economy.}
...
\TT{SXTB} (\IT{Signed Extend Byte}) \IFRU{это так же аналог}{is analogous to} \MOVSX \IFRU{в}{in} x86\IFRU{, только работает не с памятью, а с регистром.}{ as well, but works not with memory, but with register.}
