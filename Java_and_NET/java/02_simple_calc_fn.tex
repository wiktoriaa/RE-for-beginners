...
\EN{Here's the output when the \TT{iconst\_2} instruction is used:}
...
\TT{iload\_0} \EN{takes the zeroth function argument and pushes it to the stack.}
\RU{Берет нулевой аргумент функции и заталкивает его в стек.}
\TT{iconst\_2} \EN{pushes 2 in the stack}\RU{заталкивает в стек 2}.
...
\EN{\TT{idiv} just takes the two values at the \ac{TOS}, divides one by the other and leaves
...
\EN{It's the same, but the \TT{ldc2\_w} instruction is used to load the constant 
...
\EN{\TT{iload\_0} loads the first function argument (a), \TT{iload\_2}\EMDASH{}second (b).}
...
\EN{\TT{iadd} adds the two values and leaves the result at \ac{TOS}:}
