...
\RU{Наверное, самая простая из всех возможных ф-ций на Java это та, что возвращает 
некоторое значение.}
\EN{Oh, and we must keep in mind that there are no ``free'' functions in Java in common sense,
they are ``methods''. }
\RU{О, и мы не должны забывать что в Java нет ``свободных'' ф-ций в общем смысле,
это ``методы''.}
...
\RU{Но я все равно буду называть их ``функциями'', потому что я так привык.}
...
\IT{short} \EN{in name means a 16-bit value is to be pushed}\RU{в имени означает, что 16-битное значение будет заталкиваться в стек}. 
...
\RU{Так что 32-битное число 12345678 сохранено в так называемом ``constant pool'' (пул констант),
...
\RU{Код для MIPS также традиционно имеет пул констант, называемый ``literal pool'', это сегменты
с названиями ``.lit4'' (для хранения 32-битных чисел с плавающей точкой одинарной точности) и
``.lit8''(для хранения 64-битных чисел с плавающей точкой двойной точности).}
...
\RU{Но нельзя использовать возвращаемое значение булевого типа как целочисленное и наоборот ---
информация о типах сохраняется в class-файлах и проверяется при запуске.}
...
\TT{bipush} \RU{означает}\EN{mean} ``push byte''.
...
и он эквивалентен \IT{short}, но ASCII-код символа ``A'' это 65, и можно воспользоваться
...
\RU{\TT{dreturn} означает ``return double''.}
...
\RU{\TT{freturn} означает ``return float''.}

\EN{Now what about function returning nothing?}
\RU{А что насчет тех случаев, когда ф-ция ничего не возвращает?}
