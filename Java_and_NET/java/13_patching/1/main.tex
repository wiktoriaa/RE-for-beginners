...
\EN{How we would remove printing of ``This program is not registered'' string?}
\RU{Как можно избавиться от печати строки ``This program is not registered''?}
...
\RU{В начале заменим первый байт ф-ции на 177 (это опкод инструкции \TT{return}):}
...
\RU{ОК, попробуем пропатчить её иначе, удаляя вызов ф-ции \TT{nag()}:}
