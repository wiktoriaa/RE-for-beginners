...
\EN{Let's rework our \IT{Month} example (\myref{Java_2D_array_month}) slightly:}
...
\EN{Essentially, \TT{IncorrectMonthException.class} has just object constructor 
and one accessor method.}
\RU{Которко говоря, \TT{IncorrectMonthException.class} имеет только конструктор объетка и один
метод-акцессор.}

\EN{\TT{IncorrectMonthException} class is inherited from \TT{Exception}, 
...
\TT{getIndex()} \EN{is just accessor}\RU{это просто акцессор}.
\EN{\IT{Reference} to \TT{IncorrectMonthException} is passed in zeroth \ac{LVA} slot
(\IT{this}), \TT{aload\_0} takes it, \TT{getfield} loads integer value from object, 
...
\EN{Now let's take a look on \TT{get\_month()} in \TT{Month2.class}:}
...
\EN{In case of invalid index, a new object is created using \TT{new} instruction at offset 10.}
\RU{В случае неправильного индекса, создается новый объект при помощи инструкции \TT{new} 
по смещению 10.}
\EN{Object type is passed as operand to the instruction (and this is \TT{IncorrectMonthException}).}
\RU{Тип объекта передается как операнд инструкции (и это \TT{IncorrectMonthException}).}
\EN{Then its constructor is called with index integer is passed via \ac{TOS} (offset 15).}
\RU{Затем вызывается его конструктор, в который передается индекс (через \ac{TOS}) (по смещению 15).}
\EN{At the time of control flow being at offset 18, object is already constructed, 
now \TT{athrow} instruction takes \IT{reference} 
to newly constructed object and signalling to \ac{JVM} to find appropriate exception handler.}
...
\EN{\TT{athrow} instruction don't return control flow to here, 
so at offset 19 is another \gls{basic block},
...
\EN{How handler works}\RU{Как работает обработчик}?
\RU{Now take a look at}\RU{Посмотрим на} \main \InENRU \TT{Month2.class}:
...
\EN{Here is \TT{Exception table}, which defines that from offset 0 to 11 (inclusive) an exception 
\TT{IncorrectMonthException} may happen, and if it will, control flow is to be passed to offset 14.}
...
\EN{Indeed, the main program is ended at offset 11.}
\RU{Действительно, основная программа заканчивается на смещении 11.}
\EN{At the offset 14 handler started, it's not possible to get here, 
...
\EN{But \ac{JVM} will transfer execution flow here in case of exception.}
\RU{Но \ac{JVM} передаст сюда управление в случае исключения.}
\EN{The very first \TT{astore\_1} (at 14) takes incoming \IT{reference} to exception object 
and stores it to \ac{LVA} slot 1.}
...
\EN{Later, \TT{getIndex()} method (of this exception object) will be called for it at offset 31.}
\RU{Позже, по смещению 31 будет вызван метод этого объекта (\TT{getIndex()}).}
\EN{\IT{Reference} to current exception object is passed right before (offset 30).}
\RU{Указатель \IT{reference} на текующий объект исключения передался немного раньше (смещение 30).}
\EN{All the rest of the code is string manipulating code: 
first integer value returned by \TT{getIndex()}
is converted into string by \TT{toString()} method, then it concatenates with 
\q{incorrect month index: } text string (like we already considered before),
...
\q{incorrect month index: } (как мы уже рассматривали раннее),
затем вызываются \TT{println()} и \TT{printStackTrace()}.}
\EN{After \TT{printStackTrace()} finishes, exception is handled, we can proceed to normal functioning.}
\RU{После того как \TT{printStackTrace()} заканчивается, исключение уже обработано, мы можем
возвращаться к нормальной работе.}
\EN{At offset 47 there are \TT{return} which finishes \main function, 
but there could be any other code which executes as if no exceptions raised.}
...
\EN{Here is an example, how IDA shows exception ranges:}
