% TODO proof-reading
\section{\EN{Simple function calling}\RU{Простой вызов ф-ций}}
...
\RU{\TT{invokestatic} вызывает ф-цию \TT{Math.random()} и оставляет результат на \ac{TOS}.}
\EN{Then the result is divided by 0.5 and returned.}
\RU{Затем результат делится на 0.5 и возвращается.}
\EN{But how function name is encoded?}
\RU{Но как закодировано имя ф-ции?}
...
указывает на обычную текстовую строку (``java/lang/Math'').}
...
\EN{First string is ``random'' which is the name of method.}
\RU{Первая строка это ``random'', это имя метода.}
\EN{The second string is ``()D'' which encodes function type. It means it returns 
\IT{double} value (hence \IT{D} in the string).}
\RU{Вторая строка это ``()D'', которая кодирует тип ф-ции. Это означает что возвращаемый тип --- 
\IT{double} (отсюда \IT{D} в строке).}
\EN{This is the way 
...
\EN{Now let's finally try ``Hello, world!'' example:}
\RU{Наконец попробуем пример ``Hello, world!'':}
...
\EN{\TT{ldc} at offset 3 is taking pointer to the ``Hello, World'' string in constant pool 
and pushes into stack.}
\RU{\TT{ldc} по смещению 3 берет указатель (или адрес) на строку ``Hello, World'' 
...
\RU{Уже знакомая нам инструкция \TT{invokevirtual} берет информацию о ф-ции (или методе) \TT{println} 
...
\footnote{Или ``экземпляр класса'' в некоторой русскоязычной литературе.};
2) адрес строки ``Hello, World''.}
