% TODO proof-reading
\subsection{\EN{\ac{JVM} memory model}\RU{Модель памяти в \ac{JVM}}}

\EN{x86 and other low-level environments use the stack for argument passing and 
...
\EN{Used as storage for incoming function arguments and local variables.}
\RU{Используется как хранилище для аргументов функций и локальных переменных.}
\EN{Instructions like \INS{iload\_0} load values from it.}
\RU{Инструкции вроде \INS{iload\_0} загружают значения оттуда.}
\INS{istore} \EN{stores values in it}\RU{записывает значения туда}.
\EN{In the beginning the function arguments are stored: starting at 0 or at 1 
...
\EN{Then the local variables are allocated.}
