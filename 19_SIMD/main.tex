...
When user change image brightness, the editor should add a coefficient to each pixel value, or to subtract.
...
it is possible to get 4 64-bit products\footnote{multiplication result} at once.}
...
then address should be aligned on a 16-byte border. If it is not aligned, exception will be occured
...
{After that block, that is, starting from address 0x008c2000 there is nothing at all, e.g., \ac{OS} not allocated
