...
\index{\CLanguageConstructs!for}

\input{09_loops/loops_x86}

\input{09_loops/loops_ARM}
...
\IFRU{Это правильно.}{And that's correct.} 
\IFRU{Потому что если условие в самом начале не выполняется, тело цикла исполнять нельзя.}{Because, if loop condition is
not met at the beginning, loop body shouldn't be executed.}
...
\IFRU{Если}{If} \IT{total\_entries\_to\_process} \IFRU{равно нулю}{equals to zero}, 
\IFRU{тело цикла не должно исполниться ни разу}{loop body shouldn't be executed whatsoever}.
\IFRU{Поэтому проверка условия происходит перед тем как исполнить само тело.}{So that's why condition checked before
loop body execution.}
...
{However, optimizing compiler may swap condition check and loop body, if it sure that the situation described here is
