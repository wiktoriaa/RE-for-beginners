...
она проверяет значение регистра \ECX и если оно не $0$, делает декремент\footnote{уменьшение на $1$} \ECX 
...
{One very important property is that \TT{f()} function should not change value in the \ESI.
...
{Loop unwinding has advantage in these cases when there is not so much iterations and 
...
With the exception that the \EBX register will be fixed to the \TT{i} variable.}
