...
Cela petmet de charger nos propre bibilothèques dynamiques avant les autres, même celle du système, comme libc.so.6.

Cela permet de de \q{substituer} nos propres fonctions, avant celles originales des bibliothèques du système.
Par exemple, il est facile d'intercepter tous les appels à
time(), read(), write(), etc. \\
\\
\myindex{uptime}
Regardons si l'on peut tromper l'utilitaire \IT{uptime}.
Comme chacun sait, il indique depuis combien de temps l'ordinateur est démarré.
\myindex{strace}
...
elle sera chargée pour tout les programmes démarrés. \\
\\
Plus d'exemples:
