...
\dots \RU{и даже более того, можно взять указатель на аргумент ф-ции и передать его в другую ф-цию:}
\EN{even more than that, it's possible to take a pointer to the function argument and pass
...
\EN{It's hard to understand how it works until we will see the code:}

\lstinputlisting[caption=\Optimizing MSVC 2010]{OS/calling_conventions/ptr_to_argument/MSVC_2010_O3.asm}

\RU{Адрес места в стеке где была передана $a$ просто передается в другую ф-цию.
Она модифицирует переменную по этому адресу, и затем \printf выведет модифицированное значение.}
\EN{The address of the place in stack where $a$ was passed is just passed to another function.
It will modify the value by pointer and then \printf will print modified value.}\\
\\
\RU{Наблюдательный читатель может спросить, а что насчет тех соглашений о вызовах, где аргументы ф-ции
передаются в регистрах?}
\EN{Observant reader might ask, what about calling conventions where function arguments are
passed in registers?}

\RU{Это та ситуация где используется \IT{Shadow Space}.}
\EN{That's a situation when \IT{Shadow Space} is used.}
\RU{Так что входящее значение копируется из регистра в \IT{Shadow Space} в локальном стеке и затем это адрес
передается в другую ф-цию:}
\EN{So the input value is copied from register
to the \IT{Shadow Space} in the local stack, and then this address is passed into another function:}
...
\EN{GCC is also store input value in the local stack:}
...
\EN{GCC for ARM64 doing the same, but this space is called \IT{Register Save Area} here:}
...
\RU{Кстати, похожее использование \IT{Shadow Space} разбирается здесь}: \ref{variadic_arith_registers}.
