\chapterold{Arguments passing methods (calling conventions)}
...
It's almost the same as \IT{cdecl}, with the exception that the \gls{callee} must set \ESP to the initial state by executing the \TT{RET x} instruction instead of \RET, where
...
\subsectionold{Functions with variable number of arguments}
...
\subsectionold{Watcom/OpenWatcom}
\myindex{OpenWatcom}
...
\subsubsectionold{Windows x64: Passing \ITthis (\CCpp)}
...
\sectionold{Return values of \Tfloat and \Tdouble type}
\myindex{float}
\myindex{double}
...
\sectionold{Modifying arguments}
