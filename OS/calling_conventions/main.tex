...
so that the \gls{callee} can save the first 4 arguments there.
...
\LEA (\myref{sec:LEA})}.
\RU{\INS{MOV} здесь был бы на 1 байт длиннее (5 вместо 4).}%
\EN{\INS{MOV} would be 1 byte longer here (5 instead of 4).}
