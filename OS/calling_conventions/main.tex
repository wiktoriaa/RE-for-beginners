...
\EN{The \Gls{caller} pushes the arguments in the stack in reverse order: the last argument, then the penultimate element 
and finally the first argument.
...
The \Gls{caller} will not adjust the \gls{stack pointer} using the \TT{add esp, x} instruction.}
...
\\
\RU{Например, мы можем взять ф-цию из}\EN{For example, we can take the function from} 
...
It's a well known caveat: if you have two DLLs and the first uses the second, and they are built by different compilers with 
...
\RU{Так что и здесь можно легко вычислять количество аргументов}\EN{Which means that the number of arguments
...
\EN{Of course, if the number the of arguments is less than 3, not all 3 registers will be used.}
...
\RU{Для примера, см.}\EN{For example, see} (\ref{regparm}).

\subsection{Watcom/OpenWatcom}
\index{OpenWatcom}

\RU{Здесь это называется}\EN{Here it is called} ``register calling convention''.
...
\EN{The functions have an underscore added to the function name in order to distinguish them from 
...
\RU{Для примера, см.}\EN{For an example, see} (\ref{thiscall}).
...
именно для этого}
\EN{The code of the f1() function's prologue saves the arguments in the ``scratch space''---a space in the stack
...
\EN{If we compile the example with optimizations, it will be almost the same, but the ``scratch space''
will not be used, because it won't be needed}.
...
\LEA (\ref{sec:LEA})}.
\RU{Я не уверен, что это того стоит, но может быть}\EN{I'm not sure if it worth doing so, but maybe}.

\RU{Еще один пример подобного}\EN{Another example of such thing is}: \ref{TaskMgr_LEA}.

\subsubsection{\RU{Передача \ITthis}\EN{Passing \ITthis} (\CCpp)}

\EN{The}\RU{Указатель} \ITthis \RU{передается через}\EN{pointer is passed in} \RCX, 
\RU{первый аргумент метода через}\EN{the first argument of the method is in} \RDX, \RU{и т.д.}\EN{etc.}
\RU{Для примера, см. также}\EN{For an example see}: \ref{simple_CPP_MSVC_x64}.
...
used instead of 4 (\RDI, \RSI, \RDX, \RCX, \Reg{8}, \Reg{9}) and there is no ``scratch space'', 
...
may ask, what will happen if they modify the arguments?}
\RU{Ответ прост: аргументы хранятся в стеке, именно там и будет происходит модификация.}
\EN{The answer is simple: the arguments are stored in the stack, that is where the modification will occur.}
\RU{А вызывающие ф-ции не использует их после вызова ф-ции (обратного случая в своей практике я не видел
ни разу).}
\EN{The calling functions will not use them after the \gls{callee}'s exit (I have not seen any such case in my practice).}
...
\RU{Следовательно, модифицировать аргументы ф-ции можно запросто.}\EN{So yes, one can modify the arguments easily.}
...
But the \CCpp languages don't offer any way to access them.}
