\chapter{\IFRU{Способы передачи аргументов при вызове функций}{Arguments passing methods (calling conventions)}}
...
\IFRU{Этот способ используется почти везде в системных библиотеках win32, 
...
\\
\IFRU{Например, мы можем взять ф-цию из}{For example, we may take the function from} 
\ref{src:passing_arguments_ex} \IFRU{и изменить её немного добавив модификатор}{and change it
...
\IFRU{Он будет скомпилирован почти так же как и}{It will be compiled in almost the same way as} 
\ref{src:passing_arguments_ex_MSVC_cdecl},
\IFRU{но вы увидите}{but you will see} \TT{RET 12} \IFRU{вместо}{instead of} \TT{RET}. 
\ac{SP} \IFRU{не будет корректироваться в \glslink{caller}{вызывающей ф-ции}}
{is not aligned in \gls{caller}}.

\IFRU{Как следствие, количество аргументов ф-ции легко узнать из инструкции}{As a consequence, 
number of function arguments can be easily deduced from} \TT{RETN n} \IFRU{просто разделите
$n$ на 4}{instruction: just divide $n$ by 4}.

\lstinputlisting[caption=MSVC 2010]{calling_conventions/stdcall_ex.asm}

\subsection{\IFRU{Функции с переменным количеством аргументов}{Variable arguments number functions}}
...
\IFRU{Вот как он будет скомпилирован}{Here is how it will be compiled}:

\lstinputlisting[caption=MSVC 2010 /Ox /Ob0]{calling_conventions/fastcall_ex.asm}

\IFRU{Видно что \glslink{callee}{вызываемая ф-ция} сама возвращает}
{We see that a \gls{callee} returns} \ac{SP} 
\IFRU{при помощи инструкции}{by} \TT{RETN} \IFRU{с операндом}{instruction with operand}.
\IFRU{Так что и здесь можно легко вычислять количество аргументов}{Which means, number of arguments
...
\subsubsection{\IFRU{Передача \ITthis}{\ITthis passing}}
...
\section{\IFRU{Возвращение переменных типа \Tfloat, \Tdouble}{Returning values of \Tfloat and \Tdouble type}}
\index{float}
\index{double}
...
\section{\IFRU{Модификация аргументов}{Modifying arguments}}
