...
\RU{Это общее название для передачи некоторых аргументов через регистры, а всех остальных\EMDASH{}через стек.На более старых процессорах, это работало потенциально быстрее чем \IT{cdecl}/\IT{stdcall} (ведь стек
в памяти использовался меньше).
Впрочем, на современных (намного более сложных) CPU, выигрыша может и не быть.}
...
It may not help to gain performance on modern (much more complex) CPUs, however.}
...
\RU{Функции имеют символ подчеркивания, добавленный к концу имени функции, для отличия их от тех,
...
\RU{Код пролога функции f1() сохраняет аргументы в ``scratch space''\EMDASH{}место в стеке предназначенное
именно для этого}
\EN{The code of the f1() function's prologue saves the arguments in the ``scratch space''\EMDASH{}a space in the stack
...
\RU{Выделение ``scratch space'' в стеке лежит на ответственности вызывающей функции.}
...
только ``scratch space'' не используется, потому что незачем.}
\EN{If we compile the example with optimizations, it is to be almost the same, 
but the ``scratch space'' will not be used, because it won't be needed.}
...
\RU{А вызывающие функции не использует их после вызова функции (обратного случая в своей практике я не видел
ни разу).}
\EN{The calling functions is not using them after the \gls{callee}'s exit (I have not seen any such case in my practice).}
