...
\EN{Here we will take a look at \ac{SEH} in its isolated (from C++ and MSVC extensions) form}.
...
calling the \TT{RaiseException()} function), the \ac{OS} will find the last handler in the \ac{TIB} and will call it,
passing all information about the \ac{CPU} state (register values, etc) at the moment of the exception}.
\RU{Обработчик выясняет, то ли это исключение, для которого он создавался}\EN{The exception handler will
consider the exception, was it made for it}?
\RU{Если да, то он обрабатывает исключение}\EN{If so, it will handle the exceptin}.
...
\EN{If not, it will signal the \ac{OS} that it
cannot handle it and the \ac{OS} will call the next handler in the chain,
...
the \ac{OS} does not have any other way to handle the exception}.
\index{\oracle}
...
\EN{~---it saves huge dumps containing all possible information about the \ac{CPU} and memory state}.
...
\footnote{
	...
	\RU{Он должен компилироваться с опцией}\EN{It has to be compiled with the} SAFESEH\EN{ option}: 
...
\RU{Это пользовательские исключения}\EN{This is an user exception}. \RU{Обработчик проверяет код}\EN{The handler will check
the code}.
...
a system exceptions and a user one}? \RU{Вот системные}\EN{Here is are the system ones}:
...
\EN{The very first handler will be called --- yours, and it will know about it first, by checking
...
\RU{а будет содержать адрес глобальной переменной}\EN{but will contain the address of the global variable} \TT{new\_value}.
...
что для указателя будет использован именно регистр \EAX}\EN{I'm using MSVC 2010 here, but of course,
...
\EN{Supposedly because then the \ac{OS} will not need to care about freeing this information, these records
will be disposed of when the function finishes its execution}.
