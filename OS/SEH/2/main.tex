...
\index{Wine}
\index{ReactOS}
\RU{Исходный код ф-ции}\EN{The source code of the} \TT{\_except\_handler3} \RU{закрыт}\EN{function is closed}.
...
\\
\RU{Во время исполнения, значение}\EN{During execution, the} \IT{previous try level} \RU{в стеке меняется,
чтобы ф-ция}\EN{value in the stack changes, so} \TT{\_except\_handler3} \RU{знала о текущем уровне вложенности,
чтобы знать, какой элемент таблицы}\EN{will know about the current
level of nestedness, in order to know which} \IT{scope table} \RU{использовать}\EN{entry to use}.
...
\RU{действительно, эти поля не будут меняться}\EN{indeed, these fields will not change}.
\RU{Интересно, как меняется переменная}\EN{An interesting thing is how the} \IT{previous try level}\EN{ variable is changed}.
...
вставляет в прологе вызов ф-ции}\EN{Since the SEH setup code in the function prologue may be shared between many functions,
...
\RU{имеющий ``говорящее'' название ф-ции}\EN{and has a self-describing function name}.\\
\\
...
\RU{Если установить брякпойнт на ф-цию}\EN{If we set a breakpoint on the} \printf{} \RU{вызываемую из обработчика,
...
\EN{The \IT{scope table}\RU{\IT{Scope table} расширена, теперь имеет заголовок, содержащий 2 указателя на}
\EN{was extended to have a header consisting of two pointers to} \IT{security cookies}.
...
\EN{The \IT{security cookie}\RU{\IT{Security cookie} в стеке случайное каждый раз, так что атакующий, как мы надеемся, не мог предсказать его}
\EN{in the stack is random each time, so hopefully a remote attacker will not be able to predict it}.\\
\\
...
\EN{If this equation is not true, the process will be stopped due to stack corruption}:

\begin{center}
$security\_cookie \oplus (CookieXOROffset + address\_of\_saved\_EBP) == stack[address\_of\_saved\_EBP + CookieOffset]$
\end{center}

\RU{Если}\EN{If} \TT{Cookie Offset} \RU{равно}\EN{is} $-2$, \RU{это значит, что оно не присутствует}\EN{this means that it is not present}.
...
\EN{It is still possible to fall back to SEH3 in the compilers after (and including) MSVC 2005 by setting the \TT{/GS-} option,
however, the \ac{CRT} code will use SEH4 anyway}.
