...
\IFRU{Очень важно понимать что SEH это специфичное для MSVC}{It is very important to know that SEH is MSVC thing}.
...
\IFRU{Мы также видим адреса кода фильтра и обработчика}{We also see addresses of filter and handler code}.
...
\IFRU{Если установить брякпойнт на ф-цию}{If to set a breakpoint on} \printf{} \IFRU{вызываемую из обработчика,
...
\IFRU{Каждый элемент имеет смешение внутри стека на другое значение: это адрес \glslink{stack frame}{фрейма} (\EBP) также}
{Each element have an offset inside of stack of another value: 
this is address of \gls{stack frame} (\EBP)} \glslink{xoring}{\IFRU{про-XOR-еный}{xored}} \IFRU{с}{with} 
