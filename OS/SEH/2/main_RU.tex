\subsection{Теперь вспомним MSVC}
...
адрес \IT{scope table} может быть перезаписан, так что начиная с
 MSVC 2005, SEH3 был дополнен защитой от переполнения буфера, до SEH4.
...
\\
Изначальное значение \IT{previous try level} это $-2$ \InENRU SEH4 вместо $-1$.
