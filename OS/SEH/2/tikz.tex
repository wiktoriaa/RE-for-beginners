...
\begin{scope}[start chain=3 going below, node distance=0cm]
	...
	\fi
	\node[on chain=3,node] {\IFRU{предыдущий уровень try}{previous try level}};
...
\begin{scope}[start chain=2 going below, node distance=0cm]
	...
	\else
	% SEH4 header
	\node[on chain=2,nodew] (scope_tbl) [left=3.5 cm of n2handler] {\IFRU{смещение GS Cookie}{GS Cookie Offset}};
	\node[on chain=2,nodew] {\IFRU{смещение GS Cookie XOR}{GS Cookie XOR Offset}};
	\node[on chain=2,nodew] (EHCookieOffset) {\IFRU{смещение EH Cookie}{EH Cookie Offset}};
	\node[on chain=2,nodew] {\IFRU{смещение EH Cookie XOR}{EH Cookie XOR Offset}};
...
\end{scope}
	...
	\node[text_as_rect,left=0.5cm of scope_tbl_filter1] {\IFRU{информация для первого блока try/except}{information about first try/except block}};
	\node[text_as_rect,left=0.5cm of scope_tbl_filter2] {\IFRU{информация для второго блока try/except}{information about second try/except block}};
	\node[text_as_rect,left=0.5cm of scope_tbl_filter3] {\IFRU{информация для третьего блока try/except}{information about third try/except block}};
