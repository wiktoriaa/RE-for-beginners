...
message like} \q{This program cannot be run in DOS mode.} ---
...
\IT{PointerToRawData} \RU{называется}\EN{it called} \q{Offset}
\AndENRU \IT{VirtualAddress} \RU{называется}\EN{is called} \q{RVA} \InENRU Hiew.
...
\RU{Вероятно, линкер создает отдельный \q{thunk}, если вызовов слишком много, но по умолчанию --- не создает.}
