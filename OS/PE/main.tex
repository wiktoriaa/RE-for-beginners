...
\EN{if you run this program in DOS or Windows 3.1 (OSes which are not aware of the PE format), 
...
\begin{itemize}
\item
\RU{Модуль}\EN{Module} --- \RU{это отдельный файл}\EN{a separate file}, .exe \OrENRU{} .dll.
...
\RU{Там обычно имеются загруженные модули, память стека, \glslink{heap}{кучи}, и т.д}\EN{There usually are 
loaded modules, memory of the stack, \gls{heap}(s), etc}.

\item
\index{VA}
\ac{VA} --- \RU{это адрес, который будет использоваться в самой программе.}
\EN{an address which will used in program.}
...
в пространство процесса}\EN{the address within the process memory at which the module will be loaded}.
...
to reach an agreement which addresses will be assigned to whose modules.}
...
\EN{So that is why if two necessary DDLs for a process have the same base address,
...
\RU{Это значит, что}\EN{This mean that the} \ac{RVA} \RU{начала секции кода ~---}\EN{of the start of the code section is} \TT{0x1000}.
...
должен вызывать какие-то системные ф-ции}\EN{A shellcode trying to get executed on a compromised
system must call system functions}.
...
ф-ций, можно сказать, фиксированы и шеллкод может вызывать их напрямую}\EN{the addresses of functions were
...
\RU{Если попытаться записать что-то в эту секцию, процесс упадет}\EN{A process will crash if it tries to write to this
section}.
\end{itemize}
...
\index{TLS}
\index{BSS}
\RU{секция данных}\EN{the data section} --- \TT{.data}, \RU{константных данных}\EN{the constant data section} --- \TT{.rdata} 
\IT{(readable data)}.
...
\EN{If module is loaded at its original base address, nothing happens}.
...
the places in memory to which relocs will be applied, for example}: \figref{fig:switch_lot_olly3}.
...
\EN{For this, each DLL has an ``exports'' table, which consists of functions plus their addresses in a module}.

\RU{А у .exe-файла, либо DLL, есть ``импорты'', это таблица ф-ций требующихся для исполнения 
включая список имен DLL-файлов}
\EN{And every .exe file or DLL has ``imports'', a table of functions it needs for execution including
...
fast procedure, so there is a support for ``ordinals'' or ``hints'',
...
\RU{В таблице экспортов ординалы присутствуют всегда}\EN{Ordinals are always present in the ``export'' table}.
...
чтобы установить имена ф-ций}\EN{When loading such programs in \IDA, it will ask for a path to the mfs*.dll files
...
\RU{Самая главная структура ~--- это массив}\EN{The main structure is the array} \IT{IMAGE\_IMPORT\_DESCRIPTOR}.
...
Массив оканчивается нулем}\EN{While loading, if it is possible to find a function by ordinal,
...
\RU{места, где загрузчик будет проставлять адреса найденных ф-ций}\EN{of the place where the loader will
write the addresses of the resolved functions}.

\RU{Места где загрузчик проставляет адреса, \IDA именует их так}\EN{The points where
the loader writes addresses are marked by \IDA like this}: \IT{\_\_imp\_CreateFileA}, \RU{и т.д}\EN{etc}.
...
\EN{the address of the \IT{call} instruction (plus 1 or 2) will be added to the relocs table},
...
\EN{Possible, the linker creates individual ``thunk''s if there are too many calls to the function,
...
\EN{flag for code section. Otherwise, the program will crash while loading with error code}
5 (access denied). \\
\\
...
\EN{One might ask: what if I supply a program with a set of DLL files which is not supposed to change,
...
\EN{If the values are equal, then the loader does not do anything, and the loading of the process will be faster}.
\RU{Это называется}\EN{This is called} ``old-style binding''
\footnote{\href{http://go.yurichev.com/17050}{MSDN}.
\RU{Существует также}\EN{There is also the} ``new-style binding'',
\RU{про него напишу позже}\EN{I will write about it in the future}}.
\index{BIND.EXE}
...
even if one does not have special knowledge, by using for example the} ResHack\EN{ editor}(\myref{ResHack}).
...
\EN{The .NET loader is located in mscoree.dll, which will process the PE file}.
\index{Windows!Windows XP}
...
\RU{При старте нового треда, его}\EN{When a new thread startis, its} 
...
\EN{If they are present, they will be called before the control is passed to the main entry point} (\ac{OEP}).
