...
\EN{an address which is to be used in program.}
...
в пространство процесса}\EN{the address within the process memory at which the module is to be loaded}.
...
должен вызывать какие-то системные функции}\EN{A shellcode trying to get executed on a compromised
...
\RU{Для этого, у каждой DLL есть ``экспорты'', это таблица функций плюс их адреса в модуле}
\EN{For this, each DLL has an ``exports'' table, which consists of functions plus their addresses in a module}.

\RU{А у .exe-файла, либо DLL, есть ``импорты'', это таблица функций требующихся для исполнения 
...
``hint''-ов, это когда в таблице импортов проставлены номера функций вместо их имен}
...
(``hint'')\EMDASH\RU{``ординал'' функции}\EN{``ordinal'' of function}.
...
\RU{А все вызовы импортируемой функции это просто инструкция \CALL на соответствующий ``thunk''}
...
% FIXME1 поэтому эти импорты часто только и присутствуют в запакованных файлах...
...
% FIXME1: bibliography per chapter or section
\begin{itemize}
\item
