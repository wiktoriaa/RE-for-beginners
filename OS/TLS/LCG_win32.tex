...
\EN{One solution is to add \TT{\_\_declspec( thread )} modifier to the global variable, then it will be allocated
in the \ac{TLS} (line 9):}
...
the programmer can forget to call the \TT{my\_srand()} function to initialize the \ac{PRNG}, but the generator should be 
...
\EN{The function will initialize the \ac{PRNG} with the value returned from GetTickCount().}
