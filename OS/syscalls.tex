\chapter{\IFRU{Системные вызовы (syscall-ы)}{System calls (syscall-s)}}
...
\IFRU{это точка где соединяются вместе оба эти пространства}{is a point where these two areas are connected}.
...
\IFRU{В \gls{Windows NT} таблица сисколлов находится в \ac{SSDT}}
{As in \gls{Windows NT}, syscalls table reside in \ac{SSDT}}.
...
{For system calls intercepting and tracing in Linux,} strace(\ref{strace})\EN{ can be used}.
