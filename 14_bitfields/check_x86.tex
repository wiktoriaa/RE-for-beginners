...
{and that is value is used as the second argument for} \TT{CreateFile()}\footnote{\href{http://msdn.microsoft.com/en-us/library/aa363858(VS.85).aspx}{MSDN: CreateFile function}} function.
...
\IFRU{\TEST это то же что и \ANDIns, только без сохранения результата 
(вспомните что \CMP это то же что и \SUB, только без сохранения результатов}
{\TEST is merely the same instruction as \ANDIns, but without result saving
...
\IFRU{Если после операции \ANDIns останется этот бит, то флаг \ZF не будет поднят и условный переход 
\JZ не сработает. 
...
{If \ANDIns instruction leaving this bit, \ZF flag is to be cleared and \JZ conditional jump will not 
be triggered.
...
\lstinputlisting[caption=GCC 4.4.1]{14_bitfields/check.asm}
...
{Let's take a look into \TT{open()} function in the \TT{libc.so.6} library, but there is only syscall calling:}
...
\index{fastcall}
\label{regparm}
N.B. \IFRU{Помимо передачи параметров функции через стек, существует также возможность передавать 
...
{Aside from common passing arguments via stack,
...
\lstinputlisting[caption=do\_filp\_open() (linux kernel 2.6.31)]{\IFRU{14_bitfields/check2_ru.asm}{14_bitfields/check2_en.asm}}
...
\lstinputlisting[caption=do\_filp\_open() (linux kernel 2.6.31)]{14_bitfields/check3.asm}
