...
{There are some redundant code present, however, it is shorter then MSVC version without optimization.}
...
{That's shorter. It is worth noting that compiler works with the \EAX register part via the \AH register ~--- 
that is the \EAX register part from 8th to 15th bits inclusive.}
...
It would be more logical way to emit here \TT{``or eax, 04000h''} but that is 5 bytes, or even 6
