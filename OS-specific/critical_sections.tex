...
\IFRU{Это \glslink{atomic operation}{атомическая операция}}{This is \gls{atomic operation}}, 
...
\IFRU{Эта операция также атомическая, потому что также имеет префикс \TT{LOCK}, что означает что другие CPU
