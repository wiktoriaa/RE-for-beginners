...
\IFRU{$4$ записывается в \TT{R0}, затем, это значение, при помощи инструкции}{$4$ is stored in \TT{R0} and then,
...
\IFRU{так что, значение из регистра \TT{R0} ($4$) запишется туда, куда указывает \SP}{so, value from \TT{R0} register ($4$) will be written to a place \SP pointing to}.
...
\IFRU{Например, значение в регистре \TT{R0} подготавливается в трех местах, по адресам $0x2918$, $0x2920$ 
и $0x2928$, 
когда это можно было бы сделать в одном месте}{For instance, \TT{R0} register value is prepared in three
...
регистр \TT{R0}}{cannot be executed simultaneously, because they both modifying \TT{R0} register}. 
