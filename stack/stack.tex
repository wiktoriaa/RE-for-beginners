...
\IFRU{\POP это обратная операция ~--- сначала достает из \ESP/\RSP/\SP значение и кладет его в операнд 
...
вроде \TT{``PUSH {R4-R7,LR}''}, а в эпилоге \TT{``POP {R4-R7,PC}''} ~--- так сохраняются регистры, которые будут
...
\IT{leaf function}\footnote{\url{http://infocenter.arm.com/help/index.jsp?topic=/com.arm.doc.faqs/ka13785.html}}. Как следствие, она не использует регистр \LR. 
