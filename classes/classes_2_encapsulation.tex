...
\lstinputlisting{classes/classes2_0.cpp}
...
\lstinputlisting{classes/classes2_1.asm}
...
\lstinputlisting{classes/classes2_2.cpp}
...
\lstinputlisting{classes/classes2_3.cpp}
...
\lstinputlisting{classes/classes2_5.asm}

\IFRU{Проверим, как это работает:}{Let's check, how it works:}

\lstinputlisting{classes/classes2_4.cpp}
...
\IFRU{Си++ компилятор не позволит сгенерировать код прямо
модифицирующий защищенные поля, тем не менее, используя грязные трюки это вполне возможно.}
{C++ compiler won't allow to generate a code modifying protected fields straightforwardly, nevertheless,
it's possible using dirty hacks.}
