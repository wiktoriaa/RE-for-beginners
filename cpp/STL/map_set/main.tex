\section{std::map \AndENRU std::set}
...
\IFRU{Корневой узел это также место, куда указывает итератор \TT{.end()}, не имеющий ключа и/или значения}
{Root node is also used as a place \TT{.end()} iterator pointing to and also has no key and/or value}.

\subsection{\IFRU{Демонстрация перебалансировки}{Rebalancing demo} (GCC)}
