...
\subsection{\IFRU{Как устроена структура}{Internals}}
...
\IFRU{Здесь пока не будет листингов на ассемблере, потому что проиллюстрировать внутренности std::string 
...
\subsection{\IFRU{Чуть более сложный пример}{More complex example}}
...
\subsection{std::string \IFRU{как глобальная переменная}{as a global variable}}
