...
\IFRU{Из стандарта Си++ мы знаем}{From the C++ standard} \cite[12.1]{CPP11} 
\IFRU{что конструкторы не должны возвращать значение}{we know that constructors should not return
any values}.
\IFRU{В реальности, внутри, конструкторы возвращают указатель на созданный объект, т.е., \IT{this}}
{In fact, internally, constructors are returns pointer to the newly created object, i.e., \IT{this}}.
...
но это и так известно из основ языка Си++.}
