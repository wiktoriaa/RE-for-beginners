...
{\CALL is effectively \TT{PUSH address\_after\_call / JMP operand} instructions pair equivalent}.
...
{\RET is fetching the value from stack and jump to it ~--- it is effectively \TT{POP tmp / JMP tmp} instructions pair equivalent.}
...
programs also use stack for \ac{RA} savings, but differently}.
\IFRU{Как уже упоминалось в секции}{As it was mentioned in} ``\HelloWorldSectionName''~\ref{sec:hw_ARM}, 
\IFRU{\ac{RA} записывается в регистр}{\ac{RA} is saved to the} \LR (\IT{link register}).
...
{However, if one needs to call some another function and use the \LR register
...
{Thus, it is possible to call ``leaf'' function without using stack}.
\IFRU{Это может быть быстрее чем в x86, ведь внешняя память для стека не используется}
{This could be faster than in x86 since external RAM is not used for stack}
