...
{Technically, it's just a memory block in process memory + the \ESP or the \RSP register in x86, or the \SP register in ARM, as a pointer within this block.}
...
\IFRU{\POP это обратная операция ~--- сначала достает из указателя стека\footnote{\ESP/\RSP/\SP} значение и помещает его в операнд 
(который очень часто является регистром) и затем увеличивает указатель стека на $4$. 
Конечно, это для 32-битной среды. В x64-среде это будет $8$ а не $4$.}
{\POP is reverse operation: get a data from memory pointing by stack pointer\footnote{\ESP/\RSP/\SP}, 
put it to operand (often register) and then add $4$ to the stack pointer.
