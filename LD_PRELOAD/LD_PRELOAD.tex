\section{\IFRU{Трюк с }{}\IT{LD\_PRELOAD}\IFRU{}{ hack} \In Linux}
...
\IFRU{Например, легко перехватывать все вызовы к}{For example, it's easy to intercept all calls to the} 
time(), read(), write(), \IFRU{итд}{etc}. \\
\\
\index{uptime}
...
is working}.
\index{strace}
\IFRU{При помощи}{With the help of} strace\ref{strace}, \IFRU{можно увидеть, что эту информацию утилита получает из файла}{it's possible to see that this information the utility takes from the} \TT{/proc/uptime}
\IFRU{}{ file}:
...
{It's not a real file on disk, it's a virtual one, its contents is generated on fly in Linux kernel.}
\IFRU{Там просто два числа}{There are just two numbers}:
...
{On the other hand, if we could intercept, for example, strcmp(), and follow each string
...
{For example, here is how simple strcmp() interception is works} \In
...
\IFRU{Компилируем как динамическую библиотеку}{Let's compile it as usual dynamic library}:
...
\begin{itemize}
\IFRU{\item
\IFRU{Перехват}{Intercepting} time() \In Sun Solaris \url{http://yurichev.com/mirrors/LD_PRELOAD/sun_hack.txt}
}{}
